import { CurrentUser, GroupedReaction, SyncCustomReaction } from "../../../packages/types/index.ts";
import { notEmpty } from "../utils/notEmpty.ts";
import { findGroupedReaction, getCustomReaction, getStandardReaction, isExistingReaction, StandardReaction } from "../utils/reactions/index.ts";
export function removeGroupedReactionByEmoji({ grouped_reactions, reaction_id, display_name }: {
    ...
    return grouped_reactions
        .map((r) => {
        if (r.viewer_reaction_id !== reaction_id)
            return r;
...
export function addGroupedReaction({ grouped_reactions, viewer_reaction_id, reaction, currentUser }: {
    grouped_reactions: GroupedReaction[];
    ...
    if (existingGroupedReaction) {
        return grouped_reactions.map((r) => {
            if (!isExistingReaction(r, reaction))
                return r;
            return {
                ...r,
...
export function updateGroupedReaction({ grouped_reactions, id, data }: {
    grouped_reactions: GroupedReaction[];
    id: string;
    data: Partial<GroupedReaction>;
}): GroupedReaction[] {
    return grouped_reactions.map((r) => {
        if (r.viewer_reaction_id !== id)
            return r;
        return {
            ...r,
            ...data

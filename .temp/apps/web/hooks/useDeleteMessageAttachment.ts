...
import { MessagePage } from "../../../packages/types/index.ts";
...
export function useDeleteMessageAttachment() {
    ...
    return useOptimisticMutation({
        mutationFn: ({ attachmentId, messageId }: UseDeleteMessageAttachmentInput) => apiClient.organizations.deleteMessagesAttachmentsById().request(`${scope}`, messageId, attachmentId),
        optimisticFns: (props) => [
            {
                query: {
                            ...
                            results: page.data.map((message) => {
                                if (message.id === props.messageId) {
                                    ...
                                    return {
                                        ...
                                        reply: {
                                            ...message.reply,
                                            last_attachment: message.reply.last_attachment?.id === props.attachmentId ? null : message.reply.last_attachment
                                        }
                                    };
                                }
                                else {

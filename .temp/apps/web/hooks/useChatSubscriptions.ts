...
import { MessageThread, PusherInvalidateMessage } from "../../../packages/types/index.ts";
import { useAreBrowserNotificationsEnabled } from "./useAreBrowserNotificationsEnabled.ts";
import { useBindCurrentUserEventOnceInDesktopApp } from "./useBindCurrentUserEventOnceInDesktopApp.ts";
import { useCreateDesktopOrBrowserNotification } from "./useCreateDesktopOrBrowserNotification.tsx";
import { useGetCurrentUser } from "./useGetCurrentUser.ts";
import { useQueryNormalizer } from "../utils/normy/QueryNormalizerProvider.tsx";
import { apiClient, setTypedInfiniteQueriesData, setTypedQueriesData } from "../utils/queryClient.ts";
import { setNormalizedData } from "../utils/queryNormalization.ts";
import { useAppFocused } from "./useAppFocused.ts";
...
export const useChatSubscriptions = () => {
    ...
    const invalidateThreadQueries = useCallback((e: PusherInvalidateMessage, type: string) => {
        const { message_thread, message } = e;
        const isNewMessageInFocusedThread = (type === 'new-message' && isFocused && router.query?.threadId === message_thread.id) ||
        ...
        if (type === 'new-message') {
            setTypedInfiniteQueriesData(queryClient, getMessages.requestKey({ orgSlug: message_thread.organization_slug, threadId: message_thread.id }), (old) => {
                if (!old?.pages.length)
                    return old;
    ...
    const hardInvalidateThreadQueries = useCallback(({ message_thread_id, organization_slug }: {
    ...
    const updateThread = useCallback((newData: Partial<MessageThread> & Required<Pick<MessageThread, 'organization_slug' | 'id'>>) => {
        setNormalizedData({ queryNormalizer, type: 'thread', id: newData.id, update: newData });
    }, [queryNormalizer]);
    const optimisticallyMarkThreadRead = useCallback((threadId: string) => setNormalizedData({
        queryNormalizer,
        type: 'thread',
        id: threadId,
        update: { unread_count: 0, manually_marked_unread: false }
    }), [queryNormalizer]);
    const optimisticallyMarkThreadUnread = useCallback((threadId: string) => setNormalizedData({
        queryNormalizer,
        type: 'thread',
        id: threadId,
        update: { unread_count: 1, manually_marked_unread: true }
    }), [queryNormalizer]);
    const createMessageNotification = useCallback((e: PusherInvalidateMessage) => {
        const { message_thread, message, skip_push, push_body } = e;
        if (message.sender.user.id === currentUser?.id || skip_push)
            return;
    ...
    const createMessageNotificationUnlessFocused = useCallback((e: PusherInvalidateMessage) => {
        const { message_thread } = e;
        if (isFocused && router.query?.threadId === message_thread.id)
            return;
        createMessageNotification(e);
    }, [createMessageNotification, isFocused, router.query?.threadId]);
    const removeThreadQueries = useCallback(({ message_thread_id, organization_slug }: {
        message_thread_id: string;
        organization_slug: string;
    }) => {
        queryClient.invalidateQueries({ queryKey: getThreads.requestKey(organization_slug) });

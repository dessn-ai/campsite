...
import { useInboxSelectedItemId, useInboxSetSelection } from "../components/InboxItems/hooks/useInboxSelectedItemId.ts";
...
import { getInboxItemRoutePath, getInboxItemSplitViewPath } from "../utils/getInboxItemRoutePath.ts";
import { apiClient } from "../utils/queryClient.ts";
import { throttle } from "../utils/throttle.ts";
import { useBindCurrentUserEvent } from "./useBindCurrentUserEvent.ts";
import { useMarkNotificationRead } from "./useMarkNotificationRead.ts";
...
export const useInboxItemSubscriptions = () => {
    ...
    const markNotificationForCurrentPageRead = useCallback((n: NotificationData) => {
    ...
    const triggerPushNotification = useCallback((n: NotificationData) => {
        if (selectedItemInboxId === n.id)
            return;
        if (Router.query.inboxItemKey === n.inbox_key)
            return;
        if (n.subject.type === 'Reaction')
            return;
        if (n.skip_push)
            return;

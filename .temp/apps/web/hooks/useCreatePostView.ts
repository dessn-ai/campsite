...
import { OrganizationsOrgSlugPostsPostIdViewsPostRequest, PostView } from "../../../packages/types/index.ts";
...
import { createNormalizedOptimisticUpdate, getNormalizedData, setNormalizedData } from "../utils/queryNormalization.ts";
...
export function useCreatePostView() {
    ...
    return useMutation({
        mutationFn: ({ postId, ...data }: Props) => apiClient.organizations.postPostsViews().request(`${scope}`, postId, data),

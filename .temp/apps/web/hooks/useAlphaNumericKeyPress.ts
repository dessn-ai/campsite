...
import { isAlphaNumeric } from "../utils/isAlphaNumeric.ts";
import { isKeyboardCharacter } from "../utils/isKeyboardCharacter.ts";
...
export function useAlphaNumericKeyPress(onKeyPress: (key: string) => void, opts?: Options) {
    useEffect(() => {
        function handleKeyPress(event: globalThis.KeyboardEvent) {
            ...
            if (opts?.exceptions?.includes(key))
                return;
            if (key === ' ')
                return;
            if (metaKey || ctrlKey)
            ...
            if (!isValidKey)
                return;
            if (DISABLED_TAGS.includes((target as HTMLElement).tagName))
                return;
            onKeyPress(key);
        }

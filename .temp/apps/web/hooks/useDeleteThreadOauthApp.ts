...
export function useDeleteThreadOauthApp({ threadId }: {
    threadId: string;
}) {
    const { scope } = useScope();
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (id: string) => apiClient.organizations.deleteThreadsOauthApplicationsById().request(`${scope}`, threadId, id),
        onSuccess(_data, id) {

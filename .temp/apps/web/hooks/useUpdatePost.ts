...
import { OrganizationsOrgSlugPostsPostIdPutRequest, Post } from "../../../packages/types/index.ts";
import { usePusherSocketIdHeader } from "../contexts/pusher.tsx";
import { useScope } from "../contexts/scope.tsx";
import { useGetCurrentPostsFeedQueryKey } from "./useGetCurrentPostsFeedQueryKey.ts";
...
import { createNormalizedOptimisticUpdate, getNormalizedData } from "../utils/queryNormalization.ts";
import { createOptimisticTimelineEvent, insertPostTimelineEvent, isDateWithinRollupWindow, useOptimisticTimelineEventMemberActor } from "../utils/timelineEvents/optimistic.ts";
...
export function useUpdatePost() {
    ...
    return useMutation({
        ...
        mutationFn: ({ id, ...data }: Props) => apiClient.organizations.putPostsByPostId().request(`${scope}`, id, data, { headers: pusherSocketIdHeader }),
        onMutate: async ({ id, ...data }: Props) => {
            ...
            if (data.project_id && previousPost?.project_pin_id && previousPost.project.id !== data.project_id) {
                ...
                setTypedQueryData(queryClient, apiClient.organizations.getProjectsPins().requestKey(`${scope}`, previousPost.project.id), (oldData) => {
                    return {
                        ...oldData,

...
import { OrganizationsOrgSlugMembersMemberUsernameProjectMembershipListPutRequest } from "../../../packages/types/index.ts";
...
export function useUpdateMemberProjectMemberships({ memberUsername }: {
    memberUsername: string;
}) {
    const { scope } = useScope();
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (data: OrganizationsOrgSlugMembersMemberUsernameProjectMembershipListPutRequest) => apiClient.organizations.putMembersProjectMembershipList().request(`${scope}`, memberUsername, data),

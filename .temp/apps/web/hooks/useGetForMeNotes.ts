...
import { GetMembersMeForMeNotesParams } from "../../../packages/types/index.ts";
...
export function useGetForMeNotes({ enabled = true, order = { by: 'created_at', direction: 'desc' }, query }: Props) {
    ...
    return useInfiniteQuery({
        ...
        queryFn: ({ pageParam }) => getMembersMeForMeNotes.request({ orgSlug: `${scope}`, after: pageParam, order, q: query }),

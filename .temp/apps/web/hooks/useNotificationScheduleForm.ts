...
import { z } from 'zod';
import { NotificationSchedule, UsersMeNotificationSchedulePutRequest } from "../../../packages/types/generated.ts";
import { useDeleteNotificationSchedule } from "./useDeleteNotificationSchedule.ts";
import { useUpdateNotificationSchedule } from "./useUpdateNotificationSchedule.ts";
...
export function useNotificationScheduleForm({ notificationSchedule }: {
...
export function useOnSubmitNotificationScheduleForm({ onSuccess }: OnSubmitOptions = {}) {
    ...
    return {
        onSubmit: useCallback<SubmitHandler<NotificationScheduleFormSchema>>((data) => {
            if (data.type === 'none') {

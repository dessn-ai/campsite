...
import { PusherInvalidateMessage, User } from "../../../packages/types/index.ts";
...
export function useChatTypingIndicator({ threadId, channelName }: UseChatTypingIndicatorOptions) {
    ...
    const queueTypingIndicator = useCallback((e: {
        user: User;
    }) => {
        ...
        setTypers((users) => {
            if (e.user.id === currentUser?.id)
                return users;
            if (users.some((user) => user.id === e.user.id))
                return users;
            return [...users, e.user];
        });
    ...
    const removeTypingIndicator = useCallback((e: PusherInvalidateMessage) => {
        if (e.message_thread.id !== threadId)
            return;

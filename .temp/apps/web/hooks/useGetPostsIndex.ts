...
import { useGetCurrentMemberPosts } from "./useGetCurrentMemberPosts.ts";
import { useGetCurrentUser } from "./useGetCurrentUser.ts";
import { useGetForMePosts } from "./useGetForMePosts.ts";
import { useGetPosts } from "./useGetPosts.ts";
...
export const sortAtom = atomFamily(({ scope, filter }: {
    ...
    filter: string;
}) => atomWithWebStorage<PostIndexSortType>(`${scope}:posts-index-sort:${filter}`, 'last_activity_at'), (a, b) => a.scope === b.scope && a.filter === b.filter);
export const filterAtom = atomFamily(({ scope }: {
    scope: CookieValueTypes;
}) => atomWithWebStorage<PostsIndexFilterType>(`${scope}:posts-index-filter`, DEFAULT_FILTER), (a, b) => a.scope === b.scope);
interface Props {
    enabled?: boolean;

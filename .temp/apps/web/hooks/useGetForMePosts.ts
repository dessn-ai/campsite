...
import { GetMembersMeForMePostsParams } from "../../../packages/types/index.ts";
...
export function useGetForMePosts({ enabled = true, order = { by: 'last_activity_at', direction: 'desc' }, query: _query, hideResolved = false }: Options = {}) {
    const query = _query ?? '';
    const { scope } = useScope();
    return useInfiniteQuery({
        queryKey: getMembersMeForMePosts.requestKey({ orgSlug: `${scope}`, order, q: query, hide_resolved: hideResolved }),
        queryFn: ({ pageParam }) => getMembersMeForMePosts.request({
            orgSlug: `${scope}`,
            order,
            q: query,
            limit: 20,

...
import { OrganizationsOrgSlugNotesNoteIdProjectPermissionsPutRequest } from "../../../packages/types/generated.ts";
import { useScope } from "../contexts/scope.tsx";
import { useQueryNormalizer } from "../utils/normy/QueryNormalizerProvider.tsx";
import { apiClient, getTypedQueryData, setTypedInfiniteQueriesData, setTypedQueryData } from "../utils/queryClient.ts";
...
export function useUpdateNoteProjectPermission() {
    ...
    return useMutation({
        ...
        mutationFn: ({ noteId, ...data }: Props) => apiClient.organizations.putNotesProjectPermissions().request(`${scope}`, noteId, data),
        onMutate: ({ noteId, ...data }) => {
            ...
            if (previousNote?.project && previousNote.project.id !== data.project_id) {
                setTypedInfiniteQueriesData(queryClient, apiClient.organizations
                    .getProjectsNotes()
                    .requestKey({ orgSlug: `${scope}`, projectId: previousNote.project.id }), (old) => {
                    return old
                        ? {
                            ...old,
                ...
                if (previousNote?.project_pin_id) {
                    setTypedQueryData(queryClient, apiClient.organizations.getProjectsPins().requestKey(`${scope}`, previousNote.project.id), (oldData) => {
                        return {
                            ...oldData,

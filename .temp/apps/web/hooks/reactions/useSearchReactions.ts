...
import { SyncCustomReaction } from "../../../../packages/types/generated.ts";
import { commandScore } from "../../../../packages/ui/src/utils/index.ts";
import { formatReactionData, StandardReaction } from "../../utils/reactions/index.ts";
import { isStandardReactionSkin, ReactionData } from "../../utils/reactions/data.ts";
import { useReactionsData } from "./useReactionsData.ts";
export function useSearchReactions(query: string, { maxResults = 10, hideCustomReactions }: {
    maxResults?: number;
    ...
    const reactionSearchResults: (StandardReaction | SyncCustomReaction)[] = useMemo(() => {
        ...
        const reactions = Object.values(reactionsData?.reactions ?? {}).filter((reaction) => {
            ...
            if (hideCustomReactions)
                return isStandardReactionSkin(skin);
            return true;
        });
        ...
        for (const normalizedQuery of normalizedQueries) {
            ...
            for (const reaction of pool) {
                 ...
                 *
                 */
                const fuzzyScore = Math.max(commandScore(reaction.id.toLowerCase(), normalizedQuery), commandScore(reaction.name.toLowerCase(), normalizedQuery) * 0.95, ...reaction.keywords.map((keyword) => commandScore(keyword.toLowerCase(), normalizedQuery) * 0.75), ...reaction.emoticons.map((emoticons) => commandScore(emoticons.toLowerCase(), normalizedQuery) * 0.5));
        ...
        if (results.length < 2)
            return results.map(formatReactionData);
        results.sort((a, b) => {

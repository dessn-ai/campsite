...
import { getFromStorage } from "../../utils/getFromStorage.ts";
import { notEmpty } from "../../utils/notEmpty.ts";
import { formatReactionData, isStandardReaction } from "../../utils/reactions/index.ts";
import { useReactionsData } from "./useReactionsData.ts";
const didMigrateEmojiMartCacheAtom = atomWithWebStorage<boolean>('reactions:did-migrate-emoji-mart-cache', false);
const lastUsedReactionIdAtom = atomFamily((scope: string) => atomWithWebStorage<string | null>(`reactions:last-used:${scope}`, null));
const frequentReactionIdsMapAtom = atomFamily((scope: string) => atomWithWebStorage<Record<string, number> | null>(`reactions:frequent:${scope}`, null));
...
export function useAddFrequentlyUsedReaction() {
    ...
    const addReactionIdToFrequents = useCallback(({ id }: {
        id: string;
    }) => {
...
export function useFrequentlyUsedReactions({ hideCustomReactions }: {
    hideCustomReactions?: boolean;
} = {}) {
    const { scope } = useScope();
    ...
    useEffect(() => {
        ...
        if (emojiMartLastUsedCache)
            setLastUsedReactionId(emojiMartLastUsedCache);
        if (emojiMartFrequentCache)
            setFrequentReactionIdsMap(emojiMartFrequentCache);
    ...
    const frequentlyUsedReactions = useMemo(() => {
        ...
        const reactions = Object.keys(newFrequentIds)
            ...
            .filter((reaction) => {
            if (hideCustomReactions)
                return isStandardReaction(reaction);
            return true;
        });
        ...
        if (reactions.length < DEFAULT_FREQUENT_REACTIONS.length) {
            const defaultReactionsNotInList = DEFAULT_FREQUENT_REACTIONS.filter((defaultId) => !reactions.some((reaction) => reaction.id === defaultId));

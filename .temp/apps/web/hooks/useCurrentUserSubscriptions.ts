...
import { CallRoomInvitation, incomingCallRoomInvitationAtom } from "../atoms/call.ts";
...
export const useCurrentUserSubscriptions = () => {
    ...
    const updateCurrentUser = useCallback(({ current_user }: {
    ...
    const openIncomingCallInvitation = useCallback((invitation: CallRoomInvitation) => {
        setIncomingCallRoomInvitation(invitation);
    }, [setIncomingCallRoomInvitation]);
    const closeIncomingCallInvitation = useCallback(({ call_room_id }: {
        call_room_id: string;
    }) => {
        setIncomingCallRoomInvitation((prev) => (prev?.call_room_id === call_room_id ? undefined : prev));
    }, [setIncomingCallRoomInvitation]);
    const createIncomingCallNotification = useCallback((invitation: CallRoomInvitation) => {
        if (invitation.skip_push)
            return;

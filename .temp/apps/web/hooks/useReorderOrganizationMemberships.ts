...
import { PublicOrganizationMembership } from "../../../packages/types/index.ts";
...
export function useReorderOrganizationMemberships() {
    ...
    const onReorder = useCallback((ids: string[]) => {
        setTypedQueryData(queryClient, getOrganizationMemberships.requestKey(), (prev) => {
            if (!prev)
                return prev;
            const idToMembership = prev.reduce((acc, membership) => {
    ...
    const mutation = useMutation({
        mutationFn: (membership_ids: string[]) => apiClient.organizationMemberships.putOrganizationMembershipsReorder().request({
            membership_ids
        }),
        onMutate: async (ids) => {

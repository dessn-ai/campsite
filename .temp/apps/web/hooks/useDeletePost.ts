...
import { useGoBack } from "../components/Providers/HistoryProvider.tsx";
...
export function useDeletePost() {
    ...
    return useMutation({
        mutationFn: ({ post }: Props) => apiClient.organizations.deletePostsByPostId().request(`${scope}`, post.id, { headers }),
        onMutate: async ({ post }: Props) => {
            ...
            if (post.project_pin_id) {
                setTypedQueryData(queryClient, apiClient.organizations.getProjectsPins().requestKey(`${scope}`, post.project.id), (oldData) => {
                    return {
                        ...oldData,

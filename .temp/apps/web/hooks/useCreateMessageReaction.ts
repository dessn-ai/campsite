...
export function useCreateMessageReaction() {
    ...
    return useMutation({
        scope: { id: 'reaction' },
        mutationFn: ({ messageId, reaction }: Props) => postMessagesReactions.request(`${scope}`, messageId, { content: getStandardReaction(reaction)?.native, custom_content_id: getCustomReaction(reaction)?.id }, { headers: pusherSocketIdHeader }),

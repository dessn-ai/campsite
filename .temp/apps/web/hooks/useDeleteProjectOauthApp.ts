...
export function useDeleteProjectOauthApp({ projectId }: {
    projectId: string;
}) {
    const { scope } = useScope();
    const queryClient = useQueryClient();
    return useMutation({
        mutationFn: (id: string) => apiClient.organizations.deleteProjectsOauthApplicationsById().request(`${scope}`, projectId, id),
        onSuccess(_data, id) {

...
import { OrganizationProjectMembersGetRequest } from "../../../packages/types/generated.ts";
...
export function useGetProjectMembers({ projectId, organizationMembershipId, after, limit, roles, excludeRoles }: Options) {
    ...
    const result = useInfiniteQuery({
        ...
        queryFn: async ({ pageParam }) => {
            ...
            results.data.forEach((member) => {
                setTypedQueryData(queryClient, apiClient.organizations.getMembersByUsername().requestKey(`${scope}`, member.user.username), member);
            });
            return results;
        },

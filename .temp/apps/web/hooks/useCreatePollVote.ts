...
export function useCreatePollVote() {
    ...
    return useMutation({
        mutationFn: ({ postId, optionId }: Props) => apiClient.organizations.postPostsPoll2OptionsVote().request(`${scope}`, postId, optionId),
        onMutate: async ({ postId, optionId }) => {
            ...
            if (currentUser) {
                setTypedInfiniteQueriesData(queryClient, apiClient.organizations
                    ...
                    .requestKey({ orgSlug: `${scope}`, postId, pollOptionId: optionId }), (old) => {

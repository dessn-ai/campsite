...
import { GetProjectMembershipsData, ProjectMembership } from "../../../packages/types/index.ts";
...
export function setProjectMembershipsCache(queryClient: ReturnType<typeof useQueryClient>, scope: CookieValueTypes, updater: Updater<GetProjectMembershipsData | undefined, GetProjectMembershipsData | undefined>): GetProjectMembershipsData | undefined {
    return setTypedQueryData(queryClient, getProjectMemberships.requestKey(`${scope}`), updater);
}
export function updateProjectMembershipByProjectId(queryClient: ReturnType<typeof useQueryClient>, scope: unknown, projectId: string, attributes: Partial<ProjectMembership>): GetProjectMembershipsData | undefined {
    ...
    return useQuery({
        ...
        queryFn: async () => {
            ...
            results.forEach((membership) => {
                setTypedQueryData(queryClient, getProjectsByProjectId.requestKey(`${scope}`, membership.project.id), membership.project);
            });
            return results;
        },
        enabled

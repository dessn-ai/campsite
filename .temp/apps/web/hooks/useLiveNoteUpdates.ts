...
import { useChannel } from "./useChannel.ts";
...
export function useLiveNoteUpdates(note: Note | undefined) {
    ...
    const invalidateReactionQueries = useCallback(function (e: {
        subject_id: string;
        subject_type: string;
        user_id: string;
    }) {
    ...
    const invalidateCommentQueries = useCallback(function (e: {
        subject_id: string;
        user_id: string;
    ...
    const invalidateViewsQuery = useCallback(function (e: {
        user_id: string | null;
    }) {
        if (!note?.id || !e.user_id)
            return;
    ...
    const invalidateNoteTimelineQuery = useCallback(function () {
        if (!note?.id)
            return;
        queryClient.invalidateQueries({

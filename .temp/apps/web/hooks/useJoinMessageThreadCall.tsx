...
export function useJoinMessageThreadCall({ thread }: Props) {
    ...
    const singleChatWithDeactivatedMember = thread?.other_members.length === 1 && !!thread?.other_members.at(0)?.deactivated;
    const canJoin = currentUser && !currentUser.on_call && !singleChatWithDeactivatedMember && !!thread?.remote_call_room_id;
    const isDesktopApp = useIsDesktopApp();
    const join = useCallback(() => {
        if (!thread?.call_room_url)
            return;
        if (isDesktopApp) {

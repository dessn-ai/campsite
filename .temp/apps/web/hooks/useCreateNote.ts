...
import { Note, NotePage, OrganizationsOrgSlugNotesPostRequest } from "../../../packages/types/index.ts";
...
export function useCreateNote({ afterCreate }: Props = {}) {
    ...
    return useMutation({
        mutationFn: (data: OrganizationsOrgSlugNotesPostRequest | void) => apiClient.organizations.postNotes().request(`${scope}`, data ?? {}),

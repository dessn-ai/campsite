...
import { OrganizationNotificationMarkAllReadPostRequest, PublicOrganization } from "../../../packages/types/index.ts";
...
export function useMarkAllNotificationsRead({ organization }: Options = {}) {
    ...
    return useMutation({
        mutationFn: (data: OrganizationNotificationMarkAllReadPostRequest | void) => apiClient.organizations.postMembersMeNotificationsMarkAllRead().request(orgSlug, data || {}),
        onMutate: async () => {
            await Promise.all([

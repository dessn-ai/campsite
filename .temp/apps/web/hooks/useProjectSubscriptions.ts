...
import { useCreateProjectView } from "./useCreateProjectView.ts";
export const useProjectSubscriptions = () => {
    ...
    const markProjectRead = useCallback(({ project_id }: {
        project_id: string;
    }) => {
        setNormalizedData({
            queryNormalizer,
    ...
    const markProjectUnread = useCallback(({ project_id }: {
        project_id: string;
    }) => {
    ...
    const updateProject = useCallback((newData: Partial<Project> & Required<Pick<Project, 'id'>>) => {
        setNormalizedData({
            queryNormalizer,
            type: 'project',
            id: newData.id,

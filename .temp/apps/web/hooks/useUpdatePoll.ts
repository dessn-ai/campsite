...
import { OrganizationsOrgSlugPostsPostIdPoll2PutRequest } from "../../../packages/types/index.ts";
...
export function useUpdatePoll({ postId }: Props) {
    ...
    return useMutation({
        mutationFn: (data: OrganizationsOrgSlugPostsPostIdPoll2PutRequest) => apiClient.organizations.putPostsPoll2().request(`${scope}`, postId, data),
        onMutate: (data) => {

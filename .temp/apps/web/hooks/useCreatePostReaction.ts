...
export function useCreatePostReaction(postId: string) {
    ...
    return useMutation({
        scope: { id: 'reaction' },
        mutationFn: ({ reaction }: Props) => postPostsReactions.request(`${scope}`, postId, { content: getStandardReaction(reaction)?.native, custom_content_id: getCustomReaction(reaction)?.id }, { headers: pusherSocketIdHeader }),
        onMutate: ({ reaction }) => {

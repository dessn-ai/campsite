...
import { Attachment, Message, OrganizationsOrgSlugThreadsThreadIdMessagesPostRequest, PusherInvalidateMessage } from "../../../packages/types/index.ts";
...
import { useGetOrganizationMember } from "./useGetOrganizationMember.ts";
import { setServerIdToOptimisticIdAtom } from "./useUploadChatAttachments.tsx";
type Props = Omit<OrganizationsOrgSlugThreadsThreadIdMessagesPostRequest, 'attachments'> & {
    threadId: string;
    attachments?: Attachment[];
};
const { createAtom: createMessageStateAtom, setStateAtom: setCreateMessageStateAtom, updateStateAtom: updateCreateMessageStateAtom, removeStateAtom: removeCreateMessageStateAtom } = createRetryAtoms<Props>();
...
function transformAttachments(attachments?: Attachment[]): OrganizationsOrgSlugThreadsThreadIdMessagesPostRequest['attachments'] {
...
export function useCreateMessage() {
    ...
    return useMutation({
        ...
        mutationFn: ({ threadId, attachments, ...data }: Props) => postThreadsMessages.request(`${scope}`, threadId, { attachments: transformAttachments(attachments), ...data }, { headers: pusherSocketIdHeader }),
        onMutate: async (props) => {
            ...
            setTypedInfiniteQueriesData(queryClient, messagesQueryKey, (old) => {
                if (!membership || !old?.pages.length)
                    return old;
...
export function useRetryCreateMessage() {
    ...
    const { onSuccess, onError } = useCreateMessageCallbacks();
    return useMutation({
        mutationFn: ({ threadId, attachments, optimisticId: _, ...data }: RetryProps) => postThreadsMessages.request(`${scope}`, threadId, { attachments: transformAttachments(attachments), ...data }, { headers: pusherSocketIdHeader }),

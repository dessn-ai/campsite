...
import { MoonIcon } from "../../../packages/ui/src/Icons/index.tsx";
import { buildMenuItems, MenuItem } from "../../../packages/ui/src/Menu/index.ts";
import { delay } from "../../../packages/ui/src/utils/index.ts";
import { useCreateNotificationPause } from "./useCreateNotificationPause.ts";
import { useDeleteNotificationPause } from "./useDeleteNotificationPause.ts";
import { useGetCurrentUser } from "./useGetCurrentUser.ts";
import { useGetNotificationSchedule } from "./useGetNotificationSchedule.ts";
...
export function usePauseNotificationMenuItem({ setNotificationPauseCalendarDialogOpen, setNotificationScheduleDialogOpen }: {
    ...
    const formattedExpiration = currentUser?.notification_pause_expires_at &&
    ...
    const onSelectNotificationPauseExpiration = async (date: Date) => {
        await delay(0);
        createNotificationPause({ expires_at: date.toISOString() }, {
            onSuccess: () => {
    ...
    const resumeNotificationsItems = buildMenuItems([
        formattedExpiration && {
            type: 'text',
            label: createElement(Fragment, null, formattedExpiration.formattedDay ? `Paused until ${formattedExpiration.formattedDay} at ` : 'Paused until ', createElement('span', { className: 'whitespace-nowrap' }, formattedExpiration.formattedTime))
        },
        {
            type: 'item',

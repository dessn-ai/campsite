...
import { Attachment, Comment, CommentPage, CurrentUser, GroupedReaction, OrganizationMember, OrganizationPostComments2PostRequest, PostPage } from "../../../packages/types/index.ts";
import { EMPTY_HTML } from "../atoms/markdown.ts";
import { isAudio, isGif, isImage, isLottie, isOrigami, isPrinciple, isStitch, isVideo } from "../components/Post/utils.ts";
import { useScope } from "../contexts/scope.tsx";
import { useQueryNormalizer } from "../utils/normy/QueryNormalizerProvider.tsx";
import { apiClient, getTypedQueryData, setTypedInfiniteQueriesData, setTypedQueriesData } from "../utils/queryClient.ts";
import { getNormalizedData, setNormalizedData } from "../utils/queryNormalization.ts";
import { createRetryAtoms } from "../utils/retryAtoms.ts";
import { stripHtml } from "../utils/stripHtml.ts";
import { TransformedFile } from "../utils/types.ts";
import { bumpPostToTop } from "./useCreatePost.ts";
...
export const { createAtom: createCommentStateAtom, setStateAtom: setCommentStateAtom, updateStateAtom: updateCommentStateAtom, removeStateAtom: removeCommentStateAtom } = createRetryAtoms<CreateCommentData | CreateReplyData>();
...
const replaceOptimisticPageQueryData = (optimisticId: string, newComment: Comment) => (old: InfiniteData<CommentPage> | undefined) => {
    if (!old)
        return;
    return {
        ...old,
...
async function optimisticUpdate({ scope, queryClient, queryNormalizer, subjectId, subjectType, data, parentCommentId, optimisticId, currentUser }: OptimisticUpdateProps) {
    ...
    const getPostAttachmentCommentsKey = attachmentId &&
        getPostAttachmentComments.requestKey({
            orgSlug: `${scope}`,
            postId: subjectId,
            attachmentId
        });
    const getNoteAttachmentCommentsKey = attachmentId &&
    ...
    if (subjectType === 'post') {
        setNormalizedData({
            ...
            update: (old) => ({
                preview_commenters: {
                    ...
                    latest_commenters: [newComment.member, ...old.preview_commenters.latest_commenters].filter((member, index, self) => self.findIndex((m) => m.user.id === member.user.id) === index)
                },
                ...
                feedback_requests: old.feedback_requests?.map((request) => {
                    if (request.member.user.id !== currentUser?.id)
                        return request;
                    return {
                        ...request,
                        has_replied: true
        ...
        setNormalizedData({
            ...
            update: (old) => ({
                comments_count: old.comments_count + 1,
                latest_commenters: [newComment.member, ...old.latest_commenters].filter((member, index, self) => self.findIndex((m) => m.user.id === member.user.id) === index)
            })
        });
    }
    return newComment;
...
export function useCreateCommentCallbacks({ subjectId, subjectType, onOptimisticCreate, onServerCreate }: Props) {
    ...
    const onSuccess = ({ optimisticId, transformedFiles, newComment, latestCommenters, attachmentCommenters }: CommentCreateSuccessProps) => {
        ...
        if (subjectType === 'post') {
            ...
            if (post?.project) {
                setTypedInfiniteQueriesData(queryClient, getProjectsPosts.requestKey({
        ...
        if (newComment.attachment_id && attachmentCommenters) {
            setTypedQueriesData(queryClient, apiClient.organizations.getAttachmentsCommenters().requestKey(`${scope}`, newComment.attachment_id), attachmentCommenters);
        }
        const getPostCommentsKey = getPostComments.requestKey({ orgSlug: `${scope}`, postId: subjectId });
        const getNoteCommentsKey = getNoteComments.requestKey({ orgSlug: `${scope}`, noteId: subjectId });
        // replace the comment in the list cache. this is necessary to replace the temp ID with the server ID
        setTypedInfiniteQueriesData(queryClient, getPostCommentsKey, replaceOptimisticPageQueryData(optimisticId, newComment));
        setTypedInfiniteQueriesData(queryClient, getNoteCommentsKey, replaceOptimisticPageQueryData(optimisticId, newComment));
        if (newComment.attachment_id) {
            ...
            setTypedInfiniteQueriesData(queryClient, getPostAttachmentCommentsKey, replaceOptimisticPageQueryData(optimisticId, newComment));
            setTypedInfiniteQueriesData(queryClient, getNoteAttachmentCommentsKey, replaceOptimisticPageQueryData(optimisticId, newComment));
        }
            ...
            update: (old) => {
                ...
                */
                let newAttachments = newComment.attachments.map((file) => {
                    const optimisticFile = transformedFiles.find((transformedFile) => file.url.includes(transformedFile.key as string));
                    if (!optimisticFile)
                        return file;
                    return {
                        ...file,

...
import { useLayeredHotkeys } from "../../../packages/ui/src/DismissibleLayer/index.tsx";
import { useCallbackRef } from "../../../packages/ui/src/hooks/index.tsx";
type Item = {
    groupIndex: number;
    itemIndex: number;
};
export type SelectGroupItemFn = (args: {
    groupIndex: number;
    itemIndex: number;
    scroll?: boolean;
}) => void;
export function useGroupedListNavigation<T extends {
    id: string;
}>({ initialActiveItemId, groups, getItemDOMId }: {
    ...
    if (activeItemRef.current) {
        ...
        else if (activeItemRef.current.itemIndex >= groups[Object.keys(groups)[activeItemRef.current.groupIndex]].length) {
            activeItemRef.current = null;
        }
    }
    const { getItemElement, selectNextItem, selectPreviousItem, selectItem, resetActiveItem } = useMemo(() => {
        const focusActiveItem = ({ scroll }: {
            scroll?: boolean;
        } = { scroll: true }) => {
        ...
        return {
            ...
            selectItem: ({ groupIndex, itemIndex, scroll = true }: {
                groupIndex: number;
                itemIndex: number;
                scroll?: boolean;
            }) => {
     ...
     */
    useLayoutEffect(() => {
        if (!initialActiveItemIdRef.current)
            return;
        const groupKeys = Object.keys(groupsRef.current);
        const groupIndex = groupKeys.findIndex((key) => groupsRef.current[key].some((p) => p.id === initialActiveItemIdRef.current));
        const postIndex = groupsRef.current[groupKeys[groupIndex]]?.findIndex((p) => p.id === initialActiveItemIdRef.current) ?? -1;
        if (groupIndex === -1 || postIndex === -1)
            return;
        selectItem({ groupIndex, itemIndex: postIndex, scroll: false });
    }, [selectItem]);
    return { selectItem, resetActiveItem };
}
type SelectItemFn = (args: {
    itemIndex: number;
    scroll?: boolean;
}) => void;
export function useListNavigation<T extends {
    id: string;
}>({ initialActiveItemId, items, getItemDOMId }: {
    initialActiveItemId?: string;
    items: T[];
    getItemDOMId: (item: T) => string;
}) {
    ...
    const selectItem = useCallback<SelectItemFn>(({ itemIndex, scroll }) => {

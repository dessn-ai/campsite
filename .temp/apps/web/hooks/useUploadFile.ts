...
import { PresignedPostFields } from "../../../packages/types/index.ts";
...
export function useFileUploadMutation(onUpload: OnUpload, onError: OnError, property: 'key' | 'preview_file_path') {
    return useMutation({
        mutationFn: async (data: PresignedProps) => await handleFile({ file: data.file.raw, type: data.file.type, resource: data.resource, orgSlug: data.orgSlug }),

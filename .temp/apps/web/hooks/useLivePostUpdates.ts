...
import { usePostChannel } from "./usePostChannel.ts";
...
export function useLivePostUpdates(post: Post | undefined) {
    ...
    const invalidatePostQuery = useCallback(function ({ post_id }: {
        post_id: string | null;
    }) {
        if (!post_id)
            return;
    ...
    const invalidatePostTimelineQuery = useCallback(function () {
        if (!post?.id)
            return;
        queryClient.invalidateQueries({

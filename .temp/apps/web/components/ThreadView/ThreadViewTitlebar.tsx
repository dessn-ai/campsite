...
import { Button, LayeredHotkeys, UIText, VideoCameraIcon } from "../../../../packages/ui/src/index.tsx";
...
import { ChatFavoriteButton } from "../Thread/ChatFavoriteButton.tsx";
import { ChatThreadOverflowMenu } from "../Thread/ChatThreadOverflowMenu.tsx";
import { ThreadAvatar } from "../ThreadAvatar/index.tsx";
...
function ThreadBreadcrumbTitlebar({ thread }: {
    thread: MessageThread;
}) {
    const otherMembersTooltip = thread.other_members.map((m) => m.user.display_name).join(', ');
    const [otherMembersCount, otherGuestsCount] = useMemo(() => thread?.other_members.reduce(([membersCount, guestsCount], member) => member.role === 'guest' ? [membersCount, guestsCount + 1] : [membersCount + 1, guestsCount], [0, 0]) || [0, 0], [thread?.other_members]);
    ...
    if (!thread.group && thread.other_members.length === 1) {
        ...
        {thread.other_members[0]?.status && (<UIText tertiary className='flex items-center gap-1' size='text-xs'>
        ...
        {thread.other_members[0]?.user.timezone && !thread.other_members[0]?.status && (<div className='flex items-center gap-1'>
...
export function BreadcrumbCallButton({ thread }: {
    thread: MessageThread;
}) {
    ...
    if (!canJoin && !onCall)
        return null;
    return (<>
      <LayeredHotkeys keys='mod+shift+h' callback={joinCall} options={{ preventDefault: true, enableOnContentEditable: true }}/>

      <Button iconOnly={<VideoCameraIcon size={24}/>} accessibilityLabel='Start call' variant='plain' tooltip={onCall ? 'Already joined call' : 'Start call'} tooltipShortcut='⌘+shift+H' onClick={joinCall} disabled={onCall}/>
    </>);
}
export function ThreadViewTitlebar({ threadId, placement, isFocus }: {
    threadId: string;
    ...
    return (<BreadcrumbTitlebar hideSidebarToggle={!isFocus}>
      {thread && (<>
          <div className='flex flex-1 items-center gap-3'>
            <ConditionalWrap condition={!thread.group && !!firstParticipant} wrap={(children) => {
                if (!firstParticipant)
                    return children;
                return (<MemberHovercard username={firstParticipant.user.username}>

...
import { ChatBubbleIcon, LoadingSpinner } from "../../../../packages/ui/src/index.tsx";
import { cn } from "../../../../packages/ui/src/utils/index.ts";
import { EMPTY_HTML } from "../../atoms/markdown.ts";
import { attachmentsAtom, chatThreadPlacementAtom, clearRepliesAtom, editModeAtom, inReplyToAtom } from "../Chat/atoms.tsx";
import { EmptyState } from "../EmptyState/index.tsx";
import { FullPageLoading } from "../FullPageLoading.tsx";
import { useAppFocused } from "../../hooks/useAppFocused.ts";
import { useCreateMessage } from "../../hooks/useCreateMessage.ts";
import { useEditMessage } from "../../hooks/useEditMessage.ts";
...
import { useUploadChatAttachments } from "../../hooks/useUploadChatAttachments.tsx";
import { flattenInfiniteData } from "../../utils/flattenInfiniteData.ts";
import { HydrateAtoms } from "../../utils/jotai.ts";
...
import { Messages } from "../Thread/Messages.tsx";
import { StartOfThread } from "../Thread/StartOfThread.tsx";
import { UnseenMessagesButton } from "../Thread/UnseenMessagesButton.tsx";
import { DeactivatedMemberThreadComposer } from "./DeactivatedMemberThreadComposer.tsx";
export function ThreadView({ threadId, placement = undefined }: {
    threadId: string;
    placement?: 'hovercard' | undefined;
}) {
    return (<ScopeProvider atoms={[inReplyToAtom, attachmentsAtom, editModeAtom, chatThreadPlacementAtom]}>
      <HydrateAtoms atomValues={[[chatThreadPlacementAtom, placement]]}>
        <InnerThreadView threadId={threadId}/>
      </HydrateAtoms>
    </ScopeProvider>);
}
function InnerThreadView({ threadId }: {
    threadId: string;
}) {
    ...
    const { data: messageData, isError: isMessagesError, isLoading: isMessagesLoading, isFetching: isMessagesFetching, hasNextPage: hasMessagesNextPage, isFetchingNextPage: isMessagesFetchingNextPage, fetchNextPage: messagesFetchNextPage } = useGetMessages({ threadId });
    ...
    useEffect(() => {
        if (thread && replyToId && messagesQuery.isSuccess && !didSetInitialReplyTo.current) {
            ...
            const message = messagesQuery.data?.pages.flatMap((page) => page.data).find((message) => message.id === replyToId) ?? null;
    ...
    function onEditLastMessage() {
        ...
        const lastMessageByViewer = allMessages.find((message) => message.viewer_is_sender && !message.discarded_at && message.content && message.content !== EMPTY_HTML);
    ...
    return (<div className='flex flex-1 overflow-hidden'>
      <div className='isolate flex max-h-full flex-1 flex-col overflow-hidden' {...dropzone.getRootProps()}>
        {thread && (<>
            <div ref={scrollableRef} className={cn('relative flex flex-1 flex-col overflow-y-auto', {
              ...
              <div className='flex w-full flex-1 flex-col lg:mx-auto lg:max-w-3xl'>
                ...
                {hasMessagesNextPage && (<div className={cn('flex flex-1 items-center justify-center py-4', {
                ...
                <Messages key={thread?.id} messages={messages} hasNextPage={!!hasMessagesNextPage} thread={thread} onNewMessage={onNewMessage}/>
            ...
            <div className='w-full lg:mx-auto lg:max-w-3xl'>
              {!allMembersAreDeactivated && (<Composer key={thread.id} ref={composerRef} thread={thread} onMessage={onMessage} onEditLastMessage={onEditLastMessage} onScrollToBottom={() => {
          ...
          */
            onFocus={() => {
                    ...
                    if (!isMobile || !scrollableRef.current)
                        return;
                    const isNearBottom = scrollableRef.current.scrollHeight -
                    ...
                    if (isNearBottom)
                        return;
                    scrollImmediateScrollableNodeToBottom(scrollableRef.current);
                }} canSend autoFocus={!threadPlacement} onPaste={onPaste} onUpload={onUpload} dropzone={dropzone}>
                  <UnseenMessagesButton active={hasNewMessages} onClick={() => {
                    scrollImmediateScrollableNodeToBottom(scrollableRef.current);
                }}/>
                </Composer>)}

              {allMembersAreDeactivated && (<DeactivatedMemberThreadComposer plural={thread.deactivated_members.length > 1}/>)}
            </div>
          </>)}
      </div>
    </div>);

...
import { Button, cn, Link, UIText, UnreadSquareBadgeIcon, VideoCameraIcon } from "../../../../packages/ui/src/index.tsx";
    ...
    return (<HoverCard open={open} onOpenChange={handleOpenChange} disabled={isDisabled} targetHref={href}>
      ...
      <HoverCard.Content sideOffset={4} alignOffset={-44} portalContainer='quick-thread'>
        <HoverCard.Content.TitleBar>
          ...
          {(canJoin || onCall) && (<Button iconOnly={<VideoCameraIcon size={24}/>} accessibilityLabel='Start call' variant='plain' tooltip={onCall ? 'Already joined call' : 'Start call'} onClick={joinCall} disabled={onCall}/>)}
          <Button tooltip='Mark unread' variant='plain' iconOnly={<UnreadSquareBadgeIcon />} accessibilityLabel='Mark unread' onClick={() => markThreadUnread({ threadId: thread.id })}/>
        </HoverCard.Content.TitleBar>

...
import { Avatar, ChatBubbleIcon, cn } from "../../../../packages/ui/src/index.tsx";
...
export function ThreadAvatar({ thread, size = 'lg' }: Props) {
    ...
    if (thread.group && members.length === 1)
        return <Fallback size={size}/>;
...
function Fallback({ size }: {
    ...
    return (<div className={cn('flex items-center justify-center rounded-full bg-black/5 dark:bg-white/10', SIZE)}>
      <ChatBubbleIcon strokeWidth='2' className='text-quaternary'/>
    </div>);
}
export function MultiUserAvatar({ members, size = 'lg', showOnlineIndicator = true }: {
    members: OrganizationMember[];
    size?: Size;
    showOnlineIndicator?: boolean;
}) {
    // server doesn't return members in a consistent order, this is a client side hack for now
    const orderedMembers = useMemo(() => members.sort((a, b) => a.user.display_name.localeCompare(b.user.display_name)), [members]);
    if (members.length === 1) {
...
function SingleMemberAvatar({ member, size = 'lg', showOnlineIndicator }: {
...
function DoubleMemberAvatar({ members }: {
    members: OrganizationMember[];
}) {
    const [first, second] = members;
    return (<div className='bg-quaternary relative h-10 w-10 rounded-full'>
      <div className='absolute -left-[5px] -top-[5px] z-[1] scale-[0.48] rounded-full'>
        <Avatar deactivated={first.deactivated} name={first.user.display_name} urls={first.user.avatar_urls} size='lg'/>
      </div>
      <div className='absolute -bottom-[8px] -right-[8px] z-[2] scale-[0.34] rounded-full'>
        <Avatar deactivated={second.deactivated} name={second.user.display_name} urls={second.user.avatar_urls} size='lg'/>
      </div>
    </div>);
}
function ManyMemberAvatar({ members }: {
    members: OrganizationMember[];
}) {
    ...
    return (<div className='bg-quaternary relative h-10 w-10 rounded-full'>
      ...
      <div className='absolute -bottom-[11px] -right-px z-[2] scale-[0.28] rounded-full'>
        <Avatar deactivated={third.deactivated} name={third.user.display_name} urls={third.user.avatar_urls} size='lg'/>
      </div>
    </div>);
}

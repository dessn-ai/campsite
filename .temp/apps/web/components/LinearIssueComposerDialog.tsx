...
import { CreateLinearIssue, OrganizationPostLinearIssuesPostRequest, PostCommentsLinearIssuesData, PostPostsLinearIssuesData } from "../../../packages/types/generated.ts";
import { Button, FormError, TextField, UIText } from "../../../packages/ui/src/index.tsx";
...
import { lastUsedLinearTeamAtom, LinearTeamPicker } from "./LinearTeamPicker.tsx";
import { useFormSetValue } from "./PostComposer/hooks/useFormSetValue.tsx";
import { useCreateCommentLinearIssue } from "../hooks/useCreateCommentLinearIssue.ts";
import { useCreatePostLinearIssue } from "../hooks/useCreatePostLinearIssue.ts";
import { useGetLinearTeams } from "../hooks/useGetLinearTeams.ts";
...
function useIssueStatusChange({ open, onOpenChange }: {
    open: boolean;
    onOpenChange: (open: boolean) => void;
}) {
    const onStatusChange = useCallback((data: PostPostsLinearIssuesData | PostCommentsLinearIssuesData) => {
        ...
        if (open && data.status === 'success') {
            if (data.external_record) {
                toast(<ToastWithLink url={data.external_record?.remote_record_url} externalLink>
...
export function LinearPostIssueComposerDialog({ open, onOpenChange, defaultValues, postId }: DialogProps & {
    postId: string;
}) {
...
export function LinearCommentIssueComposerDialog({ open, onOpenChange, defaultValues, commentId }: DialogProps & {
    commentId: string;
}) {
    const { onStatusChange } = useIssueStatusChange({ open, onOpenChange });
...
function IssueComposer({ open, onOpenChange, status, createIssue }: {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    ...
    return (<Dialog.Root open={open} onOpenChange={checkDirtyStateAndClose} size='2xl' align={isMobile ? 'top' : 'center'}>
      <form onSubmit={onSubmit} className='flex flex-col gap-3 overflow-y-auto'>
        ...
        <Dialog.Content>
          <div className='space-y-3'>
            <TextField label='Title' name='title' value={title} onChange={(value) => setValue('title', value)} autoFocus placeholder='Issue title' onKeyDownCapture={handleCommandEnter}/>
            <TextField label='Description' name='description' value={description} onChange={(value) => setValue('description', value)} placeholder='Issue description' multiline minRows={isMobile ? 3 : 6} onKeyDownCapture={handleCommandEnter}/>
            <div>
              <UIText element='label' secondary weight='font-medium' className='mb-1.5 block' size='text-xs'>
                Team
              </UIText>
              <LinearTeamPicker activeId={teamId} onChange={(team) => setValue('team_id', team.provider_team_id)} onKeyDownCapture={handleCommandEnter}/>
            </div>
      ...
      {showDiscardDialog && (<Dialog.Root open={showDiscardDialog} onOpenChange={setShowDiscardDialog}>
          <Dialog.Header>

...
export const ProjectTag = forwardRef<ProjectTagElement, ProjectTagProps>(({ project, className, ...props }, ref) => {
    ...
    return (<ProjectHovercard projectId={project.id} side='top' align='center'>
      <Link {...props} ref={ref} className={cn('hover:text-primary text-quaternary @xl:text-tertiary @xl:border @xl:h-6 relative flex items-center gap-1 rounded-full', {
        ...
        }, className)} href={`/${scope}/projects/${project.id}`}>
        {project.accessory && (<UIText className='mr-px font-["emoji"] text-xs leading-none'>{project.accessory}</UIText>)}

...
import { CurrentUser, Project } from "../../../../packages/types/generated.ts";
...
import { ChatBubbleIcon, CheckIcon, FaceSmileIcon, FeedIcon, ListIcon, PhotoIcon, ResolvePostIcon, SlidersIcon, SwitchIcon } from "../../../../packages/ui/src/Icons/index.tsx";
...
import { useDeleteProjectViewerDisplayPreference } from "../../hooks/useDeleteProjectViewerDisplayPreference.ts";
import { useGetCurrentUser } from "../../hooks/useGetCurrentUser.ts";
import { filterAtom, sortAtom } from "../../hooks/useGetPostsIndex.ts";
import { usePostsDisplayPreference } from "../../hooks/usePostsDisplayPreference.ts";
import { useUpdatePreference } from "../../hooks/useUpdatePreference.ts";
import { useUpdateProjectDisplayPreference } from "../../hooks/useUpdateProjectDisplayPreference.ts";
import { useUpdateProjectViewerDisplayPreference } from "../../hooks/useUpdateProjectViewerDisplayPreference.ts";
interface Props {
    iconOnly?: boolean;
...
export function PostsIndexDisplayDropdown({ iconOnly = false, align = 'end', project }: Props) {
    ...
    const displayPreferencesAreDifferent = !!project?.viewer_display_preferences &&
    ...
    const displayItems = hasComfyCompactLayout && project
        ? buildMenuItems([
            {
                type: 'heading',
                label: 'Display'
            ...
            displayPreferencesAreDifferent && {
                ...
                onSelect: () => updateProjectDisplayPreference.mutate({
                    projectId: project.id,
                    orgSlug: `${scope}`,
                    ...displayPreferences
                })
            ...
            displayPreferencesAreDifferent && {
                type: 'item',
                label: 'Reset',
                onSelect: () => deleteProjectViewerDisplayPreference.mutate({
                    projectId: project.id,
                    orgSlug: `${scope}`
                })
            },
    ...
    return (<>
      ...
      <DropdownMenu open={dropdownIsOpen} onOpenChange={setDropdownIsOpen} items={items} align={align} trigger={<Button variant='plain' iconOnly={iconOnly ? <SwitchIcon /> : <SlidersIcon />} accessibilityLabel='Display dropdown' size='base' tooltip='Display and sort' tooltipShortcut={shortcut}/>}/>
    </>);
}

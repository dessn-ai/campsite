...
import { Avatar, ButtonPlusIcon, CheckCircleFilledIcon, CircleFilledCloseIcon, LinearBacklogIcon, SelectPopover, UIText } from "../../../../packages/ui/src/index.tsx";
...
export function PostComposerFeedbackManager({ form }: PostComposerFeedbackManagerProps) {
    ...
    return (<ul className='flex w-full flex-col pt-3'>
      {feedbackRequests?.map((fr) => {
            return <FeedbackRequestListItem feedbackRequest={fr} form={form} key={fr.id}/>;
        })}

      {memberOptions && (<SelectPopover open={open} setOpen={setOpen} typeAhead placeholder={'Add someone...'} options={memberOptions} onChange={(value) => {
                const member = syncedMembers?.find((m) => m.id === value);
                if (member) {
                    form.setValue('feedback_requests', [...(feedbackRequests ?? []), { id: uuid(), member, has_replied: false }], { shouldDirty: true, shouldValidate: true });
                    setOpen(false);
                }
            }}>
          <li className='py-2 first-of-type:pt-0 last-of-type:pb-0'>
            <button type='button' className='dark:text-secondary dark:hover:text-primary group relative flex items-center justify-between gap-3 text-blue-500 hover:text-blue-600' onClick={() => setOpen(true)}>
...
function FeedbackRequestListItem({ feedbackRequest, form }: {
    feedbackRequest: {
        id: string;
        member?: any;
        has_replied: boolean;
    ...
    return (<li key={feedbackRequest.id} className='group py-2 first-of-type:pt-0 last-of-type:pb-0'>
      <div className='group relative flex items-start justify-between gap-3'>
        ...
        <div className='text-quaternary dark:text-tertiary flex flex-row items-center gap-4'>
          {!feedbackRequest.has_replied && feedbackRequest.member.user.id !== currentUser?.id && (<span className='group-hover:hidden'>
              <LinearBacklogIcon size={24}/>
            </span>)}

          {feedbackRequest.has_replied && (<span className='text-blue-500'>
              <CheckCircleFilledIcon size={24}/>
            </span>)}

          {!feedbackRequest.has_replied && (<button className='text-tertiary hidden hover:text-red-500 group-hover:flex' onClick={() => {
                const feedbackRequests = form.getValues('feedback_requests') ?? [];
                form.setValue('feedback_requests', feedbackRequests?.filter((request) => request.id !== feedbackRequest.id) ?? [], { shouldDirty: true, shouldValidate: true });
            }}>
              <span>
...
function AddToProjectSuggestion({ projectId, member }: {
    projectId: string;
    ...
    if (!project || isLoading || userIsInProject)
        return null;
    if (!project.private && member.role !== 'guest')
        return null;
    ...
    return (<div className='pl-9'>
      <UIText tertiary size='text-xs'>
        Not in channel ·{' '}
        <button type='button' className='text-blue-500 hover:text-blue-600 dark:hover:text-blue-400' onClick={handleButtonClick}>
          Add to {project.name}
        </button>
      </UIText>
    </div>);

...
import { Button, Select, SelectTrigger, SelectValue } from "../../../../packages/ui/src/index.tsx";
...
export function PostComposerProjectPicker() {
    ...
    return (<div className='flex items-center gap-1.5'>
      <Select disabled={isSubmitting} align='start' showCheckmark value={projectId ?? ''} onQueryChange={setQuery} onChange={(value) => {
            setValue('project_id', value);
        }} options={filteredProjects.map(projectToOption)} onOpenChange={(open) => {
            if (open) {
                refetch();
            }
        }} shortcut={{
      ...
      {selectedProject && selectedProject.guests_count > 0 && (<Button size='sm' variant='plain' className='cursor-default bg-amber-100 text-amber-900 hover:bg-amber-100 dark:bg-amber-900/20 dark:text-amber-400 dark:hover:bg-amber-900/20'>

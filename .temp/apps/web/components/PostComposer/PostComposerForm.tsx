...
import { IS_PRODUCTION } from "../../../../packages/config/src/index.ts";
import { BlurAtTopOptions } from "../../../../packages/editor/src/extensions/index.ts";
import { Post } from "../../../../packages/types/index.ts";
import { Button } from "../../../../packages/ui/src/Button/index.tsx";
import { BugIcon } from "../../../../packages/ui/src/Icons/index.tsx";
...
import { getPostSchemaDefaultValues, PostSchema, postSchema } from "../Post/schema.ts";
import { useDefaultComposerValues } from "./hooks/useDefaultComposerValues.ts";
import { useFormSetValue } from "./hooks/useFormSetValue.tsx";
import { usePostComposerCreatePost } from "./hooks/usePostComposerCreatePost.ts";
import { usePostComposerLocalDraftActions } from "./hooks/usePostComposerLocalDraft.ts";
import { usePostComposerUpdatePost } from "./hooks/usePostComposerUpdatePost.tsx";
import { PostComposerActions } from "./PostComposerActions.tsx";
import { PostComposerFeedback } from "./PostComposerFeedback.tsx";
import { PostComposerHeaderActions } from "./PostComposerHeaderActions.tsx";
import { PostComposerInteractiveAttachments } from "./PostComposerInteractiveAttachments.tsx";
import { PostComposerNotePermissionDisclaimer } from "./PostComposerNotePermissionDisclaimer.tsx";
import { PostComposerPolls } from "./PostComposerPolls.tsx";
import { PostComposerProjectPicker } from "./PostComposerProjectPicker.tsx";
import { PostComposerUnfurledLink } from "./PostComposerUnfurledLink.tsx";
import { getSubmitButtonId, PostComposerAction, postComposerStateAtom, PostComposerType } from "./utils/index.ts";
import { TitleTextField } from "../TitleTextField.tsx";
import { useCreatePostPublication } from "../../hooks/useCreatePostPublication.ts";
import { useGetProjectMembers } from "../../hooks/useGetProjectMembers.ts";
...
export const PostComposerFormProvider = forwardRef<InlineComposerRef, React.PropsWithChildren>(function InlineComposer({ children }, ref) {
...
export function PostComposerForm({ onSubmit, onReportBug, onDeleteDraft }: ComposerFormProps) {
    ...
    const reset = useCallback(() => {
         ...
         */
        if (postComposerState?.type === PostComposerType.Draft)
            deleteLocalDraft();
    ...
    const handleSubmit = methods.handleSubmit(async (data, event) => {
        if (isSubmitting || !hasPostableContent || isPollEmpty || isUploadingAttachments)
            return;
        ...
        const promise = (() => {
            switch (submitterId) {
                ...
                case PostComposerAction.PublishPostDraft: {
                    if (postComposerState?.type !== PostComposerType.EditDraftPost)
                        return;
    ...
    return (<form ref={containerRef} className='flex flex-1 flex-col overflow-hidden rounded-b-lg' onSubmit={handleSubmit} onKeyDownCapture={(evt) => {
            if (isMetaEnter(evt)) {
                evt.preventDefault();
      ...
      <div className='relative flex flex-1 flex-col gap-1 overflow-y-auto pb-2 pt-0'>
        ...
        {isCampsiteInsiders && (<div className='bg-tertiary text-secondary mx-3 mb-2 flex items-center gap-2 rounded-md px-3 py-2'>
            <BugIcon />
            <UIText secondary>Found a bug? Use our feedback form instead!</UIText>
            <Button size='sm' className='ml-auto' variant='flat' onClick={() => onReportBug?.(methods.getValues('title'))}>
              Report a bug
            </Button>
          </div>)}

        <TitleTextField ref={titleRef} autoFocus onEnter={(e) => {
            if (!isMetaEnter(e)) {
                editorRef.current?.focus('start-newline');
            }
        }} onFocusNext={() => editorRef.current?.focus('restore')} placeholder='Subject (optional)' value={methods.getValues('title')} onChange={(val) => setValue('title', val)} className='px-3 pb-0 pt-1 text-[15px] font-semibold leading-snug'/>

        <MarkdownEditor ref={editorRef} disabled={isSubmitting} initialAttachments={attachments} placeholder='What would you like to share?' content={methods.getValues('description_html')} enableInlineLinks enableInlineAttachments enableSyntaxHighlighting onEmptyDidChange={setIsEditorEmpty} onChangeDebounced={(html) => setValue('description_html', html)} onInlineAttachmentsChange={(attachments) => setValue('attachment_ids', Array.from(attachments))} containerClasses='px-3 py-1' minHeight='128px' maxHeight='none' appendBubbleMenuTo={() => containerRef.current ?? document.body} onBlurAtTop={onBlurAtTop} defaultMentions={projectMembers}/>

        {hasPoll && (<div className='px-3'>
            <PostComposerPolls />
          </div>)}

        {hasRequestFeedback && (<div className='px-3'>
      ...
      <div className={cn('z-20 flex flex-row gap-2 border-t px-3 pt-2.5', {
            'border-primary': !bottomInView,
            'border-transparent': bottomInView,
            'pt-0': hasPoll && hasRequestFeedback
        })}>
        {!hasRequestFeedback && (<Button type='button' size='sm' disabled={isSubmitting} onClick={() => setValue('status', 'feedback_requested')} variant='flat'>
            Request feedback
          </Button>)}
      </div>
      ...
      <div className='bg-elevated sticky bottom-0 z-10 flex flex-row flex-nowrap justify-between gap-3 p-3 pr-2.5 pt-1.5 dark:bg-gray-900'>
        ...
        <PostComposerActions isUploadingAttachments={isUploadingAttachments} hasPostableContent={hasPostableContent} isPollEmpty={isPollEmpty}/>
      </div>
    </form>);
}

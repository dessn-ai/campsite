...
import { getSaveDraftButtonId, postComposerStateAtom } from "./utils/index.ts";
interface PostComposerDiscardDialogProps {
    showDiscardDialog: boolean;
    setShowDiscardDialog: (show: boolean) => void;
    onDiscard: () => void;
}
export function PostComposerDiscardDialog({ showDiscardDialog, setShowDiscardDialog, onDiscard }: PostComposerDiscardDialogProps) {
    const formState = useFormState();
    const postComposerState = useAtomValue(postComposerStateAtom);
    return (<Dialog.Root open={showDiscardDialog} onOpenChange={setShowDiscardDialog}>
      <Dialog.Header>
      ...
      <Dialog.Footer>
        ...
        <Dialog.TrailingActions>
          ...
          <Button autoFocus variant='primary' loading={formState.isSubmitting} onClick={() => {

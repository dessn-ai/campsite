...
import { IMGIX_DOMAIN } from "../../../../packages/config/src/index.ts";
import { MediaGalleryItemAttributes } from "../../../../packages/editor/src/extensions/index.ts";
import { ImageUrls } from "../../../../packages/types/index.ts";
import { cn } from "../../../../packages/ui/src/utils/index.ts";
import { CarouselPagination } from "./Pagination.tsx";
import { GifAttachment } from "../Thread/Bubble/AttachmentCard/GifAttachment.tsx";
import { ImageAttachment } from "../Thread/Bubble/AttachmentCard/ImageAttachment.tsx";
import { LottieAttachment } from "../Thread/Bubble/AttachmentCard/LottieAttachment.tsx";
import { OrigamiAttachment } from "../Thread/Bubble/AttachmentCard/OrigamiAttachment.tsx";
import { PrincipleAttachment } from "../Thread/Bubble/AttachmentCard/PrincipleAttachment.tsx";
import { StitchAttachment } from "../Thread/Bubble/AttachmentCard/StitchAttachment.tsx";
import { VideoAttachment } from "../Thread/Bubble/AttachmentCard/VideoAttachment.tsx";
...
export function MediaGallery(props: MediaGalleryProps) {
    ...
    useEffect(() => {
        ...
        if (nextRelativeUrl && !prefetchedAttachments.has(nextRelativeUrl)) {
            if (next.image_urls)
                prefetchPostImages(next.image_urls);
            if (nextIsVideo)
                prefetchPostVideo(nextRelativeUrl);
    ...
    return (<div className='not-prose relative overflow-hidden rounded-md'>
      ...
      <div className='bg-tertiary rounded-t-md'>
        <AnimatePresence mode='popLayout' initial={false} custom={direction}>
          <m.div className={cn('group relative flex h-[34rem] select-none items-center justify-center', {
            'pointer-events-none': isDragging
        })} key={activeIndex} initial='enter' animate='center' exit='exit' custom={direction} variants={variants} transition={{
            x: { type: 'spring', stiffness: 300, damping: 30 },
            opacity: { duration: 0.2 }
        }} drag={attachments.length > 1 ? 'x' : false} dragConstraints={{ left: 0, right: 0 }} dragElastic={1} whileDrag={{ cursor: 'grabbing' }} onDragStart={() => setIsDragging(true)} onDragEnd={(_, { offset, velocity }) => {
            setIsDragging(false);
      ...
      <CarouselPagination paginate={paginate} activeIndex={imageIndex} setActiveIndex={setActiveIndex} attachments={attachments} onRemoveItem={props.onRemoveItem} dropzone={props.dropzone} onReorder={props.onReorder}/>
    </div>);
}

...
import { Button, ChevronLeftIcon, ChevronRightIcon, LoadingSpinner, PlusIcon, ThickCloseIcon } from "../../../../packages/ui/src/index.tsx";
import { cn } from "../../../../packages/ui/src/utils/index.ts";
import { useExecuteOnChange } from "../../hooks/useExecuteOnChange.ts";
import { useServerOrOptimisticAttachment } from "../../hooks/useServerOrOptimisticAttachment.ts";
function PaginationButton(props: {
    onClick: () => void;
    direction: 'previous' | 'next';
}) {
    const { onClick, direction } = props;
    return (<Button variant='plain' onClick={onClick} type='button' iconOnly={<span className={cn({ '-mr-px': direction === 'next', '-ml-px': direction === 'previous' })}>
          {direction === 'previous' ? <ChevronLeftIcon /> : <ChevronRightIcon />}
        </span>} accessibilityLabel={direction === 'next' ? 'Next attachment' : 'Previous attachment'}/>);
}
...
function PaginationItem(props: PaginationItemProps) {
    ...
    return (<>
      <button key={props.attachment.id} type='button' className={cn('bg-tertiary relative h-9 w-9 flex-none cursor-[inherit] rounded ring-1 ring-black/5 after:absolute after:-bottom-[6px] after:left-1/2 after:h-[3px] after:w-4 after:-translate-x-1/2 after:rounded-full after:opacity-0 after:content-[""] dark:ring-white/5', {
        ...
        {attachment && (<>
            {attachment.video && vidPreviewSrc && (<Image key={attachment.id} width={56} height={56} src={vidPreviewSrc} alt='Uploaded video' className='aspect-square rounded object-cover object-top'/>)}

            {attachment.gif && (<video key={attachment.id} controls={false} preload='metadata' className='object-top-center pointer-events-none h-full w-full rounded object-cover' draggable={false}>
                <source src={`${attachment.url}?fm=mp4#t=0.1`} type='video/mp4'/>
                <source src={`${attachment.url}#t=0.1`}/>
              </video>)}

            {attachment.image && imgSrc && (<Image width={56} height={56} src={imgSrc} alt='' className='aspect-square rounded object-cover object-top'/>)}

            {attachment.origami && (<div className='bg-tertiary flex h-full w-full items-center justify-center rounded' key={attachment.id}>
                <Image src={'/img/origami.png'} width={20} height={20} alt='Origami attachment'/>
              </div>)}

            {attachment.principle && (<div className='bg-tertiary flex h-full w-full items-center justify-center rounded' key={attachment.id}>
                <Image src={'/img/principle.png'} width={20} height={20} alt='Principle attachment'/>
              </div>)}

            {attachment.stitch && (<div className='bg-tertiary flex h-full w-full items-center justify-center rounded' key={attachment.id}>
                <Image src={'/img/stitch.png'} width={20} height={20} alt='Stitch attachment'/>
              </div>)}

            {attachment.lottie && (<div className='bg-tertiary flex h-full w-full items-center justify-center rounded' key={attachment.id}>
                <Image src={'/img/lottie.png'} width={20} height={20} alt='Lottie attachment'/>
              </div>)}
          </>)}
      </button>
      {isUploading && (<div className='absolute inset-0 flex h-9 w-9 items-center justify-center'>
          <LoadingSpinner />
        </div>)}
    </>);
}
...
function useLocalSortOrder(attachments: MediaGalleryItemAttributes[]) {
    ...
    function updateSortedAttachments(ids: string[]) {
        setSortedAttachments(ids.map((id) => attachments.find((a) => a.optimistic_id === id)).filter(Boolean) as MediaGalleryItemAttributes[]);
    }
...
export function CarouselPagination(props: CarouselPaginationProps) {
    ...
    return (<div className='mx-auto flex items-start justify-center pb-1.5 pt-1'>
      ...
      <div className='scrollbar-hidden overflow-scroll px-2'>
        <Reorder.Group axis='x' as='div' values={displayAttachments.map((a) => a.optimistic_id)} onReorder={setSortedAttachments} className='flex items-start gap-2 py-2' ref={containerRef}>
          {displayAttachments.map((attachment, index) => {
            return (<Reorder.Item as='div' key={attachment.optimistic_id} value={attachment.optimistic_id} layout='position' drag={onReorder ? 'x' : false} dragConstraints={containerRef} dragElastic={0} onDragStart={() => {
                ...
                <PaginationItem index={index} attachment={attachment} activeIndex={activeIndex} setActiveIndex={setActiveIndex}/>
                {onRemoveItem && (<button type='button' className='absolute right-0 top-0 -m-1.5 flex h-4 w-4 items-center justify-center rounded-full bg-red-500 text-white hover:bg-red-400' aria-label='Remove item' onClick={() => onRemoveItem(attachment.optimistic_id)}>
      ...
      {dropzone && (<div className='py-2'>
          <div className='flex h-9 flex-none place-items-center'>
            <Button variant='plain' type='button' iconOnly={<PlusIcon className='h-4 w-4'/>} accessibilityLabel='Add attachment' className={dropzone.isDragActive ? 'bg-secondary-action ring-2 ring-blue-500' : ''} onClick={() => dropzone.open()}/>
          </div>
        </div>)}
      <div className='mt-2 flex h-9 flex-none place-items-center'>

...
import { ALL_CONTAINER_STYLES, ANIMATION_CONSTANTS, Button, DownloadIcon, ExternalLinkIcon, Link, Popover, PopoverContent, PopoverPortal, PopoverTrigger, QRCodeIcon, UIText } from "../../../../packages/ui/src/index.tsx";
import { getFileMetadata } from "../../utils/getFileMetadata.ts";
import { FileTypeIcon } from "../FileTypeIcon.tsx";
export function FileAttachment({ attachment, showActions = true, extraActions }: {
    ...
    return (<div className='relative flex flex-1 items-center gap-2 px-3 py-2'>
      ...
      {showActions && (<div className='text-tertiary flex flex-row items-center justify-end gap-0.5'>
          {qrCode && <QRCodePopover attachment={attachment}/>}
          {downloadUrl && (<Button href={downloadUrl} download={attachment.name || 'file'} iconOnly={<DownloadIcon />} variant='plain' accessibilityLabel='Download'/>)}
          {openUrl && (<Button variant='plain' externalLink accessibilityLabel='Open file' iconOnly={<ExternalLinkIcon />} href={openUrl}/>)}
          {extraActions}
        </div>)}
...
function QRCodePopover({ attachment }: {
    attachment: Pick<Attachment, 'download_url'>;
}) {
    ...
    return (<Popover open={open} onOpenChange={setOpen}>
      ...
      <AnimatePresence>
        ...
        className='z-10' asChild forceMount side='bottom' align='center' sideOffset={4}>

...
import { Call, CallPage, Project } from "../../../../packages/types/index.ts";
import { Command, HighlightedCommandItem, Link, LoadingSpinner, Tooltip, UIText, useCommand, VideoCameraFilledIcon, VideoCameraIcon } from "../../../../packages/ui/src/index.tsx";
import { cn, ConditionalWrap } from "../../../../packages/ui/src/utils/index.ts";
import { CallOverflowMenu } from "./CallOverflowMenu.tsx";
import { callsFilterAtom, CallsIndexFilter } from "./CallsIndexFilter.tsx";
import { MobileCallsTitlebar } from "./MobileCallsTitlebar.tsx";
import { NewCallButton } from "./NewCallButton.tsx";
import { useGetCallPeerMembers } from "./useGetCallPeerUsers.tsx";
import { EmptySearchResults } from "../Feed/EmptySearchResults.tsx";
import { FloatingNewCallButton } from "../FloatingButtons/NewCall.tsx";
import { HTMLRenderer } from "../HTMLRenderer/index.tsx";
import { IndexPageContainer, IndexPageContent, IndexPageEmptyState, IndexPageLoading, IndexSearchInput } from "../IndexPages/components.tsx";
...
import { refetchingCallsAtom } from "../NavigationBar/useNavigationTabAction.ts";
import { useHandleCommandListSubjectSelect } from "../Projects/hooks/useHandleHighlightedItemSelect.tsx";
import { ProjectCallButton } from "../Projects/ProjectCallButton.tsx";
...
import { MultiUserAvatar } from "../ThreadAvatar/index.tsx";
import { CallBreadcrumbIcon } from "../Titlebar/BreadcrumbPageIcons.tsx";
import { BreadcrumbLabel, BreadcrumbTitlebar, BreadcrumbTitlebarContainer } from "../Titlebar/BreadcrumbTitlebar.tsx";
import { useScope } from "../../contexts/scope.tsx";
import { useCallsSubscriptions } from "../../hooks/useCallsSubscriptions.ts";
import { useGetCalls } from "../../hooks/useGetCalls.ts";
import { useIsCommunity } from "../../hooks/useIsCommunity.ts";
import { encodeCommandListSubject } from "../../utils/commandListSubject.ts";
...
export function CallsContent({ getCalls, isSearching, project }: CallsContentProps) {
    ...
    return (<>
      ...
      <InfiniteLoader hasNextPage={!!getCalls.hasNextPage} isError={!!getCalls.isError} isFetching={!!getCalls.isFetching} isFetchingNextPage={!!getCalls.isFetchingNextPage} fetchNextPage={getCalls.fetchNextPage}/>
    </>);
}
function CallsList({ calls, isSearching, isLoading, project }: {
    calls: Call[];
    isSearching: boolean;
    isLoading: boolean;
    project?: Project;
    ...
    return (<ConditionalWrap condition={needsCommandWrap} wrap={(children) => (<SubjectCommand>
          <Command.List className={cn('flex flex-1 flex-col', { 'gap-4 md:gap-6 lg:gap-8': !isSearching, 'gap-px': isSearching })}>
            {children}
          </Command.List>
        </SubjectCommand>)}>
      {isSearching ? (<SearchCallsIndexContent calls={calls} hideProject={!!project}/>) : (<GroupedCallsIndexContent calls={calls} hideProject={!!project}/>)}
    </ConditionalWrap>);
}
function GroupedCallsIndexContent(props: {
    calls: Call[];
    hideProject?: boolean;
}) {
    ...
    return Object.entries(callGroups).map(([date, calls]) => {
        ...
        <ul className='flex flex-col gap-1 py-2'>
          {calls.map((call) => (<CallRow key={call.id} call={call} hideProject={props.hideProject}/>))}
        </ul>
      </div>);
    });
}
function SearchCallsIndexContent({ calls, hideProject }: {
    calls: Call[];
    hideProject?: boolean;
}) {
    return calls.map((call) => <CallRow key={call.id} call={call} display='search' hideProject={hideProject}/>);
}
export function CallsIndexEmptyState({ project }: {
    project?: Project;
}) {
    ...
    return (<IndexPageEmptyState>
      ...
      {project?.message_thread_id && isProjectCalls ? (<ProjectCallButton project={project} variant='primary'/>) : (<NewCallButton alignMenu='center'/>)}
    </IndexPageEmptyState>);
}
...
export const CallRow = memo(({ call, display, hideProject = false }: CallRowProps) => {
    ...
    return (<div className='relative flex items-center gap-3 px-3 py-2.5 pr-2'>
      <CallOverflowMenu type='context' call={call}>
        <HighlightedCommandItem className='absolute inset-0 z-0' value={encodeCommandListSubject(call, { href })} onSelect={handleSelect}/>
      </CallOverflowMenu>

      {!!callMembers.length && <MultiUserAvatar members={callMembers} size='lg' showOnlineIndicator={false}/>}
      {!callMembers.length && (<div className='bg-quaternary text-quaternary flex h-10 w-10 items-center justify-center rounded-full'>
          <VideoCameraFilledIcon />
        </div>)}

      <div className='flex flex-1 flex-col'>
        <div className='flex flex-1 flex-row items-center gap-3'>
          <UIText weight='font-medium' size='text-[15px]' className='line-clamp-1' tertiary={call.processing_generated_title}>
          ...
          {display === 'search' && (<UIText size='text-[15px]' quaternary>
              {format(call.created_at, 'MMM d, yyyy')}
            </UIText>)}
          {call.processing_generated_summary && (<Tooltip label='Creating summary...'>
        ...
        {summary && (<HTMLRenderer className='text-tertiary break-anywhere line-clamp-1 max-w-xl select-text text-sm' text={summary}/>)}
      </div>
...
export function CompactCallRow({ call, display }: CompactCallRowProps) {
    ...
    if (display === 'pinned') {
        return (<div className='relative flex items-center gap-3 px-3 py-2.5 pr-2'>
        <CallOverflowMenu type='context' call={call}>
          <HighlightedCommandItem className='absolute inset-0 z-0' value={encodeCommandListSubject(call, { href, pinned: true })} onSelect={handleSelect}/>
        </CallOverflowMenu>

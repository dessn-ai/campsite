...
import { CallPeer, OrganizationMember } from "../../../../packages/types/index.ts";
...
export function useGetCallPeerMembers({ peers, activeOnly = false, excludeCurrentUser = false }: Props) {
    ...
    return useMemo(() => peers.reduce<OrganizationMember[]>((acc, peer) => {
        if (activeOnly && !peer.active)
            return acc;
        if (excludeCurrentUser && currentUser?.id === peer.member.user.id)
            return acc;

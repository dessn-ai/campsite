...
import { Call, OrganizationCallsGetRequest } from "../../../../packages/types/index.ts";
import { Button, Command, LoadingSpinner, UIText, VideoCameraIcon } from "../../../../packages/ui/src/index.tsx";
import { HoverCard } from "../../../../packages/ui/src/HoverCard/index.ts";
import { cn } from "../../../../packages/ui/src/utils/index.ts";
import { CallRow } from "./index.tsx";
...
export function CallsHoverCard({ children, side = 'right', align = 'start', sideOffset = 0, alignOffset = 0, disabled: _disabled = false }: {
    ...
    const calls = useMemo(() => groupByDate(flattenInfiniteData(getCalls.data) || [], (call) => call.created_at), [getCalls.data]);
    ...
    return (<HoverCard open={open} onOpenChange={setOpen} disabled={disabled} targetHref={href}>
      ...
      <HoverCard.Content side={side} align={align} sideOffset={sideOffset} alignOffset={alignOffset}>
        <HoverCard.Content.TitleBar>
          ...
          <Button className='mr-auto' onClick={() => setFilter('joined')} variant={filter === 'joined' ? 'flat' : 'plain'}>
            Joined
          </Button>
        ...
        {!hasCalls && !getCalls.isLoading && (<div className='flex flex-1 items-center justify-center px-6 py-12'>
            <EmptyState title='No recorded calls yet' icon={<VideoCameraIcon className='text-quaternary' size={32}/>}/>
          </div>)}
...
function CallsList({ calls }: {
    ...
    return (<Command className='scrollbar-hide flex max-h-[420px] flex-col gap-px overflow-y-auto overscroll-contain outline-none' disableAutoSelect focusSelection>
      <Command.List>

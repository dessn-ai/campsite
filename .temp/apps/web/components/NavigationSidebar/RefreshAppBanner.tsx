...
import { LAST_CLIENT_JS_BUILD_ID_LS_KEY } from "../../../../packages/config/src/index.ts";
import { RefreshIcon, UIText } from "../../../../packages/ui/src/index.tsx";
import { useHasLatestBuild } from "../../hooks/useHasLatestBuild.ts";
...
export function RefreshAppBanner() {
    ...
    useEffect(() => {
        ...
        if (staleRefresh && !hasLatestBuild)
            setIsVisible(true);
    }, [hasLatestBuild, lsLastChecked]);
    return (<AnimatePresence>
      {isVisible && (<motion.div initial={{ opacity: 0, height: 0, top: 2, scale: 0.95 }} animate={{ opacity: 1, height: 'auto', top: 0, scale: 1 }} exit={{ opacity: 0, height: 0, top: 2, scale: 0.95 }} transition={{ duration: 0.3 }}>
          <button onClick={() => window.location.reload()} className='flex w-full flex-1 items-center justify-center py-2'>

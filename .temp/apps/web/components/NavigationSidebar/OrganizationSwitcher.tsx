...
import { AppsIcon, Avatar, Badge, ChevronSelectIcon, FaceSmileIcon, GearIcon, GlobeIcon, PlusIcon, ProjectIcon, ReorderHandlesIcon, UIText, useBreakpoint, UserCircleIcon } from "../../../../packages/ui/src/index.tsx";
...
import { useViewerCanCreateCustomReaction } from "../../hooks/useViewerCanCreateCustomReaction.ts";
import { useViewerCanManageIntegrations } from "../../hooks/useViewerCanManageIntegrations.ts";
import { useViewerIsAdmin } from "../../hooks/useViewerIsAdmin.ts";
export function OrganizationSwitcher({ trigger }: {
    ...
    const { data: memberships, isLoading: organizationsLoading, refetch: refetchOrganizations } = useGetOrganizationMemberships();
    ...
    const otherOrgHasUnread = !!unreadCounts.data &&
    ...
    if (organizationsLoading || (scope && !currentOrganization)) {
        ...
        return (<div className='flex h-8 flex-1 items-center gap-2 p-1.5'>
        <div className='bg-quaternary h-5 w-5 flex-none rounded-full'/>
        <div className='bg-quaternary h-1.5 w-1/2 rounded-full'/>
      </div>);
    }
    const defaultTrigger = (<button className='hover:bg-quaternary relative flex cursor-pointer items-center justify-between gap-2 overflow-hidden rounded-md p-1.5 text-sm focus-visible:border-blue-500 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-100'>
    ...
    const items = buildMenuItems([
        organization?.viewer_can_see_people_index && {
            ...
            rightSlot: hasMembershipRequests && (<Badge color='blue' className='h-4.5 ml-1 flex bg-blue-500 font-mono text-white dark:bg-blue-500 dark:text-white'>
    ...
    const mobileItems = buildMenuItems(memberships
        ...
        return {
            ...
            leftSlot: (<Avatar size='xs' key={organization.id} name={organization.name} urls={organization.avatar_urls} rounded='rounded'/>),
            rightSlot: (<>
              {organization.slug === COMMUNITY_SLUG && <GlobeIcon />}
              {unreadCount > 0 && (<span className='ml-1 flex h-5 items-center justify-center self-center rounded-full bg-blue-500 px-2.5 font-mono text-[10px] font-semibold leading-none text-white'>
                  {unreadCount}
                </span>)}
            </>)
        };
    ...
    return (<DropdownMenu open={open} onOpenChange={(val) => {
            if (val)
                refetchOrganizations();
            setOpen(val);
        }} align='start' sideOffset={4} desktop={{ width: 'w-[280px]' }} trigger={trigger ? (<button className='relative flex'>
            {otherOrgHasUnread && (<div className={cn('absolute -right-1 -top-1 z-10 h-2.5 w-2.5 flex-none rounded-full bg-blue-500 ring-4 ring-gray-50 dark:ring-gray-900', { 'right-5.5': !isMobile && memberships && memberships.length > 1 })}/>)}
            {trigger}
          </button>) : (defaultTrigger)} items={allItems}/>);
}
function OrganizationAvatarAndName({ organization, className }: {
    organization?: PublicOrganization;
    className?: string;
}) {
    return (<div className={cn('flex min-w-[0] items-center gap-2 text-left', className)}>
      <div className='shrink-0'>
        {organization ? (<Avatar rounded='rounded' key={organization.id} size='xs' name={organization.name} urls={organization.avatar_urls}/>) : (<ReorderHandlesIcon />)}
      </div>

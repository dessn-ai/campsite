...
import { ArrowUpRightIcon, Button, Link, ShipIcon, ShipUnreadIcon } from "../../../../packages/ui/src/index.tsx";
...
import { useGetChangelog } from "../../hooks/useGetLatestRelease.ts";
import { useStoredState } from "../../hooks/useStoredState.ts";
import { Changelog } from "../../utils/types.ts";
interface LinkProps {
...
export function ChangelogDropdown({ side = 'top', align = 'start' }: {
    side?: 'top' | 'bottom';
    ...
    useEffect(() => {
        ...
        const userOnboardedAfterLatestChangelog = currentUser?.onboarded_at && currentUser.onboarded_at > changelog[0].published_at;
        if (userOnboardedAfterLatestChangelog)
            return;
        const v2LsHasLatestRelease = v2Ls && v2Ls === changelog[0].slug;
        if (v2LsHasLatestRelease)
            return;
    ...
    const trigger = (<Button iconOnly={hasUnread ? <ShipUnreadIcon /> : <ShipIcon />} variant='plain' href={`${SITE_URL}/changelog`} externalLink accessibilityLabel='Changelog' tooltip='Changelog' className='text-tertiary hover:text-primary'/>);
    if (!changelog) {
        return (<Button iconOnly={<ShipIcon />} variant='plain' href={`${SITE_URL}/changelog`} externalLink accessibilityLabel='Changelog' tooltip='Changelog' className='text-tertiary hover:text-primary'/>);
    }
    const items = buildMenuItems([
        ...buildMenuItems(changelog?.map((c) => ({
            type: 'item',
            label: c.title,
    ...
    return (<DropdownMenu items={items} align={align} side={side} onOpenChange={() => {

...
import { TextareaAutosize } from "../../../packages/ui/src/TextField/index.tsx";
...
export const TitleTextField = forwardRef<HTMLTextAreaElement, Props>(function TitleTextField(props, outerRef) {
    ...
    return (<TextareaAutosize ref={ref} className={cn('scrollbar-hide w-full shrink-0 resize-none rounded-none border-0 bg-transparent p-0 outline-none focus:border-0 focus:outline-none focus:ring-0 dark:bg-transparent', className)} placeholder={placeholder} onChange={(e) => onChange(e.target.value)} onBlur={onBlur} onKeyDownCapture={onTitleKeydownCapture} value={value} autoFocus={autoFocus} readOnly={readOnly} autoComplete='off'/>);
});

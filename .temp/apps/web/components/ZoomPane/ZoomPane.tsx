...
import { setupZoomAtom, setZoomAtom, zoomAtom } from "./atom.ts";
import { useResizeHandler } from "./useResizeHandler.ts";
import { constrainedFitTransform, eventTargetClickDisabled, eventTargetWheelDisabled, getCurrentTransform } from "./utils.ts";
...
export function ZoomPane({ width, height, onClick, minZoom = 0.1, maxZoom = 8, children }: Props) {
    ...
    useEffect(() => {
        if (!d3Zoom || !d3Container)
            return;
        d3Container.on('wheel.pane', (event: any) => {
            if (eventTargetWheelDisabled(event.target))
                return;
            ...
            if (event.shiftKey) {
                ...
                d3Zoom.translateBy(d3Container, -(deltaX / currentZoom) * panOnScrollSpeed, -(deltaY / currentZoom) * panOnScrollSpeed);
            }
        }, { passive: false });
    }, [d3Container, d3Zoom]);
    // handle click events
    useEffect(() => {
        ...
        d3Container.on('click.pane', (event: any) => {
            if (eventTargetClickDisabled(event.target))
                return;
            const point = pointer(event);
    ...
    return (<div ref={containerRef} className='zoom-pane relative h-full w-full overflow-clip'>
      {children}
    </div>);
}

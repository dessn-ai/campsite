...
import { clamp, constrainedFitTransform } from "./utils.ts";
...
export const setZoomAtom = atom(null, (_get, set, data: Partial<Pick<ZoomState, 'viewport' | 'size' | 'transform' | 'panning'>>) => set(_zoomAtom, (prev) => ({ ...prev, ...data })));
export const panZoomAtom = atom(null, (_get, set, action: {
    x: number;
    y: number;
...
export const changeZoomAtom = atom(null, (_get, set, action: 'zoom-50%' | 'zoom-100%' | 'zoom-fit' | 'zoom-in' | 'zoom-out') => {
    set(_zoomAtom, (prev) => {

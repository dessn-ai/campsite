...
import { createTiles, filterVisibleTiles, getCurrentTransform, Tile } from "./utils.ts";
import { zoomAtom } from "./atom.ts";
interface Props {
    width: number;
    height: number;
    src: string;
...
export function ZoomCanvasTileRenderer({ width, height, src, backgroundSrc, style }: Props) {
    ...
    useEffect(() => {
        if (!d3Zoom || !d3Container || !ref.current)
            return;
        // setup D3 containers
            ...
            if (backgroundLoader) {
                if (!canDrawTile && backgroundLoader.loaded && backgroundLoader.image) {
                    if (callback === 'background') {
                        ...
                        ctx?.drawImage(backgroundLoader.image, tile.x * widthRatio, tile.y * heightRatio, tile.width * widthRatio, tile.height * heightRatio, tile.x, tile.y, tile.width, tile.height);
                    }
                }
            }
            else {

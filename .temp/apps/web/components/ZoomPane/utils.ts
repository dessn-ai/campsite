...
const preferredTileSize = (size: number) => size < MIN_TILE_DIMENSION ? size : Math.ceil(size / Math.floor(size / PREFERRED_TILE_DIMENSION));
...
function tileUrl({ src, x, y, width, height }: {
    src: string;
    x: number;
    y: number;
    width: number;
...
export function constrainedFitTransform({ d3Zoom, width, height, containerWidth, containerHeight, maxZoom }: {
...
export function filterVisibleTiles({ tiles, transform, width, height, containerWidth, containerHeight }: {
    tiles: Tile[];
...
export const getCurrentTransform = (selection: D3Selection<Element, unknown, null, undefined>) => selection.property('__zoom');

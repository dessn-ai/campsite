...
import { FileAiIcon, FileApkIcon, FileAviIcon, FileCodeIcon, FileCsvIcon, FileDmgIcon, FileDwgIcon, FileEpsIcon, FileExeIcon, FileIcon, FileJsonIcon, FileLogIcon, FileM4aIcon, FileMarkdownIcon, FileMkvIcon, FileMp3Icon, FileOggIcon, FilePdfIcon, FilePsdIcon, FileQtzIcon, FileRarIcon, FileSqlIcon, FileTarIcon, FileTxtIcon, FileWavIcon, FileXmlIcon, FileZipIcon } from "../../../packages/ui/src/index.tsx";
...
export function FileTypeIcon({ name, fileType, origami, principle, stitch, figma }: {
    name: string | null;
    fileType?: string;
    origami?: boolean;
    principle?: boolean;
    ...
    switch (fileType) {
        ...
        case 'application/octet-stream':
            if (name?.endsWith('.sketch'))
                return <Image src='/img/services/sketch.png' width={24} height={24} alt=''/>;
            if (name?.endsWith('.dwg'))
                return <FileDwgIcon size={24} className='text-tertiary'/>;
            if (name?.endsWith('.qtz'))
                return <FileQtzIcon size={24} className='text-tertiary'/>;
            if (name?.endsWith('.dwg'))
                return <FileDwgIcon size={24} className='text-tertiary'/>;
            break;
        // figma

...
import { Button, Calendar, CloseIcon, LayeredHotkeys, Popover, POPOVER_MOTION, PopoverContent, PopoverPortal, useBreakpoint } from "../../../../packages/ui/src/index.tsx";
import { DropdownMenu } from "../../../../packages/ui/src/DropdownMenu/index.tsx";
import { buildMenuItems, MenuItem } from "../../../../packages/ui/src/Menu/index.ts";
import { FollowUpCalendarDialog } from "./FollowUpCalendarDialog.tsx";
import { defaultCustomDate, getFollowUpDates } from "./utils.ts";
...
function FollowUpHotkeyRegistration({ dropdownOptions, setDropdownOpen }: {
    dropdownOptions: MenuItem[];
    setDropdownOpen: (open: boolean) => void;
}) {
    const dropdownIndexHotkeys = Array.from({ length: dropdownOptions.length }, (_, i) => (i + 1).toString());
    return (<LayeredHotkeys keys={dropdownIndexHotkeys} options={{ enabled: dropdownIndexHotkeys.length > 1 }} callback={(kbEvent, hotkeyEvent) => {
...
export const FollowUpDropdown = forwardRef<FollowUpDropdownRef, FollowUpDropdownProps>(function FollowUpDropdown({ children, followUps, onCreate, onDelete, modal = true, side = 'top', align = 'center' }, ref) {
    ...
    const getDeleteOptions = () => buildMenuItems(viewerFollowUp && [
        {
            type: 'item',
    ...
    const getCreateOptions = () => {
        ...
        const dropdownOptions = buildMenuItems(dates.map(({ date, label, formatStr }, index) => ({
            type: 'item',
            label,
    ...
    return (<>
      ...
      <div className='relative leading-none'>
        <DropdownMenu header={<FollowUpHotkeyRegistration setDropdownOpen={setDropdownOpen} dropdownOptions={dropdownOptions}/>} open={dropdownOpen} onOpenChange={(open) => {
            if (open) {
                setDropdownOptions(viewerFollowUp ? getDeleteOptions() : getCreateOptions());
                setCustomDate(defaultCustomDate);
            }
            setDropdownOpen(open);
        }} align={align} side={side} items={dropdownOptions} trigger={children} desktop={{ modal }}/>
        {isPopover && (<Popover modal={modal} open={calendarOpen} onOpenChange={(open) => {
            ...
            <AnimatePresence>
              {calendarOpen && (<PopoverPortal forceMount>
                  <PopoverContent>
                    <m.div className='bg-elevated shadow-popover flex flex-col gap-2 rounded-lg border p-3' {...POPOVER_MOTION} initial={false}>
                      <Calendar initialFocus fromDate={addDays(new Date(), 1)} toDate={addYears(new Date(), 1)} mode='single' selected={customDate} onSelect={(date) => setCustomDate(date)}/>
                      <Button fullWidth className='py-1' variant='primary' onClick={() => {
                    if (customDate) {
        ...
        {!isPopover && (<FollowUpCalendarDialog open={calendarOpen} onOpenChange={setCalendarOpen} onCreate={onCreate}/>)}
      </div>
    </>);
});

...
import type { FollowUp, Notification, NotificationTarget } from "../../../../packages/types/index.ts";
import { AlarmIcon, InboxIcon, LayeredHotkeys, Link, UIText, useBreakpoint, useCallbackRef, useIsDesktopApp } from "../../../../packages/ui/src/index.tsx";
import { cn, ConditionalWrap } from "../../../../packages/ui/src/utils/index.ts";
import { useTrackActivityView } from "../Activity/Activity.tsx";
import { CallView } from "../CallView/index.tsx";
import { HomeFollowUpDialog } from "../Home/HomeFollowUpDialog.tsx";
import { FollowUpListItem } from "./FollowUpListItem.tsx";
import { useInboxFilterHrefs } from "./hooks/useInboxFilterHrefs.tsx";
import { useInboxDetailItemId, useInboxSelectedItemId } from "./hooks/useInboxSelectedItemId.ts";
import { InboxFilterButtons } from "./InboxHoverCard.tsx";
import { InboxNotificationItem } from "./InboxNotificationItem.tsx";
import { InboxProjectRenderer } from "./InboxProjectRenderer.tsx";
import { InboxViewOptions } from "./InboxViewOptions.tsx";
import { NotificationListItem } from "./NotificationListItem.tsx";
import { NotificationOverflowMenu } from "./NotificationOverflowMenu.tsx";
import { expandedNotificationGroupsAtom, getGroupedNotificationsByInboxKey, isNotification, isNotificationGroupExpandedAtom, setExpandedNotificationGroupAtom } from "./utils/index.ts";
import { IndexPageEmptyState, IndexPageLoading } from "../IndexPages/components.tsx";
...
import { refetchingInboxAtom } from "../NavigationBar/useNavigationTabAction.ts";
import { NoteView } from "../NoteView/index.tsx";
import { PostView } from "../Post/PostView.tsx";
import { ScrollableContainer } from "../ScrollableContainer.tsx";
import { BreadcrumbTitlebar, BreadcrumbTitlebarContainer } from "../Titlebar/BreadcrumbTitlebar.tsx";
...
import { useDeleteFollowUp } from "../../hooks/useDeleteFollowUp.ts";
import { useDeleteNotification } from "../../hooks/useDeleteNotification.ts";
import { useExecuteOnChange } from "../../hooks/useExecuteOnChange.ts";
import { useGetArchivedNotifications } from "../../hooks/useGetArchivedNotifications.ts";
import { useGetFollowUps } from "../../hooks/useGetFollowUps.ts";
import { useGetNotifications } from "../../hooks/useGetNotifications.ts";
import { useMarkNotificationRead } from "../../hooks/useMarkNotificationRead.ts";
import { useMarkNotificationUnread } from "../../hooks/useMarkNotificationUnread.ts";
import { useScopedStorage } from "../../hooks/useScopedStorage.ts";
import { useUnarchiveNotification } from "../../hooks/useUnarchiveNotification.ts";
import { flattenInfiniteData } from "../../utils/flattenInfiniteData.ts";
import { getGroupDateHeading } from "../../utils/getGroupDateHeading.ts";
import { getInboxItemRoutePath } from "../../utils/getInboxItemRoutePath.ts";
...
export function InboxSplitView({ view }: InboxSplitViewProps) {
    ...
    const groupedNotificationsByInboxKey = useMemo(() => getGroupedNotificationsByInboxKey(notifications), [notifications]);
    const visibleNotifications = useMemo(() => Object.entries(groupedNotificationsByInboxKey).flatMap(([inboxKey, group]) => {
        const isExpanded = expandedNotificationGroups[inboxKey] ?? false;
        if (hasGroupedNotifications && isExpanded)
            return group;
    ...
    const archivedNotifications = useMemo(() => flattenInfiniteData(getArchivedNotifications.data) || [], [getArchivedNotifications.data]);
    ...
    const [lastSelectedItemInboxKey, setLastSelectedItemInboxKey] = useScopedStorage<string | undefined>('lastSelectedItemInboxKey', undefined);
    ...
    const detailItem: InboxDetailItem | undefined = useMemo(() => {
        const item = items?.find(function (n) {
            if (hasGroupedNotifications)
                return n.id === detailItemInboxId;
    ...
    const getSelectedItem = () => {
        return items.find((n) => {
            if (hasGroupedNotifications)
                return n.id === selectedItemInboxId;
    ...
    const handleDeleteItem = useCallbackRef((item: Notification | FollowUp) => {
        ...
        if (hasGroupedNotifications && selectedItemInboxId !== item.id)
            return;
        if (!hasGroupedNotifications && selectedItemInboxKey !== item.inbox_key)
            return;
        next();
    });
    ...
    const handleNavigate = (e: KeyboardEvent, dir: 1 | -1) => {
        ...
        if (hasGroupedNotifications && !selectedItemInboxId)
            return select(0);
        if (!hasGroupedNotifications && !selectedItemInboxKey)
            return select(0);
        const index = getSelectedItemIndex();
        if (index < 0)
            return select(0);
    ...
    const toggleUnread = () => {
        const selectedItem = getSelectedItem();
        if (!selectedItem || !isNotification(selectedItem))
            return;
     ...
     */
    useEffect(() => {
        if (detailItem || !detailItemInboxKey || !hasGroupedNotifications)
            return;
    ...
    return (<>
      ...
      <LayeredHotkeys keys={['u']} callback={toggleUnread} options={{ enabled: hasGroupedNotifications ? !!selectedItemInboxId : !!selectedItemInboxKey }}/>

      {followUpItem && isNotification(followUpItem) && followUpItem.follow_up_subject && (<HomeFollowUpDialog title={followUpItem.summary} id={followUpItem.follow_up_subject.id} type={followUpItem.follow_up_subject.type} viewerFollowUp={followUpItem.follow_up_subject.viewer_follow_up} 
        // creating the follow up will delete the notification, so advance to the next item
        onBeforeCreate={() => {
                if (!followUpItem.archived)
                    next();
            }} onOpenChange={(open) => {
                if (!open) {
      ...
      <InboxSplitViewContext.Provider value={context}>
        <div className='flex flex-1 overflow-hidden'>
          <div className={cn('w-full flex-col overflow-hidden lg:max-w-[600px] lg:basis-[30%] lg:border-r', {
            ...
            <div className='flex h-full flex-1 flex-col overflow-hidden'>
              <BreadcrumbTitlebar>
                <span className='flex flex-1 items-center gap-3'>
                  <InboxViewOptions view={view} rightSlot={
        ...
        view === 'updates' && <InboxFilterButtons notifications={notifications}/>}/>
              ...
              <ScrollableContainer disableStableGutter>
                {getItems.isLoading ? (<IndexPageLoading />) : noItems ? (<InboxIndexEmptyState inboxView={view}/>) : view === 'updates' ? (<InboxNotifications notifications={notifications}/>) : view === 'archived' ? (<UngroupedInboxNotifications notifications={archivedNotifications}/>) : view === 'later' ? (<InboxFollowUps followUps={followUps}/>) : view === 'activity' ? (<InboxActivity notifications={activity}/>) : null}

                <InfiniteLoader hasNextPage={!!getItems.hasNextPage} isError={!!getItems.isError} isFetching={!!getItems.isFetching} isFetchingNextPage={!!getItems.isFetchingNextPage} fetchNextPage={getItems.fetchNextPage}/>
              </ScrollableContainer>
            </div>
          </div>
...
function InboxGroup({ date, children }: PropsWithChildren & {
    date: string;
}) {
...
function InboxNotifications({ notifications }: {
    ...
    return Object.entries(groups).map(([date, items]) => {
        return (<InboxGroup key={date} date={date}>
        <AnimatePresence initial={false}>
          {items.map((item) => (<InboxNotificationItemGroup key={item[0].inbox_key} notifications={item}/>))}
        </AnimatePresence>
      </InboxGroup>);
    });
}
function UngroupedInboxNotifications({ notifications }: {
    notifications: Notification[];
}) {
    return (<ul className='flex flex-col gap-px p-2'>
      {notifications.map((notification) => (<m.li key={notification.inbox_key} initial={{ opacity: 0, height: 0, y: -4 }} animate={{ opacity: 1, height: 'auto', y: 0 }} exit={{ opacity: 0, height: 0, y: -4 }} transition={{ duration: 0.15 }}>
...
function InboxNotificationItemGroup({ notifications }: InboxNotificationItemGroupProps) {
    ...
    return (<m.div className='flex flex-col' initial={{ opacity: 0, height: 0, y: -4 }} animate={{ opacity: 1, height: 'auto', y: 0 }} exit={{ opacity: 0, height: 0, y: -4 }} transition={{ duration: 0.15 }}>
      <AnimatePresence initial={false}>
        {notifications.map((item, index) => {
            ...
            if (shouldHide)
                return null;
            return (<m.li key={item.id} initial={{ opacity: 0, height: 0, y: -4 }} animate={{ opacity: 1, height: 'auto', y: 0 }} exit={{ opacity: 0, height: 0, y: -4 }} transition={{ duration: 0.15 }}>
              <InboxItem item={item} variant={variant} isGroupExpanded={isExpanded} className={cn('transition-all duration-150', variant === 'group-parent' && isExpanded && 'rounded-b-none', variant === 'group-child' && index === notifications.length - 1 && 'rounded-t-none', variant === 'group-child' && index > 0 && index < notifications.length - 1 && 'rounded-none')}>
                <InboxNotificationItem notification={item} variant={variant} groupSize={notifications.length} isGroupExpanded={isExpanded} toggleGroup={() => setIsExpanded(!isExpanded)}/>
              </InboxItem>
            </m.li>);
        })}
      </AnimatePresence>
...
function InboxFollowUps({ followUps }: {
    followUps: FollowUp[];
}) {
    const groups = useMemo(() => groupByDate(followUps, (fu) => fu.show_at, 'asc'), [followUps]);
    return Object.entries(groups).map(([date, items]) => {
        return (<InboxGroup key={date} date={date}>
        {items.map((item) => (<m.li key={item.inbox_key} initial={{ opacity: 0, height: 0, y: -4 }} animate={{ opacity: 1, height: 'auto', y: 0 }} exit={{ opacity: 0, height: 0, y: -4 }} transition={{ duration: 0.15 }}>
...
function InboxActivity({ notifications }: {
    notifications: Notification[];
}) {
    const ref = useTrackActivityView();
    return (<ul ref={ref} className='flex flex-col gap-px p-2'>
      <AnimatePresence initial={false}>
        {notifications.map((item) => (<m.li key={item.inbox_key} initial={{ opacity: 0, height: 0, y: -4 }} animate={{ opacity: 1, height: 'auto', y: 0 }} exit={{ opacity: 0, height: 0, y: -4 }} transition={{ duration: 0.15 }}>
            <InboxItem item={item}>
...
function InboxIndexEmptyState({ inboxView }: {
    inboxView: InboxView;
}) {
    return (<IndexPageEmptyState>
      {inboxView === 'updates' ? (<InboxIcon className='text-gray-300 dark:text-gray-700' size={80}/>) : (<AlarmIcon className='text-gray-300 dark:text-gray-700' size={80}/>)}
    </IndexPageEmptyState>);
}
const Detail = memo(function Detail({ target }: {
...
function InboxItem({ item, children, variant = 'plain', isGroupExpanded, className }: InboxItemProps) {
    ...
    return (<ConditionalWrap condition={canHover} wrap={(children) => (<NotificationOverflowMenu item={item} type='context'>
          {children}
        </NotificationOverflowMenu>)}>
      <div ref={ref} className={cn('dark:focus:bg-tertiary group relative flex min-h-12 flex-none cursor-pointer scroll-m-2 scroll-mt-12 items-center gap-3 rounded-lg p-2.5', 'border-none outline-none ring-0 focus-within:border-none focus-within:outline-none focus-within:ring-0 focus:border-none focus:outline-none focus:ring-0', className, 
        // plain
        variant === 'plain' && isActive && 'bg-tertiary hover:bg-quaternary', variant === 'plain' && !isActive && 'hover:bg-tertiary', 
        // group-parent
        isActive && variant === 'group-parent' && 'bg-quaternary hover:bg-quaternary', !isActive && variant === 'group-parent' && 'hover:bg-tertiary', !isActive &&
            isGroupExpanded &&
            variant === 'group-parent' &&
            'hover:bg-quaternary bg-black/[0.025] dark:bg-white/[0.045]', 
        // group-child
        variant === 'group-child' && isActive && 'bg-quaternary hover:bg-quaternary', variant === 'group-child' && !isActive && 'hover:bg-quaternary bg-black/[0.025] dark:bg-white/[0.045]')}>
        <Link className='absolute inset-0 focus:outline-none focus:ring-0' href={getInboxItemRoutePath(item)} onClick={(e) => {
...
export function InboxSplitViewTitleBar({ children, hideSidebarToggle = false }: PropsWithChildren & {

...
import { FollowUp } from "../../../../packages/types/index.ts";
import { Avatar, Button, DotsHorizontal, UIText, VideoCameraIcon } from "../../../../packages/ui/src/index.tsx";
...
export function FollowUpListItem({ followUp }: {
    followUp: FollowUp;
}) {
    ...
    return (<>
      ...
      <div className='flex flex-1 flex-col'>
        <div className={cn('flex items-center justify-between gap-2 leading-tight', {
          ...
          {!canHover && (<NotificationOverflowMenu item={followUp} type='dropdown'>
...
function FollowUpListItemLeadingAccessory({ followUp }: {
    followUp: FollowUp;
}) {
    if (followUp.subject.member) {
        return (<div className={cn('flex h-6 w-6 items-start justify-end')}>
        <Avatar name={followUp.subject.member.user.display_name} urls={followUp.subject.member.user.avatar_urls} size='sm' rounded={followUp.subject.member.user.integration ? 'rounded' : 'rounded-full'}/>
      </div>);
    }

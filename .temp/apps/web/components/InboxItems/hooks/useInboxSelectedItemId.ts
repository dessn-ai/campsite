 ...
 */
const selectedItemInboxIdAtomFamily = atomFamily((scope: string) => atomWithStorage<string | undefined>(`${scope}:selected-inbox-item-id`, undefined));
const detailItemInboxIdAtomFamily = atomFamily((scope: string) => atomWithStorage<string | undefined>(`${scope}:detail-inbox-item-id`, undefined));
...
function useInboxSetSelection() {
    ...
    const setInboxSelection = useCallback((id: string) => {

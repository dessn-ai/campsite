...
import { AlarmIcon, ContextMenu, InboxArchiveIcon, InboxUnarchiveIcon, ReadSquareBadgeIcon, UnreadSquareBadgeIcon } from "../../../../packages/ui/src/index.tsx";
...
export function NotificationOverflowMenu({ item, type, children }: NotificationOverflowMenuProps) {
    ...
    if (type === 'context') {
        return (<ContextMenu asChild items={items} onOpenChange={setDropdownIsOpen}>
        {children}
      </ContextMenu>);
    }
    return (<DropdownMenu open={dropdownIsOpen} onOpenChange={setDropdownIsOpen} items={items} align='start' trigger={children}/>);
}

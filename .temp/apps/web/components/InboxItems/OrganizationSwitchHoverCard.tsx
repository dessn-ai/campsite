...
import { Notification, PublicOrganization } from "../../../../packages/types/index.ts";
import { Button, ChatBubbleIcon, InboxIcon, KeyboardShortcut, Link, LoadingSpinner, UIText } from "../../../../packages/ui/src/index.tsx";
import { HoverCard } from "../../../../packages/ui/src/HoverCard/index.ts";
import { ExistingThreadListItem } from "../Chat/ExistingThreadListItem.tsx";
import { EmptyState } from "../EmptyState/index.tsx";
import { InboxHoverCardItemLink } from "./InboxHoverCard.tsx";
import { InboxNotificationItem } from "./InboxNotificationItem.tsx";
import { defaultInboxView } from "./InboxSplitView.tsx";
...
export function OrganizationSwitchHoverCard({ organization, shortcut, children, alignOffset = -44, disabled }: {
    ...
    return (<HoverCard open={open} disabled={isDisabled} onOpenChange={(newVal) => {
            setOpen(newVal);
      ...
      <HoverCard.Content sideOffset={12} alignOffset={alignOffset}>
        ...
        <HoverCard.Content.TitleBar>
          <Button onClick={() => setSubview('notifications')} variant={subview === 'notifications' ? 'flat' : 'plain'} className='flex-1'>
...
function Chat({ organization, open }: {
    organization: PublicOrganization;
    open?: boolean;
}) {
    ...
    return (<>
      {hasThreads && (<div className='scrollbar-hide flex max-h-[420px] flex-col gap-px overflow-y-auto overscroll-contain p-2'>
          {threads.map((thread) => (<ExistingThreadListItem key={thread.id} thread={thread}/>))}
        </div>)}

      {!hasThreads && !isLoading && (<div className='flex flex-1 items-center justify-center px-6'>
...
function Notifications({ organization, open }: {
    organization: PublicOrganization;
    open?: boolean;
}) {

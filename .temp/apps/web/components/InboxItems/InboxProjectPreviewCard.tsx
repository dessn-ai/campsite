...
import { Post, Project } from "../../../../packages/types/generated.ts";
import { Button } from "../../../../packages/ui/src/Button/index.tsx";
import { ArrowRightIcon, BellCheckIcon, BellIcon, ClockIcon, LockIcon, StarFilledIcon, StarOutlineIcon } from "../../../../packages/ui/src/Icons/index.tsx";
...
function RecentPost({ post }: RecentPostProps) {
    ...
    return (<Link href={`/${scope}/posts/${post.id}`} key={post.id} className='hover:bg-tertiary flex flex-1 items-center gap-3 rounded-lg p-2'>
...
function ProjectPreviewRecentPosts({ project }: ProjectPreviewRecentPostsProps) {
    ...
    if (posts.length === 0)
        return null;
    return (<div className='bg-elevated flex w-full max-w-md flex-col rounded-lg border p-2'>
      <div className='gap-4.5 flex items-center px-3.5 py-2'>
        <ClockIcon />
        <UIText weight='font-semibold'>Recent posts</UIText>
      </div>

      {posts.slice(0, 3).map((post) => (<RecentPost post={post} key={post.id}/>))}

      {posts.length > 3 && (<Link href={`/${scope}/projects/${project.id}`} className='hover:bg-tertiary text-tertiary hover:text-primary mt-1 flex flex-1 items-center justify-center gap-3 rounded-lg p-2 text-center'>
...
function InboxProjectPreviewCard({ project }: InboxProjectPreviewCardProps) {
    const { scope } = useScope();
    const { mutate: deleteProjectSubscription, isPending: isDeletingSubscription } = useDeleteProjectSubscription(project.id);
    const { mutate: createProjectSubscription, isPending: isCreatingSubscription } = useCreateProjectSubscription(project.id);
    ...
    return (<div className='bg-secondary dark:bg-primary flex flex-1 flex-col items-center justify-center gap-6 p-6'>
      <div className={cn('bg-elevated relative flex w-full max-w-md flex-col rounded-lg border p-4', {
        ...
        {project.accessory && (<Link href={`/${scope}/projects/${project.id}`} className='mb-2 flex h-8 w-8 items-center justify-center font-["emoji"] text-2xl'>
            {project.accessory}
          </Link>)}
        ...
        {project.description && (<UIText className='mt-1 line-clamp-3 whitespace-pre-wrap' tertiary>
            {project.description}
          </UIText>)}

        {memberUsers && memberUsers.length > 1 && getMembers.total && (<div className='mt-4 flex items-center gap-3'>
        ...
        <div className='mt-4 flex flex-col gap-3'>
          {(showSubscriptionButton || showFavoriteButton) && (<div className='flex items-center gap-3'>
              {showSubscriptionButton && (<Button fullWidth variant='base' disabled={subscriptionIsLoading} onClick={() => {
              ...
              {showFavoriteButton && (<Button fullWidth variant='base' disabled={favoriteIsLoading} onClick={() => {
                ...
                }} leftSlot={project.viewer_has_favorited ? (<StarFilledIcon className={cn({

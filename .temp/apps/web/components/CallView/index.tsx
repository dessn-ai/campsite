...
import { BlurAtTopOptions, getMarkdownExtensions } from "../../../../packages/editor/src/index.ts";
import { Call, CallRecording, CallRecordingTranscription } from "../../../../packages/types/index.ts";
import { Avatar, Button, ChevronDownIcon, LayeredHotkeys, Link, LoadingSpinner, LockIcon, PaperAirplaneIcon, shortTimestamp, UIText } from "../../../../packages/ui/src/index.tsx";
import { cn, ConditionalWrap } from "../../../../packages/ui/src/utils/index.ts";
import { CallOverflowMenu } from "../Calls/CallOverflowMenu.tsx";
import { useGetCallPeerMembers } from "../Calls/useGetCallPeerUsers.tsx";
import { CallSharePopover } from "../CallSharePopover/index.tsx";
import { CallFavoriteButton } from "./CallFavoriteButton.tsx";
import { CallFollowUps } from "./CallFollowUps.tsx";
import { CallSchema, callSchema, getDefaultValues } from "./schema.ts";
import { CopyCurrentUrl } from "../CopyCurrentUrl.tsx";
import { EmptyState } from "../EmptyState/index.tsx";
import { FullPageError } from "../Error/index.tsx";
import { FacePile } from "../FacePile/index.tsx";
import { FullPageLoading } from "../FullPageLoading.tsx";
import { GeneratedContentFeedback } from "../GeneratedContentFeedback.tsx";
...
import { TitleTextField } from "../TitleTextField.tsx";
import { useScope } from "../../contexts/scope.tsx";
import { useBeforeRouteChange } from "../../hooks/useBeforeRouteChange.ts";
import { useCallSubscriptions } from "../../hooks/useCallSubscriptions.ts";
import { useGetCall } from "../../hooks/useGetCall.ts";
import { useGetCallRecordings } from "../../hooks/useGetCallRecordings.ts";
import { useGetCallRecordingTranscription } from "../../hooks/useGetCallRecordingTranscription.ts";
import { useUpdateCall } from "../../hooks/useUpdateCall.ts";
import { convertNumberToTimestring } from "../../utils/convertNumberToTimestring.ts";
import { flattenInfiniteData } from "../../utils/flattenInfiniteData.ts";
import { parseVtt } from "../../utils/vttParser.ts";
...
export function CallView({ callId }: Props) {
    ...
    const { data: recordingsData, isLoading: recordingsAreLoading, error: recordingsError } = useGetCallRecordings({ callId: callId });
    ...
    return (<CallViewContext.Provider value={call.id}>
...
function InnerCallView({ call, recordings }: {
    call: Call;
    ...
    if (!recordings.some((recording) => recording.url)) {
        ...
        <EmptyState icon={<LoadingSpinner />} title='Processing call' message='We’re still processing this call — hang tight.'/>
      </div>);
    }
    return (<div ref={trackRef} className='flex h-full flex-1 flex-col'>
      <CallTitlebar call={call}/>

      <ScrollableContainer>
        <div className='mx-auto flex w-full max-w-4xl flex-none flex-col px-4 pt-4 md:pt-6 lg:pt-8 xl:pt-12 2xl:pt-14'>
          <RecordingVideos recordings={recordings} activeRecordingIndex={activeRecordingIndex} setActiveRecordingIndex={setActiveRecordingIndex}/>
        </div>
        ...
        <CallTranscripts recordings={recordings} activeRecordingIndex={activeRecordingIndex} setActiveRecordingIndex={setActiveRecordingIndex}/>
      </ScrollableContainer>
    </div>);
}
...
function CallTranscripts({ recordings, activeRecordingIndex, setActiveRecordingIndex }: {
    recordings: CallRecording[];
    activeRecordingIndex: number;
    setActiveRecordingIndex: (index: number) => void;
}) {
    const recording = recordings.at(activeRecordingIndex);
    return (<div className='bg-secondary dark:bg-primary w-full flex-1 border-t'>
      <div className='mx-auto flex h-full w-full max-w-3xl flex-1 flex-col gap-4 px-4 py-4 md:py-6 lg:gap-6 lg:py-8 xl:py-12 2xl:py-14'>
        <div className='flex flex-col gap-4'>
          ...
          {recordings.length > 1 && (<div className='flex items-center gap-2'>
              {recordings.map((recording, index) => (<Button round key={recording.id} variant={index === activeRecordingIndex ? 'flat' : 'plain'} onClick={() => setActiveRecordingIndex(index)} className={cn({
        ...
        {recording ? (<CallRecordingTranscript callRecording={recording} activeRecordingIndex={activeRecordingIndex}/>) : (<MissingTranscript />)}
      </div>
    </div>);
}
...
function CallRecordingTranscript({ callRecording, activeRecordingIndex }: {
    ...
    if (callRecording.transcription_status === 'IN_PROGRESS')
        return (<EmptyState icon={<LoadingSpinner />} title='Processing transcript' message='We’re still processing this call’s transcript.'/>);
    if (!transcription?.vtt)
        return <MissingTranscript />;
    return <CallTranscript transcript={transcription} activeRecordingIndex={activeRecordingIndex}/>;
}
function CallTranscript({ transcript, activeRecordingIndex }: {
    ...
    return (<>
      <div className='flex flex-col'>
        {parsedVtt.map((group, index) => {
            ...
            return (<div className={cn('grid grid-cols-[24px,1fr] gap-4 py-2 text-left')} 
            // this is a static list, so we can be comfortable using the index as a key
            // eslint-disable-next-line react/no-array-index-key
            key={index}>
              <button onClick={() => navigateToTimestamp(Math.round(group[0].start))} className='text-tertiary hover:text-primary self-start'>
                {!member && group[0].speaker && (<UIText className='truncate pt-px text-right' size='text-xs'>
                    {group[0].speaker}
                  </UIText>)}

                {member && (<div className='flex justify-end'>
                    {!previousGroupHasSameUser && (<ConditionalWrap condition={!!member.id} wrap={(children) => (<MemberHovercard username={member.user.username}>{children}</MemberHovercard>)}>
                        <Avatar urls={member.user.avatar_urls} name={member.user.display_name} tooltip={member.user.display_name} size='sm'/>
                      </ConditionalWrap>)}
                  </div>)}
              </button>
              <div>
                <button onClick={() => navigateToTimestamp(Math.round(group[0].start))} className='text-quaternary hover:text-primary block'>
                ...
                {group.map((line) => {
                    return (<Fragment key={`vtt-${line.index}`}>
                      <span id={`vtt-${line.index.toString()}`} className={cn('-mx-0.5 w-full select-text rounded px-0.5 py-0.5 text-[15px]')} title={line.text}>
                        {line.text}
                      </span>{' '}
                    </Fragment>);
                })}
...
function CallDetails({ call }: {
    ...
    return (<div className='flex flex-col gap-2 pt-8'>
      ...
      {call.processing_generated_summary ? (<EmptyState icon={<LoadingSpinner />} title='Processing summary' message='We’re still processing this call’s summary.'/>) : (<>
          {call.viewer_can_edit ? (<CallDetailsForm 
            // key by call.id in order to reset tiptap editor state
            key={call.id} call={call}/>) : (<ReadOnlyCallSummary call={call}/>)}
      ...
      <div className='mt-8 flex items-start gap-3'>
        ...
        <UIText className='pt-0.5' tertiary>
          {callMembers
            .map((member) => member.role === 'guest' ? `${member.user.display_name} (Guest)` : member.user.display_name)
            .join(', ')}
        </UIText>
      </div>
    </div>);
}
function ReadOnlyCallSummary({ call }: {
    call: Call;
}) {
    ...
    return (<>
      ...
      {summary && (<div className='prose mt-4 select-text whitespace-pre-wrap focus:outline-none lg:leading-normal'>
          <RichTextRenderer content={summary} extensions={extensions}/>
        </div>)}
    </>);
}
function CallDetailsForm({ call }: {
    call: Call;
}) {
...
function callFormHasChanges({ isDirty, isValidating, isValid, watched, previous }: {
    isDirty: boolean;
...
function CallDetailsEditor({ call, defaultValues }: {
    call: Call;
    defaultValues: CallSchema;
}) {
    ...
    return (<>
      <TitleTextField ref={titleRef} className='mx-auto w-full text-2xl font-bold leading-[1.2]' placeholder='Meeting title' value={methods.getValues('title')} onChange={(value) => methods.setValue('title', value, { shouldDirty: true, shouldValidate: true })} onEnter={() => editorRef.current?.focus('start-newline')} onFocusNext={() => editorRef.current?.focus('restore')}/>
      <MarkdownEditor ref={editorRef} placeholder='Add meeting notes...' content={methods.getValues('summary')} onChangeDebounced={(html) => methods.setValue('summary', html, { shouldDirty: true })} onChangeDebounceMs={0} onBlurAtTop={focusTitle} containerClasses='px-0 pt-2' disableMentions disableSlashCommand/>
    </>);
}
function RecordingVideos({ recordings, activeRecordingIndex, setActiveRecordingIndex }: {
    recordings: CallRecording[];
    activeRecordingIndex: number;
    setActiveRecordingIndex: (index: number) => void;
}) {
    ...
    return (<>
      <LayeredHotkeys keys='space' callback={() => {
            if (!videoRef.current)
                return;
      ...
      {activeRecording?.url && (<>
          <video key={activeRecording.id} controls preload='auto' className='dark:border-primary aspect-video overflow-hidden rounded-2xl border border-transparent' id={getVideoElementId(activeRecordingIndex)} ref={videoRef}>
            <source src={activeRecording.url} type={'video/mp4'}/>
            <source src={activeRecording.url}/>
          </video>
        </>)}
      {recordings.length > 1 && (<div className='mt-2 flex items-center justify-center gap-0.5'>
          {recordings.map((recording, index) => (<button key={recording.id} className='group/dot flex h-4 w-4 items-center justify-center' onClick={() => setActiveRecordingIndex(index)}>
              <span className={`h-2 w-2 rounded-full ${index === activeRecordingIndex
...
function CallTitlebar({ call }: {
    call: Call;
}) {
    const { isSplitViewAvailable } = useIsSplitViewAvailable();
    return (<InboxSplitViewTitleBar hideSidebarToggle={isSplitViewAvailable}>
...
function BreadcrumbProjectAndCallTitle({ call }: {
    call: Call;
}) {
    ...
    return (<div className='flex min-w-0 flex-1 items-center gap-1.5'>
      {call.project && call.project_permission !== 'none' ? (<>
          <Link href={`/${scope}/projects/${call.project.id}`} className='break-anywhere flex min-w-0 items-center gap-1 truncate'>
          ...
          <span className='-ml-1 -mr-0.5 inline-flex min-w-1 items-center'>
            {call.viewer_can_edit && (<CallSharePopover call={call} align='start'>
      ...
      {isProcessing && (<span className='opacity-50'>
          <LoadingSpinner />
        </span>)}
      <Link href={`/${scope}/calls/${call.id}`} title={call?.title || 'Untitled'} className='break-anywhere min-w-0 truncate'>

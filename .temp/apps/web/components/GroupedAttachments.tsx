...
import { DisplayType } from "./InlinePost/index.tsx";
import { AttachmentGrid } from "./AttachmentGrid/index.tsx";
...
function contentIncludesAttachment(content: string, attachment: Attachment) {
    return (content.includes(`<post-attachment id="${attachment.id}"`) ||
        content.includes(`<media-gallery-item id="${attachment.id}"`));
}
function getRenderableAttachments(props: Props, opts?: {
    isTruncated?: boolean;
}) {
...
function UnrenderableAttachments({ attachments }: {
    attachments: Attachment[];
}) {
    if (attachments.length === 0)
        return null;
    return (<div className='divide-secondary flex flex-col divide-y rounded-md border'>
      {attachments.map((attachment) => (<FileAttachment showActions attachment={attachment} key={attachment.id}/>))}
    </div>);
}

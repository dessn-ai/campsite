...
import { EnrichedQuotedTweet, EnrichedTweet, enrichTweet, TweetBody, TweetInReplyTo, TweetMedia, useTweet, Verified, VerifiedBusiness, VerifiedGovernment, type TweetProps, type TwitterComponents } from 'react-tweet';
import { TweetUser, type Tweet as TweetType } from 'react-tweet/api';
import { AlertIcon, UIText } from "../../../../packages/ui/src/index.tsx";
import { useHasMounted } from "../../../../packages/ui/src/hooks/index.tsx";
import { cn } from "../../../../packages/ui/src/utils/index.ts";
function TweetContainer({ className, children, tweet }: {
    className?: string;
    children: ReactNode;
    tweet: EnrichedTweet;
}) {
    return (<div onClickCapture={(evt) => {
            const didClickLinkOrButton = evt.target instanceof Element && (evt.target.closest('a') || evt.target.closest('button'));
            if (didClickLinkOrButton)
                return;
            window.open(tweet.url, '_blank');
        }} className={cn('tweet bg-primary not-prose border-primary-opaque relative flex flex-1 rounded-xl border p-4 transition-all active:scale-[0.99]', className)}>
...
function TweetHeader({ tweet }: {
    ...
    const { user } = tweet;
    const isBusiness = user.verified_type === 'Business' || user.verified_type === 'Government' || user.profile_image_shape === 'Square';
    const isUser = user.profile_image_shape === 'Circle' || (!user.profile_image_shape && !isBusiness);
    return (<div className='flex items-center gap-3 pb-3'>
      <a href={tweet.url} className='' target='_blank' rel='noopener noreferrer'>
        <div className={cn('overflow-hidden', {
            'rounded-md': isBusiness,
            'rounded-full': isUser
        })}>
          <Image className={cn('h-11 w-11', {
        ...
        })} src={user.profile_image_url_https} alt={user.name} width={176} height={176}/>
        </div>
      </a>
      <div className='flex flex-1 flex-col gap-0.5'>
        <a href={tweet.url} className='flex flex-1 items-center gap-0.5 leading-none' target='_blank' rel='noopener noreferrer'>
          <span className='text-primary text-[15px] font-semibold'>
            <span title={user.name}>{user.name}</span>
          </span>
          <VerifiedBadge user={user}/>
        </a>
        <a href={tweet.url} className='text-tertiary text-[15px] leading-none' target='_blank' rel='noopener noreferrer'>
...
function TweetInfoCreatedAt({ tweet }: {
    tweet: EnrichedTweet;
}) {
    ...
    return !createdAt ? null : (<a className={'text-tertiary -mb-1 mt-3 flex items-center text-sm hover:underline'} href={tweet.url} target='_blank' rel='noopener noreferrer' aria-label={format(createdAt, 'h:mm a · MMM d, y')}>
      <time dateTime={createdAt.toISOString()}>{format(createdAt, 'h:mm a · MMM d, y')}</time>
    </a>);
}
function QuotedTweetHeader({ tweet }: {
    tweet: EnrichedQuotedTweet;
}) {
    const { user } = tweet;
    const isBusiness = user.verified_type === 'Business' || user.verified_type === 'Government' || user.profile_image_shape === 'Square';
    const isUser = user.profile_image_shape === 'Circle' || (!user.profile_image_shape && !isBusiness);
    return (<div className='mb-2 flex gap-2 overflow-hidden whitespace-nowrap break-words'>
...
function QuotedTweetBody({ tweet }: {
    tweet: EnrichedQuotedTweet;
}) {
    return (<p className='not-prose m-0 whitespace-pre-wrap break-words text-[15px] font-normal leading-normal'>
      {tweet.entities.map((item, i) => (
        // eslint-disable-next-line react/no-array-index-key
        <span key={i} className='not-prose' dangerouslySetInnerHTML={{ __html: item.text }}/>))}
    </p>);
}
function QuotedTweetContainer({ tweet, children }: {
    tweet: EnrichedQuotedTweet;
    children: ReactNode;
}) {
    return (<div className='hover:bg-secondary my-4 w-full overflow-hidden rounded-xl border p-3' onClick={(e) => {
            e.preventDefault();
            window.open(tweet.url, '_blank');
        }}>
...
function QuotedTweet({ tweet }: {
    ...
    return (<QuotedTweetContainer tweet={tweet}>
      <QuotedTweetHeader tweet={tweet}/>
      <QuotedTweetBody tweet={tweet}/>
      {tweet.mediaDetails?.length ? (<div className='tweet-media-max-height'>
          <TweetMedia quoted tweet={tweet}/>
        </div>) : null}
    </QuotedTweetContainer>);
}
function CustomTweet({ className, tweet: t, components }: {
    className?: string;
    tweet: TweetType;
    components?: TwitterComponents;
}) {
    const tweet = enrichTweet(t);
    return (<TweetContainer className={className} tweet={tweet}>
      <TweetHeader tweet={tweet}/>
      {tweet.in_reply_to_status_id_str && (<div className='pb-1.5'>
      ...
      {tweet.quoted_tweet && (<div className='quoted-tweet relative mt-3 block rounded-lg'>
          <QuotedTweet tweet={tweet.quoted_tweet}/>
          {tweet.quoted_tweet.url && (<a href={tweet.quoted_tweet.url} className='absolute inset-0' target='_blank' rel='noopener noreferrer'/>)}
        </div>)}
      <TweetInfoCreatedAt tweet={tweet}/>
    </TweetContainer>);
}
function TweetSkeleton() {
    return (<div className='tweet relative w-full flex-1 rounded-xl border p-4 transition-all active:scale-[0.99]'>
...
function TweetTombstone({ text }: {
    text: string;
}) {
    return (<div className='tweet relative rounded-xl border p-3 transition-all active:scale-[0.99]'>
...
export function TweetPreview({ className, id, components }: TweetProps & {
    className?: string;
}) {
    ...
    if (error || !data)
        return <TweetTombstone text='Unable to load this post'/>;

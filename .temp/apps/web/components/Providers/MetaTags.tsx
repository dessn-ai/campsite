...
import { DEFAULT_SEO, IS_NGROK } from "../../../../packages/config/src/index.ts";
import { PostSeoInfo } from "../../../../packages/types/index.ts";
import { useGetPostSeoInfo } from "../../hooks/useGetPostSeoInfo.ts";
interface Props {
...
export function MetaTags(props: Props) {
    ...
    return (<>
      {postSeoInfo ? (<>
          <DefaultSeo title={postSeoInfo.seo_title} description={postSeoInfo.seo_description} openGraph={{
        ...
        </>) : (<DefaultSeo {...DEFAULT_SEO} title={isOrgPage ? 'Campsite' : DEFAULT_SEO.title} openGraph={{

...
import { BlurAtTopOptions } from "../../../../packages/editor/src/extensions/BlurAtTop.ts";
...
import { useEditorSync } from "../Post/Notes/useEditorSync.ts";
import { NoteContent, NoteContentRef } from "./NoteContent.tsx";
import { useHandleBottomScrollOffset } from "./useHandleBottomScrollOffset.ts";
import { useHasConnectionIssue } from "./useHasConnectionIssue.ts";
interface Props {
    note: Note;
    quickNote?: boolean;
}
export function NoteEditor({ note, quickNote }: Props) {
    ...
    const onContainerMouseUp = (event: MouseEvent) => {
        if (mouseDownInEditorRef.current)
            return;
        const isReactionsClick = event.target === reactionsRef.current || !!reactionsRef.current?.contains(event.target as Node);
    ...
    return (<div className='flex w-full flex-1 flex-col'>
      {syncError === 'invalid-schema' ? (
        ...
        </ConnectionIssueBanner>) : syncError ? (
        ...
        </ConnectionIssueBanner>) : showConnectionIssues && !syncError ? (
        ...
        </ConnectionIssueBanner>) : null}

      <div id={id} 
    // pb-[x] is how much space we want to leave between the bottom of the note editor and the viewport as a user types
    className='flex w-full flex-1 cursor-text flex-col gap-4 pb-[10vh]' onDragOverCapture={(e) => {
            e.preventDefault();
            editorRef?.current?.handleDragOver(true, e);
        }} onDragLeaveCapture={(e) => editorRef?.current?.handleDragOver(false, e)} onDragExitCapture={(e) => editorRef?.current?.handleDragOver(false, e)} onDrop={(e) => editorRef?.current?.handleDrop(e)} onMouseDownCapture={onContainerMouseDown} onMouseUpCapture={onContainerMouseUp}>
        {!quickNote && (<div className={cn('group/title flex w-full flex-row-reverse items-start gap-4 max-lg:flex-col md:gap-5 lg:gap-6', {
                'flex-col-reverse max-lg:flex-col-reverse': isMobile
            })}>
            <NoteTitle ref={titleRef} note={note} onEnter={() => editorRef.current?.focus('start-newline')} onFocusNext={() => editorRef.current?.focus('restore')} autofocus={canAutofocusTitle}/>
          </div>)}

        <div ref={editorContainerRef} className='w-full'>
          <NoteContent ref={editorRef} note={note} provider={provider} editable={editable} isSyncError={!!syncError} content={note?.description_html || EMPTY_HTML} onBlurAtTop={focusTitle} autofocus={canAutofocusDescription} onKeyDown={onKeyDownScrollHandler}/>
        </div>
      </div>
    </div>);
}
...
const NoteTitle = forwardRef<HTMLTextAreaElement, NoteTitleProps>(function NoteTitle(props, ref) {
    ...
    const save = useCallback(() => {
        if (!note?.viewer_can_edit || !note?.id || formTitle === note.title)
            return;
        updateNote({ noteId: note.id, title: formTitle ?? '' }, {
    ...
    return (<TitleTextField ref={ref} className='mx-auto w-full max-w-[44rem] text-[clamp(2rem,_4vw,_2.5rem)] font-bold leading-[1.2]' placeholder={note ? 'Untitled' : undefined} value={formTitle} onChange={(value) => {
            setFormTitle(value);
            debouncedSave();
        }} onEnter={onEnter} onFocusNext={onFocusNext} autoFocus={autofocus} readOnly={!note?.viewer_can_edit}/>);
});
function ConnectionIssueBanner({ children }: {
    children: React.ReactNode;
}) {
    return (<button className='mx-auto mb-8 w-full max-w-[44rem] rounded-md border-yellow-200 bg-yellow-100 p-3 dark:border-yellow-800/50 dark:bg-yellow-950/30' onClick={() => {

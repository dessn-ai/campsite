...
import { Avatar, Tooltip } from "../../../../packages/ui/src/index.tsx";
import { cn } from "../../../../packages/ui/src/utils/index.ts";
import { useScope } from "../../contexts/scope.tsx";
import { userListTooltipText } from "../../utils/userListTooltipText.ts";
type UserWithWrapper = User & {
...
export function FacePile({ limit = 3, users, size = 'base', link = false, totalUserCount, showTooltip = true, showIsPresent = false }: Props) {
    ...
    return (<span className={cn('flex', containerLeftPadding)}>
      {visibleUsers.map((user, index) => {
            const shouldClip = showOverflow ? true : index >= 0 && index < maxUsers - 1;
            const wrapper = user.wrapper || ((children) => children);
            return (<span key={user.logged_in ? user.id : user.display_name} className={cn('flex', overlapMargin)}>
            {wrapper(<Avatar size={size} name={user.display_name} urls={user.avatar_urls} href={link ? `/${scope}/people/${user.username}` : undefined} tooltip={showTooltip ? user.display_name : undefined} tooltipDelayDuration={0} clip={shouldClip ? 'facepile' : undefined} fade={showIsPresent && !user.isPresent}/>)}
          </span>);
        })}

      {showOverflow && (<Tooltip delayDuration={0} label={tooltipText}>
          <div className={cn('flex flex-none items-center justify-center rounded-full bg-black text-white dark:bg-neutral-700', overflowMinWidth, overflowPadding, overlapMargin)}>

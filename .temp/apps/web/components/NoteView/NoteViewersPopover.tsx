...
import { ANIMATION_CONSTANTS, Button, CONTAINER_STYLES, GlobeIcon, Popover, PopoverContent, PopoverPortal, PopoverTrigger, UIText } from "../../../../packages/ui/src/index.tsx";
...
import { useGetNoteViews } from "../../hooks/useGetNoteViews.ts";
import { useNoteViewsWithPresence } from "../../hooks/useNoteViewsWithPresence.ts";
interface Props {
    note: Note;
}
export function NoteViewersPopover({ note }: Props) {
    ...
    const facepileUsers = useMemo(() => {
        ...
        const viewFacepileUsers = viewMembers
            ?.map((view) => view.user)
            .filter((user) => !!user)
    ...
    if (!currentUser?.logged_in)
        return null;
    if (viewMembers.length === 0 && note.follow_ups.length === 0)
        return null;
    if (!facepileUsers.length)
        return null;
    return (<Popover open={open} onOpenChange={setOpen}>
      ...
      <AnimatePresence>
        {open && (<PopoverPortal forceMount>
            <PopoverContent avoidCollisions className='w-[320px]' asChild forceMount side='bottom' align='end' sideOffset={8}>
              <m.div {...ANIMATION_CONSTANTS} className={cn(CONTAINER_STYLES.base, CONTAINER_STYLES.shadows, 'bg-elevated rounded-lg dark:border dark:bg-clip-border')}>
...
function Views({ note }: {
    ...
    const nonMemberViewersDescriptor = note.non_member_views_count > 0
    ...
    return (<div className='p-1.5'>
      ...
      {note.non_member_views_count > 0 && (<div className='flex items-center gap-2 px-2 py-1.5'>

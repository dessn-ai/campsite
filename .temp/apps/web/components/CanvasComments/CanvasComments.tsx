...
import { Attachment, Comment } from "../../../../packages/types/index.ts";
import { LayeredHotkeys } from "../../../../packages/ui/src/DismissibleLayer/index.tsx";
import { FileMenu } from "../AttachmentLightbox/FileDropdown.tsx";
import { usePostAttachmentLightbox } from "../AttachmentLightbox/PostAttachmentLightbox.tsx";
import { useGetPost } from "../../hooks/useGetPost.ts";
import { useGetPostCanvasComments } from "../../hooks/useGetPostCanvasComments.ts";
...
import { useCommentCursor } from "../ZoomPane/useCommentCursor.ts";
import { Annotation, ZoomAnnotationsOverlay } from "../ZoomPane/ZoomAnnotationsOverlay.tsx";
import { ZoomCanvasTileRenderer } from "../ZoomPane/ZoomCanvasTileRenderer.tsx";
import { ZoomImageRenderer } from "../ZoomPane/ZoomImageRenderer.tsx";
import { MediaCoordinates, ZoomPane } from "../ZoomPane/ZoomPane.tsx";
import { CanvasComment, NewCanvasComment } from "./CanvasComment.tsx";
export const selectedCanvasCommentIdAtom = atom<string | undefined>(undefined);
export const newCommentCoordinatesAtom = atom<{
    x: number;
    y: number;
...
export const CanvasComments = memo(function CanvasComments({ attachment, preventNewComment }: Props) {
    ...
    const shouldOpenComment = useCallback((comment: Comment) => {
        return (comment.id === currentCanvasCommentId || comment.replies.some((reply) => reply.id === currentCanvasCommentId));
    ...
    const onCanvasSelected = useCallback((mediaCoords: MediaCoordinates | null) => {
    ...
    const commentAnnotations = useMemo(() => {
        ...
        const annotations: CommentAnnotation[] = comments
            ?.map((comment) => {
            if (attachment.id === comment.attachment_id &&
                comment.x &&
                comment.y &&
    ...
    if (!imageUrl || !blurredImageUrl)
        return <FullPageLoading />;
    return (<>
      <LayeredHotkeys keys='Escape' callback={(e) => {
            e.stopPropagation();
            e.preventDefault();
            setNewCommentCoordinates(undefined);
            setCurrentCanvasCommentId(undefined);
      ...
      <ZoomPane width={imageWidth} height={imageHeight} onClick={onCanvasSelected}>
        <FileMenu type='menu' attachment={attachment} links={[]}>
          <>
            {tile && (<ZoomCanvasTileRenderer width={imageWidth} height={imageHeight} src={imageUrl} backgroundSrc={blurredImageUrl} style={{ cursor }}/>)}
            {!tile && <ZoomImageRenderer width={imageWidth} height={imageHeight} src={imageUrl} style={{ cursor }}/>}
          </>
        </FileMenu>

        <ZoomAnnotationsOverlay annotations={commentAnnotations} getNode={(annotation) => {
            if (!isSuccess)
                return null;
            const sharedProps = {

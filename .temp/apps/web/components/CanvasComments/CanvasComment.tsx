...
import { Comment, Post, User } from "../../../../packages/types/index.ts";
import { ANIMATION_CONSTANTS, RelativeTime, UIText } from "../../../../packages/ui/src/index.tsx";
import { cn } from "../../../../packages/ui/src/utils/index.ts";
import { selectedCanvasCommentIdAtom } from "./CanvasComments.tsx";
import { CommentComponent } from "../Comments/Comment.tsx";
import { FacePile } from "../FacePile/index.tsx";
import { useGetComment } from "../../hooks/useGetComment.ts";
...
import { panZoomAtom, zoomAtom } from "../ZoomPane/atom.ts";
export const hoveredCanvasCommentAtom = atom<string | undefined>(undefined);
const COLLISION_PADDING = 8;
function getTranslation({ coordinates, transform, triggerRect, commentRect, viewport }: {
...
function InnerCanvasComment({ post, coordinates, attachmentId, comment, onCreate, onSelected, onDismiss, isOpen = false }: InnerCanvasCommentProps) {
    ...
    return (<m.div className={cn('absolute -translate-y-[32px]', {
            'z-[3]': isOpen,
      ...
      <PopoverPrimitive.Root open={isOpen} onOpenChange={(open) => {
        ...
        <PopoverPrimitive.Trigger ref={triggerRef} className={cn('group relative rounded-[40px_40px_40px_4px] !ring-0', {
          ...
          <m.div className='relative z-10' initial={{ y: 0, x: 0 }} animate={{
            y: isExpanded ? -28 : 0,
            x: isExpanded ? 4 : 0
        }} transition={transition}>
            <CanvasCommentFacepile comment={comment}/>
          </m.div>
          {comment && (<div aria-hidden className={cn('absolute bottom-0 left-0 rounded-[20px_20px_20px_2px] group-focus-visible:ring-4 group-focus-visible:ring-blue-500/20', {
                'pointer-events-none': !isExpanded
            })}>
              <m.div className='dark:bg-elevated flex min-w-[32px] origin-bottom-left items-center overflow-hidden rounded-[inherit] bg-white pr-3 shadow-md ring-1 ring-black/5 transition-shadow hover:shadow-lg dark:shadow-[inset_0px_1px_0px_rgba(255,255,255,0.10),_0px_2px_4px_rgba(0,0,0,0.5),_0px_0px_0px_1px_rgba(0,0,0,1)]' initial={{ maxWidth: triggerRect.width, height: 32 }} animate={{
                maxWidth: (isExpanded ? 300 : 0) + triggerRect.width,
                height: isExpanded ? 64 : 32
            }} transition={transition}>
                <m.div className='flex flex-col whitespace-nowrap pr-2 text-left' style={{ paddingLeft: triggerRect.width + 8 }} initial={{ opacity: 0 }} animate={{
                opacity: isExpanded ? 1 : 0
                  ...
                  <div className='line-clamp-1 opacity-60'>
                    <CommentDescription subjectId={post.id} subjectType='Post' comment={comment} isEditing={false} isReply={false} setIsEditing={() => {
                return;
            }}/>
                  </div>
                </m.div>
          ...
          {!comment && (<div aria-hidden className={cn('pointer-events-none absolute bottom-0 left-0 rounded-[20px_20px_20px_2px] group-focus-visible:ring-4 group-focus-visible:ring-blue-500/20')}>
              <m.div className='dark:bg-elevated flex min-w-[32px] origin-bottom-left items-center overflow-hidden rounded-[inherit] bg-white pr-3 shadow-md ring-1 ring-black/5 transition-shadow hover:shadow-lg dark:shadow-[inset_0px_1px_0px_rgba(255,255,255,0.10),_0px_2px_4px_rgba(0,0,0,0.5),_0px_0px_0px_1px_rgba(0,0,0,1)]' initial={{ maxWidth: triggerRect.width, height: 32 }} transition={transition}/>
            </div>)}
        </PopoverPrimitive.Trigger>

        <AnimatePresence>
          {isOpen && (<PopoverPrimitive.Content forceMount ref={popoverRef} asChild avoidCollisions={false} side={side} sideOffset={8} collisionPadding={8} align='start' onKeyDownCapture={(evt) => {
                jumpIntoView();
                if (evt.key === 'Escape') {
                    evt.preventDefault();
                    evt.stopPropagation();
              ...
              <m.div ref={commentRef} className='scrollable w-[365px] outline-none' style={{
                translateY,
                // Hide comment until it has been measured
                opacity: !commentRect.width ? 0 : undefined
            }}>
                <m.div {...ANIMATION_CONSTANTS} className={cn('bg-elevated flex w-full flex-col rounded-xl shadow-lg shadow-black/20 ring-1 ring-black/[0.04] dark:shadow-[inset_0px_1px_0px_rgba(255,255,255,0.04),_0px_2px_12px_rgba(0,0,0,0.4),_0px_0px_0px_1px_rgba(0,0,0,0.8)] dark:ring-white/[0.02]', 'origin-[--radix-popover-content-transform-origin]')} data-zoom-wheel-disabled>
                ...
                'border-t': !!comment
                    ...
                    <PostCommentComposer maxHeight='120px' placeholder={hasServerComment ? 'Write a reply...' : 'Write a comment...'} replyingToCommentId={hasServerComment ? comment.id : undefined} onCreated={(comment) => {
                if (!hasServerComment) {
                    onCreate?.(comment.id);
            ...
            }} attachmentId={attachmentId} closeComposer={onDismiss} autoFocus={!comment} initialValues={!comment ? coordinates : undefined} display='inline' postId={post.id}/>
                  </div>
                </m.div>
              </m.div>
...
function CanvasCommentFacepile({ comment }: {
    comment: Comment | undefined;
}) {
    const { data: currentUser } = useGetCurrentUser();
    const uniqueAuthors = useMemo(() => {
        if (!comment)
            return [{ ...currentUser, type_name: 'user' } as User];
        const set = new Set();
    ...
    return (<span className='dark:bg-elevated relative z-10 flex cursor-pointer overflow-hidden rounded-[40px_40px_40px_4px] bg-white p-1 transition-all'>

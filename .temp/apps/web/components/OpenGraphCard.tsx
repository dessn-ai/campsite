...
export function OpenGraphCard({ className, url, onForceRemove }: OpenGraphCardProps) {
    ...
    if (isLoading)
        return null;
    if (!data || (data && !data.title && !data.image_url))
        return null;
    return (<Link href={url} target='_blank' rel='noopener noreferrer' className={cn('border-primary-opaque not-prose flex flex-1 overflow-hidden rounded-lg border transition-all active:scale-[0.99]', className)}>
      <div className={cn('flex-1', {
            'grid grid-rows-[minmax(0,1fr)_auto]': !faviconFallback,
            'flex flex-row items-center gap-6 px-3 py-2.5 pr-5': faviconFallback
        })}>
        {!faviconFallback && data?.image_url && !ogImageIsBroken && (<div className={cn('row-span-1 flex aspect-[2/1] border-b', {
                'bg-secondary': !data?.image_url
            })}>
            {data?.image_url && !ogImageIsBroken && (<Image src={data.image_url} alt={''} className='flex h-full w-full object-cover object-center' width={600} height={300} onError={() => setOgImageIsBroken(true)}/>)}
          </div>)}
        ...
        <div className={cn('row-span-1 flex flex-1 flex-col overflow-hidden', { 'px-3 py-2.5': !faviconFallback })}>
          ...
          {(isLoading || data) && (<UIText className='break-anywhere line-clamp-1 min-w-0 text-[15px] font-medium' primary>
              {isLoading ? 'Loading...' : data?.title}
            </UIText>)}
        </div>

        {faviconFallback && (<div className='h-5 w-5'>
            {data?.favicon_url && !faviconError && (<Image src={data?.favicon_url} alt={''} className='flex rounded-sm object-cover object-center' width={24} height={24} onError={() => setFaviconError(true)}/>)}
          </div>)}
      </div>
    </Link>);
}

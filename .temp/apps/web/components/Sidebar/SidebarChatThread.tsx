...
import { ChatBubbleIcon, VideoCameraFilledIcon } from "../../../../packages/ui/src/Icons/index.tsx";
...
import { ThreadHoverCard } from "../ThreadView/ThreadHoverCard.tsx";
import { SidebarLink } from "./SidebarLink.tsx";
export function SidebarChatThread({ thread, onRemove, removeTooltip, onPeek, isDragging }: {
    ...
    return (<ThreadHoverCard thread={thread} disabled={isDragging} 
    ...
    onOpenChange={(open) => {
      ...
      <SidebarLink id={thread.id} key={thread.id} as={`/${scope}/chat/${thread.id}`} href={href} label={thread.title} onClick={onClick} labelAccessory={!thread.group && thread.other_members.length === 1 && <MemberStatus status={status}/>} scroll={false} active={active} onMouseEnter={() => setPrefetch(true)} onMouseLeave={() => setPrefetch(false)} onRemove={onRemove} removeTooltip={removeTooltip} unread={unread} trailingAccessory={<>
            {thread.active_call && (<div className={cn('flex h-4 w-4 items-center justify-center rounded-md', {
                    'bg-green-500 text-white': onCall,
                    'bg-green-100 text-green-500 dark:bg-green-900/50': !onCall
                })}>
                <VideoCameraFilledIcon size={12}/>
              </div>)}
          </>} leadingAccessory={thread.integration_dm ? (<Avatar urls={thread.avatar_urls} name={thread.title} size='xs' rounded='rounded-md'/>) : thread.group ? (thread.image_url ? (<Avatar urls={thread.avatar_urls} name={thread.title} size='xs'/>) : memberCount > 0 ? (<span className='bg-quaternary text-tertiary flex h-5 w-5 items-center justify-center rounded-md font-mono text-xs font-semibold'>
                {memberCount}
              </span>) : (<ChatBubbleIcon />)) : (firstMember && <MemberAvatar displayStatus member={firstMember} size='xs'/>)}/>
    </ThreadHoverCard>);
}

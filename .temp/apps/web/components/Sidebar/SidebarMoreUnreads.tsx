...
import { getSidebarLinkId } from "./SidebarLink.tsx";
...
export const setUnreadSidebarItemIdsAtom = atom(null, (_, set, action: {
    type: 'remove';
    id: string;
} | {
    type: 'add';
...
function SidebarMoreUnreadsButton({ active, onClick, align, offset = 0 }: SidebarMoreUnreadsButtonProps) {
    return (<AnimatePresence>
      {active && (<m.button transition={{
                ...
                opacity: 0,
                y: align === 'top' ? 40 : 12 - offset
            }} onClick={onClick} className='absolute bottom-2 z-10 flex transform-gpu items-center justify-center rounded-full bg-blue-500 px-4 py-2 text-blue-50 shadow shadow-[inset_0px_1px_0px_rgba(255,255,255,0.04),_0px_2px_12px_rgba(0,0,0,0.4),_0px_0px_0px_1px_rgba(0,0,0,0.8)]'>
...
export function SidebarMoreUnreadsTop() {
    ...
    const firstUnreadSidebarbarItemId = useMemo(() => {
        ...
        if (!scrollElement || !unreadSidebarItemIds.length)
            return;
...
export function SidebarMoreUnreadsBottom() {
    ...
    return (<SidebarMoreUnreadsButton align='bottom' active={!!lastUnreadSidebarbarItemId} onClick={handleClick} offset={offset}/>);
}

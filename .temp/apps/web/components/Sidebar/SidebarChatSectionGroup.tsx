...
export function SidebarChatSectionGroup() {
    ...
    return (<SidebarGroup className='group/chat'>
      <div className='flex items-center gap-px'>
        <SidebarCollapsibleButton collapsed={collapsed} setCollapsed={setCollapsed} label='Chat'/>

        <button onClick={() => setCreateDialogOpen(true)} className='hover:bg-quaternary text-tertiary hover:text-primary group flex h-6 w-6 items-center justify-center rounded-md p-0.5 opacity-0 focus:outline-0 focus:ring-0 group-hover/chat:opacity-100 group-has-[[data-state="open"]]/chat:opacity-100'>
          <PlusIcon size={16} strokeWidth='2'/>
        </button>
      </div>

      <ul className='flex flex-col gap-px'>
        {renderableItems?.map((thread) => (<SidebarChatThread thread={thread} key={thread.id} onPeek={setHoveredId} location='chats'/>))}
        {!collapsed && (<SidebarLink id='new-chat' onClick={() => setCreateDialogOpen(true)} label='New chat' leadingAccessory={<PlusIcon />}/>)}
      </ul>

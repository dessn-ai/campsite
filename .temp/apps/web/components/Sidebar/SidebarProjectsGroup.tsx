...
import { cn, CompassIcon, DotsHorizontal, Link, PlusIcon, ReorderDotsIcon, UIText } from "../../../../packages/ui/src/index.tsx";
...
import { SidebarCollapsibleButton } from "./SidebarCollapsibleButton.tsx";
import { SidebarGroup } from "./SidebarGroup.tsx";
import { SidebarLink } from "./SidebarLink.tsx";
import { SidebarProject } from "./SidebarProject.tsx";
...
import { useReorderProjectMemberships } from "../../hooks/useReorderProjectMemberships.ts";
...
export function SidebarProjectsGroup() {
    ...
    if (!hasJoinedSpaces) {
        ...
        {!collapsed && (<div className='text-quaternary p-2 pt-0.5'>
            <div className='flex flex-col gap-1'>
              <UIText size='text-xs' inherit>
                Join channels for quick access to projects, teams, and topics you care about.
              </UIText>
              {organization?.viewer_can_see_projects_index && (<Link href={`/${scope}/projects`} className='text-blue-500 hover:underline'>
                  <UIText size='text-xs' inherit>
                    Browse all channels
                  </UIText>
                </Link>)}
    ...
    return (<SidebarGroup className='group/spaces'>
      <div className='flex items-center gap-px'>
        ...
        open={dropdownOpen} onOpenChange={setDropdownOpen} align='start' side='bottom' sideOffset={2} items={items} trigger={<button className='hover:bg-quaternary data-[state="open"]:bg-quaternary group flex h-6 w-6 items-center justify-center rounded-md p-0.5 opacity-0 focus:outline-0 focus:ring-0 group-hover/spaces:opacity-100 group-has-[[data-state="open"]]/spaces:opacity-100 data-[state="open"]:opacity-100'>
      ...
      <Reorder.Group key={collapsed ? 'collapsed' : 'expanded'} ref={containerRef} axis='y' values={sortedIds} onReorder={onReorder} className='flex flex-col gap-px'>
        {renderableItems?.map((projectMembership) => (<Reorder.Item key={projectMembership.id} value={projectMembership.id} id={projectMembership.id} layout='position' drag={!collapsed} dragConstraints={containerRef} dragElastic={0.065} onDragStart={() => setDraggingId(projectMembership.id)} onDragEnd={() => {
            ...
            {!collapsed && (<span className='text-quaternary absolute -left-[11px] top-1/2 -translate-y-1/2 cursor-move opacity-0 group-hover/reorder-item:opacity-100 group-has-[[data-state="open"]]/reorder-item:opacity-100'>
      ...
      {!collapsed && organization?.viewer_can_see_new_project_button && (<SidebarLink id='new-projects' label='New channel' onClick={() => setCreateDialogOpen(true)} leadingAccessory={<PlusIcon />}/>)}

      <CreateProjectDialog onOpenChange={setCreateDialogOpen} open={createDialogOpen} onCreate={(channel) => {

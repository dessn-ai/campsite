...
import { Call, Note, Post } from "../../../../../packages/types/generated.ts";
...
export const recentlyViewedAtom = atomFamily((scope: CookieValueTypes) => atomWithWebStorage<LocalRecentlyViewedType[]>(`recently-viewed:${scope}`, []));
...
const addRecentlyViewedAtom = atom(null, (get, set, { scope, item }: {
...
const removeRecentlyViewedAtom = atom(null, (get, set, { scope, id }: {
    scope: CookieValueTypes;
    id: string;
}) => {
    const scoped = recentlyViewedAtom(scope);
...
const updateRecentlyViewedAtom = atom(null, (get, set, { scope, item }: {
    scope: CookieValueTypes;
    item: RecentlyViewedItem;
}) => {
    const scoped = recentlyViewedAtom(scope);
...
export function useSyncRecentlyViewedItem({ id, post, note, call, isError }: RecentlyViewedItem & {
    isError: boolean;
}) {
    const { scope } = useScope();

...
import { NoteFilledIcon, PostFilledIcon, UIText, VideoCameraFilledIcon } from "../../../../../packages/ui/src/index.tsx";
...
import { LocalCall, LocalNote, LocalPost, useSyncRecentlyViewedItem } from "./utils.ts";
...
function RecentlyViewedContainer({ onSelect, value, children, route }: RecentlyViewedContainerProps) {
    return (<HighlightedCommandItem value={value} className='group h-9 gap-2 pr-1.5' onSelect={() => {
            onSelect();
            Router.push(route);
        }}>
      {children}
...
export function RecentlyViewedPost({ post, onSelect }: {
    post: LocalPost;
    onSelect: () => void;
}) {
    ...
    return (<RecentlyViewedContainer value={post.id} route={`/${scope}/posts/${post.id}`} onSelect={onSelect}>
...
export function RecentlyViewedNote({ note, onSelect }: {
    note: LocalNote;
    onSelect: () => void;
}) {
    ...
    return (<RecentlyViewedContainer value={note.id} onSelect={onSelect} route={`/${scope}/notes/${note.id}`}>
...
export function RecentlyViewedCall({ call, onSelect }: {
    call: LocalCall;
    onSelect: () => void;
}) {
    ...
    return (<RecentlyViewedContainer value={call.id} onSelect={onSelect} route={`/${scope}/calls/${call.id}`}>

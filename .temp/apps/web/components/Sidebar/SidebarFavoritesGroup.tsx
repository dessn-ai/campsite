...
import { ReorderDotsIcon, StarOutlineIcon } from "../../../../packages/ui/src/Icons/index.tsx";
...
import { SidebarFavorite } from "./SidebarFavorite.tsx";
import { SidebarGroup } from "./SidebarGroup.tsx";
import { useGetFavorites } from "../../hooks/useGetFavorites.ts";
import { useReorderFavorites } from "../../hooks/useReorderFavorites.ts";
import { useScopedStorage } from "../../hooks/useScopedStorage.ts";
import { isOptimisticFavorite } from "../../utils/optimisticFavorites.ts";
export function SidebarFavoritesGroup() {
    ...
    if (!hasFavorites) {
        ...
        {!collapsed && (<div className='text-quaternary flex items-center gap-2 p-2 pt-0.5'>
    ...
    const unreadAndSelectedItems = favorites.filter((fav) => {
        if (fav.project) {
            ...
            return (fav.message_thread.unread_count > 0 ||
    ...
    return (<SidebarGroup className='group/favorites'>
      <div className='flex items-center gap-px'>
        <SidebarCollapsibleButton collapsed={collapsed} setCollapsed={setCollapsed} label='Favorites'/>
      </div>

      <Reorder.Group ref={containerRef} axis='y' key={collapsed ? 'collapsed' : 'expanded'} values={sortedIds} onReorder={onReorder} className='flex flex-col gap-px' layoutScroll>
        {renderableItems.map((fav) => (<ConditionalWrap key={fav.id} condition={!hasOptimisticFavorite} wrap={(children) => (<Reorder.Item value={fav.id} id={fav.id} drag={!collapsed} layout='position' dragConstraints={containerRef} dragElastic={0.065} onDragStart={() => setDraggingId(fav.id)} onDragEnd={() => {

...
import { Badge, Button, CheckCircleFilledIcon, ChevronDownIcon, cn, InformationIcon, RotateIcon, UIText } from "../../../../packages/ui/src/index.tsx";
import { EMPTY_HTML } from "../../atoms/markdown.ts";
import { enumerateCommentElements } from "../Comments/CommentsList.tsx";
import { ConfirmDeleteResolutionDialog } from "./ConfirmDeleteResolutionDialog.tsx";
...
export function Resolution({ post, display, className }: Props) {
    ...
    return (<div className={cn('group flex w-full flex-col overflow-hidden rounded-lg border shadow-sm', {
      ...
      <div className='relative flex h-11 items-center gap-3 p-3 pr-2 text-left'>
        ...
        <span className='pointer-events-none flex flex-1 items-center gap-1.5'>
          {!expanded && previewHtml && (<>
              <HTMLRenderer text={previewHtml} className='line-clamp-1 flex-1 select-none text-sm opacity-40 max-lg:hidden'/>
            </>)}
        </span>
        <span className='flex gap-0.5'>
          {post.viewer_can_resolve && (<>
              <ConfirmDeleteResolutionDialog postId={post.id} open={confirmationDialogOpen} onOpenChange={setConfirmationDialogOpen}/>
              <Button variant='plain' className='text-quaternary hover:text-primary opacity-0 group-hover:opacity-100 max-lg:opacity-100' iconOnly={<RotateIcon />} onClick={handleDelete} accessibilityLabel='Reopen post' tooltip='Reopen post' tooltipShortcut={display === 'page' ? 'shift+r' : undefined}/>
            </>)}
          {html && display !== 'hovercard' && (<Button variant='plain' className='text-quaternary hover:text-primary hidden opacity-0 group-hover:opacity-100 lg:flex' iconOnly={<ChevronDownIcon className={cn('-rotate-180 transform transition-all duration-300', {
                    'rotate-0': expanded
                })}/>} onClick={() => setExpanded(!expanded)} accessibilityLabel={expanded ? 'Collapse' : 'Expand'}/>)}
        </span>
      </div>
      {expanded && html && (<>
          <div className='flex flex-col gap-3 px-3 pb-3'>
            <HTMLRenderer text={html} className='prose w-full max-w-full select-text focus:outline-none'/>
            {comment && display !== 'hovercard' && (<div>
...
export function CommentComposerResolutionBanner() {
    return (<span className='text-secondary relative left-px flex w-[calc(100%-2px)] items-center justify-center gap-1 rounded-t-lg bg-green-500 px-2 pb-3.5 pt-1 font-mono text-[11px] font-semibold uppercase tracking-wide text-white dark:bg-green-900/50 dark:text-green-400'>

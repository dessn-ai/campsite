...
import { PostViewersPopover } from "../Post/PostViewersPopover.tsx";
import { PAGE_COMMENTS_ID } from "../Post/PostView.tsx";
...
export function InlinePostEngagements({ display, post }: {
    display: DisplayType;
    post: Post;
}) {
    if (display === 'feed') {
...
function FeedPostEngagements({ post }: {
    post: Post;
}) {
    const viewCount = totalViews(post);
    const followUpCount = totalFollowUps(post);
    if (post.comments_count === 0 && viewCount === 0 && followUpCount === 0)
        return null;
    return (<>
...
function PreviewPostEngagements({ post }: {
    post: Post;
}) {
    const viewCount = totalViews(post);
    const followUpCount = totalFollowUps(post);
    if (post.comments_count === 0 && viewCount === 0)
        return null;
    return (<div className='flex items-center gap-1.5'>
      ...
      {(post.comments_count > 0 || viewCount > 0 || followUpCount > 0) && post.grouped_reactions.length > 0 && (<UIText quaternary>{' · '}</UIText>)}

      {post.grouped_reactions.length > 0 && <CompactPostReactions post={post}/>}
    </div>);
}
function CompactPostViewers({ post, side = 'top', align = 'start' }: {
    post: Post;
    ...
    if (viewCount === 0)
        return null;
    return (<PostViewersPopover modal side={side} align={align} post={post} display='viewers'>
      <button type='button' className='text-quaternary dark:text-tertiary flex cursor-pointer items-center gap-0.5 hover:underline'>
        <UIText inherit>
...
function CompactPostFollowUps({ post }: {
    post: Post;
}) {
    const followUpCount = totalFollowUps(post);
    if (followUpCount === 0 || !post.viewer_is_organization_member)
        return null;
    return (<PostViewersPopover modal side='top' align='start' display='follow-ups' post={post}>
      <button type='button' className='text-quaternary dark:text-tertiary flex cursor-pointer items-center gap-0.5 hover:underline'>
        <UIText inherit>
...
function CompactPostReactions({ post }: {
    post: Post;
}) {
    if (post.grouped_reactions.length === 0)
        return null;
    const reactionsCount = post.grouped_reactions.reduce((acc, reaction) => acc + reaction.reactions_count, 0);
    return (<PostLink postId={post.id} className='flex items-center gap-2 focus:ring-0'>
...
function PostCommenters({ post }: {
    ...
    if (post.comments_count === 0)
        return null;
    return (<PostLink hash={hasNew ? '#comments-end' : '#comments'} postId={post.id} className='flex items-center gap-2 focus:ring-0'>
      <FacePile users={users} limit={3} size='xs'/>
      <span className='text-quaternary dark:text-tertiary flex items-center gap-0.5 hover:underline'>
        <UIText inherit>
          {post.comments_count} {pluralize('comment', post.comments_count)}
        </UIText>
        {hasNew && (<span className='ml-1 rounded-full bg-blue-500 px-1.5 py-0.5 text-[9px] font-semibold uppercase text-white'>
            new
          </span>)}
      </span>
    </PostLink>);
...
function PagePostCommenters({ post }: {
    post: Post;
}) {
    if (post.comments_count === 0)
        return null;
    return (<button type='button' onClick={() => scrollCommentsIntoView('start')} className='flex items-center gap-2 focus:ring-0'>
      <span className='text-quaternary dark:text-tertiary flex items-center gap-0.5 hover:underline'>
        <UIText inherit>
          {post.comments_count} {pluralize('comment', post.comments_count)}
        </UIText>

import { Post, SyncCustomReaction } from "../../../../packages/types/index.ts";
import { Button, FaceSmilePlusIcon } from "../../../../packages/ui/src/index.tsx";
import { cn } from "../../../../packages/ui/src/utils/index.ts";
import { useCreatePostReaction } from "../../hooks/useCreatePostReaction.ts";
...
export function InlinePostReactions({ post }: InlinePostReactionsProps) {
    ...
    function getClasses(hasReacted: boolean) {
        return cn('flex gap-1.5 pointer-events-auto items-center p-1 pl-2 pr-2.5 justify-center group h-7.5 rounded-full text-xs font-semibold ring-1 min-w-[32px]', {
    ...
    if (!post)
        return null;
    return (<>
      <ReactionPicker custom={!!post.viewer_is_organization_member} onReactionSelect={handleReactionSelect} trigger={<Button round variant='plain' iconOnly={<FaceSmilePlusIcon size={24}/>} accessibilityLabel='Add reaction'/>}/>

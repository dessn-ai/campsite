...
import { AlarmCheckIcon, AlarmIcon, Button, ChatBubbleIcon, FeedbackRequestAltIcon, FeedbackRequestCompleteIcon, PaperAirplaneIcon } from "../../../../packages/ui/src/index.tsx";
import { cn } from "../../../../packages/ui/src/utils/index.ts";
import { commentComposerId } from "../Comments/CommentComposer.tsx";
import { DisplayType } from "./index.tsx";
import { InlinePostReactions } from "./InlinePostReactions.tsx";
import { PostSharePopover } from "../Post/PostSharePopover.tsx";
import { PostCommentComposer } from "../Comments/PostCommentComposer.tsx";
import { focusEditor } from "../MarkdownEditor/index.tsx";
import { InlinePostFollowUpDropdown } from "./InlinePostFollowUpDropdown.tsx";
...
export function InlinePostSocialActions({ post, display }: InlinePostSocialActionsProps) {
    ...
    if (display === 'page') {
        return (<div className='-ml-1 flex flex-wrap items-center gap-x-0.5 gap-y-1 py-2'>
    ...
    function focusCommentInput() {
        if (!inlineBehavior && !isCommenting)
            return;
    ...
    return (<div>
      <div className='flex flex-col'>
        <div className='-ml-1 flex flex-wrap items-center gap-x-0.5 gap-y-1'>
          {post.status === 'feedback_requested' && (<>
              {(post.viewer_feedback_status === 'none' && !post.viewer_is_author) ||
                (post.viewer_has_commented && !post.viewer_is_author) ? (<Button iconOnly={<FeedbackRequestCompleteIcon size={24}/>} variant={'plain'} round accessibilityLabel='Commented' {...commentButtonProps}/>) : post.viewer_is_author ? (<Button iconOnly={post.status === 'feedback_requested' ? (<FeedbackRequestAltIcon size={24}/>) : (<ChatBubbleIcon size={24}/>)} accessibilityLabel='Comment' variant='plain' round {...commentButtonProps}/>) : (<Button leftSlot={<FeedbackRequestAltIcon />} variant={viewerFeedbackRequested ? 'plain' : 'flat'} round className={cn({
                    'bg-brand-primary hover:bg-brand-secondary dark:hover:bg-brand-secondary text-white': viewerFeedbackRequested,
          ...
          {post.status === 'none' && (<Button iconOnly={<ChatBubbleIcon size={24}/>} accessibilityLabel='Comment' variant='plain' round {...commentButtonProps}/>)}

          {post.viewer_is_organization_member && (<PostSharePopover side='top' align='start' post={post} open={sharePopoverOpen} onOpenChange={setSharePopoverOpen} source='feed'>
          ...
          <InlinePostFollowUpDropdown post={post}>
            <Button iconOnly={viewerFollowUp ? <AlarmCheckIcon size={24}/> : <AlarmIcon size={24}/>} accessibilityLabel='Follow-up' variant='plain' round/>
        ...
        <AnimatePresence>
          {!inlineBehavior && isCommenting && (<m.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.1 }} className='cursor-auto' id={inlineReplyId}>
              <div className='pt-2'>

...
import { InlinePostActor } from "./InlinePostActor.tsx";
import { InlinePostContainer } from "./InlinePostContainer.tsx";
import { InlinePostEngagements } from "./InlinePostEngagements.tsx";
import { InlinePostSocialActions } from "./InlinePostSocialActions.tsx";
import { InlinePostTags } from "./InlinePostTags.tsx";
import { useLivePostUpdates } from "../../hooks/useLivePostUpdates.ts";
import { RichLinkCard } from "../RichLinkCard.tsx";
import { InlinePostByline } from "./InlinePostByline.tsx";
import { InlinePostContent } from "./InlinePostContent.tsx";
import { InlinePostPoll } from "./InlinePostPoll.tsx";
import { InlinePostTitle } from "./InlinePostTitle.tsx";
import { InlinePostVersionStatus } from "./InlinePostVersionStatus.tsx";
...
export const InlinePost = memo(function InlinePost({ post, display = 'feed', hideProject }: InlinePostProps) {
    ...
    return (<InlinePostContainer postId={post.id} display={display} className={cn({
      ...
      <InlinePostGrid display={display}>
        ...
        <InlinePostContentGrid display={display}>
          ...
          <GroupedAttachments postId={post.id} content={post.description_html} truncatedContent={post.truncated_description_html} attachments={post.attachments} display={display}/>

          {post.unfurled_link && (<div className='max-w-lg'>
...
export function InlinePostContentGrid({ display, children }: {
    display: DisplayType;
...
export function InlinePostGrid({ display, children }: {
    display: DisplayType;
    children: React.ReactNode;
}) {
    return (<div className={cn('grid-rows relative isolate grid', {

...
import { CopyIcon, LayeredHotkeys, Popover, PopoverAnchor, PopoverContent, PopoverPortal, ReplyIcon } from "../../../../packages/ui/src/index.tsx";
import { ANIMATION_CONSTANTS } from "../../../../packages/ui/src/utils/index.ts";
import { draftKey } from "../../atoms/markdown.ts";
import { quotableHtmlNode, quotableHtmlString, useTextSelection } from "../../hooks/useTextSelection.tsx";
import { getImmediateScrollableNode } from "../../utils/scroll.ts";
import { PostCommentComposer } from "../Comments/PostCommentComposer.tsx";
import { BubbleMenuButton } from "../EditorBubbleMenu/BubbleMenuButton.tsx";
...
export function AutoBlockquoteReply({ post, replyingToCommentId, enabled = true, className, children }: AutoBlockquoteReplyProps) {
...
function InnerAutoBlockquoteReply({ post, replyingToCommentId, element }: {
    post: Post;
    replyingToCommentId?: string;
    ...
    return (<>
      ...
      <Popover open={popoverContent === 'menu' && selectionRange !== null}>
        {anchor}
        <PopoverPortal>
          <PopoverContent side={isMobile ? 'bottom' : 'top'} align='center' sideOffset={15} collisionBoundary={closestScrollParent} forceMount onOpenAutoFocus={(e) => e.preventDefault()}>
            {selectionRange && (<div className='text-primary dark flex gap-1 rounded-lg bg-black p-1 shadow-lg dark:bg-neutral-700'>
                <BubbleMenuButton icon={<ReplyIcon />} tooltip='Reply' shortcut='R' onClick={() => onReply()} title='Reply'/>

                {!isMobile && (<BubbleMenuButton icon={<CopyIcon />} onClick={() => {
      ...
      <Popover modal open={popoverContent === 'composer'} onOpenChange={(open) => {
            if (!open) {
                closePopover();
            }
        }}>
        {anchor}
        {popoverContent === 'composer' && (<PopoverPortal>
            <PopoverContent align='center' collisionBoundary={closestScrollParent} forceMount>
              <m.div {...ANIMATION_CONSTANTS}>
                <div className='bg-elevated flex w-[500px] max-w-[--radix-popover-content-available-width] origin-[--radix-popover-content-transform-origin] flex-col rounded-xl shadow-lg shadow-black/20 ring-1 ring-black/[0.04] dark:shadow-[inset_0px_1px_0px_rgba(255,255,255,0.04),_0px_2px_12px_rgba(0,0,0,0.4),_0px_0px_0px_1px_rgba(0,0,0,0.8)] dark:ring-white/[0.02]' ref={composerRef}>
                  <PostCommentComposer autoFocus='end' display='inline' postId={post.id} replyingToCommentId={replyingToCommentId} onSubmitting={() => {
                closePopover();
                toast('Comment posted');
            }} closeComposer={closePopover} draftKeyOverride={draftKey({
                postId: post.id,
                replyingToCommentId
            }) + selectionHtml.replace(/\s/g, '.')} maxHeight='120px' initialValues={{

...
import { FaceSmilePlusIcon, Tooltip } from "../../../packages/ui/src/index.tsx";
import { cn, ConditionalWrap } from "../../../packages/ui/src/utils/index.ts";
import { getExpiration } from "../hooks/useCreateStatus.ts";
...
export function MemberStatus({ status, disabled, asTrigger, size = 'base' }: StatusProps) {
    ...
    return (<ConditionalWrap condition={!disabled} wrap={(c) => (<Tooltip disableHoverableContent sideOffset={8} label={<span className='flex items-center gap-1.5'>
...
export function MemberStatusTimeRemaining({ status }: {
    status?: OrganizationMember['status'];
}) {

...
import { GetMembersMeForMePostsParams, GetMembersMeViewerPostsParams, GetPostsParams, PostPage } from "../../../../packages/types/index.ts";
import { Button } from "../../../../packages/ui/src/Button/index.tsx";
import { LayeredHotkeys } from "../../../../packages/ui/src/DismissibleLayer/index.tsx";
import { ChevronDownIcon, ChevronUpIcon } from "../../../../packages/ui/src/Icons/index.tsx";
import { useScope } from "../../contexts/scope.tsx";
import { useGetCurrentMemberPosts } from "../../hooks/useGetCurrentMemberPosts.ts";
import { useGetForMePosts } from "../../hooks/useGetForMePosts.ts";
import { useGetMemberPosts } from "../../hooks/useGetMemberPosts.ts";
import { useGetPosts } from "../../hooks/useGetPosts.ts";
import { useGetProjectPosts } from "../../hooks/useGetProjectPosts.ts";
import { useGetTagPosts } from "../../hooks/useGetTagPosts.ts";
import { atomWithWebStorage } from "../../utils/atomWithWebStorage.ts";
import { CommandListSubject, getCommandListSubject } from "../../utils/commandListSubject.ts";
import { flattenInfiniteData } from "../../utils/flattenInfiniteData.ts";
type LastUsedPostFeed = undefined | {
    type: 'member';
    username: string;
} | {
    type: 'project';
...
const lastUsedPostFeedAtom = atomWithWebStorage<LastUsedPostFeed>('lastUsedPostFeed', undefined, 
/**
...
function PostNavigationButtonsImpl({ postId, getPosts }: {
    postId: string;
    ...
    const handleNavigateUp = () => () => {
        if (!hasPreviousPost || !posts || currentPostIndex === -1)
            return;
    ...
    const handleNavigateDown = () => () => {
        if (!hasNextPost || !posts || currentPostIndex === -1)
            return;
    ...
    return (<div className='flex items-center gap-1'>
      ...
      <Button variant='plain' iconOnly={<ChevronUpIcon />} accessibilityLabel='Go to previous post' tooltip='Navigate up' tooltipShortcut='k' onClick={handleNavigateUp()} disabled={!hasPreviousPost}/>
      <Button variant='plain' iconOnly={<ChevronDownIcon />} accessibilityLabel='Go to next post' tooltip='Navigate down' tooltipShortcut='j' onClick={handleNavigateDown()} disabled={!hasNextPost}/>

      <div className='mx-1 h-6 w-[1px] border-r'/>
    </div>);
}
function MemberFeedNavigationButtons({ postId, username }: {
    postId: string;
    username: string;
}) {
...
function ProjectFeedNavigationButtons({ postId, projectId }: {
    postId: string;
    projectId: string;
}) {
...
function TagFeedNavigationButtons({ postId, tagName }: {
    postId: string;
    tagName: string;
}) {
...
function CreatedFeedNavigationButtons({ postId, order }: {
    postId: string;
    order: GetMembersMeViewerPostsParams['order'];
}) {
...
function ForMeFeedNavigationButtons({ postId, order }: {
    postId: string;
    order: GetMembersMeForMePostsParams['order'];
}) {
...
function AllFeedNavigationButtons({ postId, order }: {
    postId: string;
    order: GetPostsParams['order'];
}) {
...
export function PostNavigationButtons({ postId }: {
    postId: string;
}) {
    const router = useRouter();

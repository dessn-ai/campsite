...
import { AlarmCheckIcon, AlarmIcon, BellCheckIcon, BellIcon, Button, ContextMenu, CopyIcon, DotsHorizontal, LinearIcon, LinkIcon, LoadingSpinner, PaperAirplaneIcon, PencilIcon, PinTackFilledIcon, PinTackIcon, ProjectIcon, RefreshIcon, ResolvePostIcon, RotateIcon, SmartSummaryIcon, StarFilledIcon, StarOutlineIcon, TrashIcon } from "../../../../packages/ui/src/index.tsx";
...
import { DeletePostDialog } from "./DeletePostDialog.tsx";
import { PostFollowUpDialog } from "./PostFollowUpDialog.tsx";
import { PostMoveProjectDialog } from "./PostMoveProjectDialog.tsx";
import { PostShareDialog } from "./PostShareDialog.tsx";
...
import { useCreatePostPin } from "../../hooks/useCreatePostPin.ts";
import { useCreatePostSubscription } from "../../hooks/useCreatePostSubscription.ts";
import { useDeletePostFavorite } from "../../hooks/useDeletePostFavorite.ts";
import { useDeletePostPin } from "../../hooks/useDeletePostPin.ts";
import { useDeletePostSubscription } from "../../hooks/useDeletePostSubscription.ts";
import { useGetLinearIntegration } from "../../hooks/useGetLinearIntegration.ts";
import { useGetPost } from "../../hooks/useGetPost.ts";
import { useGetPostVersions } from "../../hooks/useGetPostVersions.ts";
...
export function PostOverflowMenu({ post, type, children, onTldrOpen, align = 'start' }: PostOverflowMenuProps) {
    ...
    const items = buildMenuItems([
        ...
        },
        {
            ...
            onSelect: () => {
                if (post.project_pin_id) {
                    removePin.mutate({ projectId: post.project.id, pinId: post.project_pin_id, postId: post.id }, { onSuccess: () => toast('Unpinned post') });
                }
                else {
        ...
        showNewVersionAction && {
            type: 'item',
            leftSlot: isLoadingVersions ? (<div className='flex h-5 w-5 items-center justify-center'>
          <LoadingSpinner />
        </div>) : (<RefreshIcon />),
            label: 'New version',
            disabled: !latestPost,
    ...
    return (<>
      ...
      {type === 'context' ? (<ContextMenu asChild items={items} onOpenChange={setDropdownIsOpen}>
          {children}
        </ContextMenu>) : (<DropdownMenu open={dropdownIsOpen} onOpenChange={setDropdownIsOpen} items={items} align={align} trigger={children ?? (<Button variant='plain' iconOnly={<DotsHorizontal />} accessibilityLabel='Post actions dropdown'/>)}/>)}
    </>);
}

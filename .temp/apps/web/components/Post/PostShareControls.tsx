...
import { Post, SlackChannel } from "../../../../packages/types/index.ts";
import { ArrowRightCircleIcon, Avatar, Button, CheckIcon, GlobeIcon, InformationIcon, LinkIcon, LockIcon, PencilIcon, ProjectIcon, Switch, Tooltip, UIText } from "../../../../packages/ui/src/index.tsx";
import { useCopyToClipboard } from "../../../../packages/ui/src/hooks/index.tsx";
import { cn } from "../../../../packages/ui/src/utils/index.ts";
import { ConnectSlackButton } from "../OrgSettings/ConnectSlackButton.tsx";
import SlackChannelPicker from "../OrgSettings/SlackChannelPicker.tsx";
import { PostMoveProjectDialog } from "./PostMoveProjectDialog.tsx";
import { useCreatePostShare } from "../../hooks/useCreatePostShare.ts";
import { useGetCurrentOrganization } from "../../hooks/useGetCurrentOrganization.ts";
import { useGetProject } from "../../hooks/useGetProject.ts";
import { useGetSlackIntegration } from "../../hooks/useGetSlackIntegration.ts";
import { useIsOrganizationMember } from "../../hooks/useIsOrganizationMember.ts";
import { useSlackBroadcastsAuthorizationUrl } from "../../hooks/useSlackBroadcastsAuthorizationUrl.ts";
import { useUpdatePostVisibility } from "../../hooks/useUpdatePostVisibility.ts";
...
export function PostShareControls({ isOpen, post }: PostShareControlsProps) {
    ...
    return (<>
      {canMoveProject && (<PostMoveProjectDialog post={post} open={moveProjectDialogIsOpen} onOpenChange={setMoveProjectDialogIsOpen}/>)}
      <div ref={inViewRef} className='flex flex-col gap-4 p-4'>
        ...
        <div className='flex items-center justify-between gap-2'>
          <span className='flex min-w-0 shrink items-center gap-2'>
            <span className='text-secondary flex h-5 w-5 items-center justify-center'>
              {post.project.accessory ? (<UIText className='font-["emoji"]' size='text-base' inherit>
            ...
            <span title={post.project.name} className='flex min-w-0 shrink items-center gap-1 whitespace-nowrap'>
              ...
              {post.project.private && (<Tooltip label='Private'>
                  <span>
                    <LockIcon className='text-tertiary flex-none' size={16}/>
                  </span>
                </Tooltip>)}
              {canMoveProject && (<Tooltip label='Move to channel...'>
                  <button 
        // this is the first focusable element in the popover; without this, the tooltip would be open by default
        tabIndex={-1} className='text-tertiary hover:text-primary' onClick={() => setMoveProjectDialogIsOpen(true)}>
        ...
        <div>
          <Button variant='primary' fullWidth tooltipShortcut='mod+shift+c' onClick={onCopy} leftSlot={isCopied ? <CheckIcon /> : <LinkIcon />} className={cn({
        ...
        {(!post.project.private || post.visibility === 'public') && (<div className='flex flex-col gap-2'>
            {hasIntegrationWithScopes ? (<form onSubmit={handleSlackSubmit} className='flex items-center gap-1'>
                <div className='flex-1'>
                  <SlackChannelPicker activeId={slackChannel?.id} onChange={setSlackChannel} includeSlackIcon/>
                </div>
                <button type='submit' className={cn('-mr-2', {
                    'dark:text-tertiary text-gray-400': !slackChannel,
                    'text-blue-500 dark:text-blue-500': slackChannel
                })} aria-label='Send' disabled={!slackChannel}>
                  <ArrowRightCircleIcon size={36}/>
                </button>
              </form>) : (<ConnectSlackButton href={slackBroadcastsAuthorizationUrl}/>)}

            {post.project.private && post.visibility === 'public' && (<div className='-mx-4 -mb-4 mt-2 flex items-start gap-2 rounded-b-lg border-t border-yellow-300 bg-yellow-100 p-4 text-yellow-800 dark:border-yellow-900/70 dark:bg-yellow-800/30 dark:text-yellow-400'>

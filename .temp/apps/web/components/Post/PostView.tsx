...
import { Comment, Post, TimelineEvent, User } from "../../../../packages/types/index.ts";
import { Button, CheckCircleIcon, LayeredHotkeys, PaperAirplaneIcon, PostIcon, RotateIcon, useBreakpoint } from "../../../../packages/ui/src/index.tsx";
...
import { ConfirmDeleteResolutionDialog } from "../InlinePost/ConfirmDeleteResolutionDialog.tsx";
import { CommentComposerResolutionBanner } from "../InlinePost/Resolution.tsx";
import { ResolutionHovercard } from "../InlinePost/ResolutionHovercard.tsx";
import { PostBreadcrumbs } from "./PostBreadcrumbs.tsx";
import { PostInlineIssues } from "./PostInlineIssues.tsx";
import { PostInlineReferences } from "./PostInlineReferences.tsx";
import { PostNavigationButtons } from "./PostNavigationButtons.tsx";
import { PostOverflowMenu } from "./PostOverflowMenu.tsx";
import { PostSharePopover } from "./PostSharePopover.tsx";
import { PostViewersFacepilePopover } from "./PostViewersFacepilePopover.tsx";
import { ResolveDialog } from "./ResolveDialog.tsx";
import { PostInlineSummary } from "./TLDR.tsx";
import { UnseenCommentsButton } from "./UnseenCommentsButton.tsx";
import { useCreateActivePostView } from "./useCreateActivePostView.ts";
import { useUnseenCommentScrollSync } from "./useUnseenCommentsScrollSync.tsx";
...
import { useGetPostComments } from "../../hooks/useGetPostComments.ts";
import { useGetPostTimelineEvents } from "../../hooks/useGetPostTimelineEvents.ts";
...
import { getImmediateScrollableNode, scrollImmediateScrollableNodeToBottom } from "../../utils/scroll.ts";
import { CommentListHeader } from "../Comments/CommentListHeader.tsx";
import { CommentsList } from "../Comments/CommentsList.tsx";
import { PostCommentComposer } from "../Comments/PostCommentComposer.tsx";
import { FullPageError } from "../Error/index.tsx";
import { InlinePost } from "../InlinePost/index.tsx";
import { ReplyContent } from "../ReplyContent/index.tsx";
...
export function PostView(props: {
    ...
    const { data: post, isSuccess, isPending, isFetching, error } = useGetPost({ postId, refetchOnWindowFocus: true, fetchIfStale: true });
    ...
    if (error) {
        if (currentUser?.logged_in) {
            if (error.message === 'Record not found.') {
                return (<EmptyState icon={<PostIcon size={48} className='text-quaternary'/>} title='Post not found' message='The post you are looking for does not exist or was deleted.'>
            <div className='mt-4'>
    ...
    if (isSuccess && !post) {
        return (<EmptyState icon={<PostIcon />} title='Post not found' message='The post you are looking for does not exist or has been deleted.'/>);
    }
    if (!post) {
        return (<EmptyState icon={<PostIcon size={48} className='text-quaternary'/>} title='Post not found' message='The post you are looking for does not exist or has been deleted.'>
        <div className='mt-4'>
          <Button onClick={() => Router.push(`/${scope}`)} variant='primary'>
            Go home
          </Button>
    ...
    return (<PostViewContext.Provider value={postId}>
      <SubjectEspcapeLayeredHotkeys />
      <CopyCurrentUrl override={post?.url}/>

      <InnerPostView key={postId} post={post} isFetching={isFetching} getComments={getComments} timelineEvents={timelineEvents}/>
...
function InnerPostViewContent({ post, getComments, timelineEvents }: Omit<Props, 'isFetching'>) {
    ...
    return (<>
      <LayeredHotkeys keys='shift+r' options={{ enabled: post.viewer_can_resolve }} callback={() => {
            if (post.resolution) {
      ...
      {currentUser?.logged_in && (<>
          ...
          <ConfirmDeleteResolutionDialog postId={post.id} open={confirmDeleteResolutionDialogIsOpen} onOpenChange={setConfirmDeleteResolutionDialogIsOpen}/>
        </>)}
      ...
      <div ref={trackRef} className='bg-secondary dark:bg-primary flex min-w-0 flex-1 flex-col overflow-hidden'>
        ...
        <ScrollableContainer disableScrollRestoration className='relative flex-initial flex-col'>
          ...
          <div className='bg-primary'>
            <div className={cn('relative mx-auto flex w-full max-w-[--post-width] flex-col', 'px-4 pb-4 pt-6 md:pt-8 lg:pt-10 xl:pt-12 2xl:pt-14')}>
              <InlinePost post={post} display='page' 
    ...
    hideProject={!isMobile}/>
              ...
              {post.viewer_is_organization_member && (<div className='-ml-1 flex flex-col gap-4 py-4'>
          ...
          <div className='bg-secondary dark:bg-primary border-t'>
            <div className='relative mx-auto flex w-full max-w-[--post-width] flex-col px-4'>
              <Comments postId={post.id} replyingToCommentId={replyingToCommentId} setReplyingToCommentId={setReplyingToCommentId} getComments={getComments} timelineEvents={timelineEvents} comments={comments}/>
            </div>
          </div>
        ...
        <ReplyComposer post={post} endOfCommentsRef={endOfCommentsRef} endOfCommentsInView={endOfCommentsInView} replyingToComment={replyingToComment} setReplyingToCommentId={setReplyingToCommentId} unseenComments={unseenComments} unseenCommentUsers={unseenCommentUsers}/>
      </div>
    </>);
}
function PostViewTitlebar({ post }: {
    post: Post;
}) {
    ...
    return (<>
      ...
      <InboxSplitViewTitleBar hideSidebarToggle={isSplitViewAvailable}>
        ...
        {post.viewer_is_organization_member && (<div className='flex items-center gap-1.5'>
            <PostViewersFacepilePopover post={post}/>
            <PostSharePopover side='bottom' align='end' post={post} open={sharePopoverOpen} onOpenChange={setSharePopoverOpen} source='feed'>
              <Button leftSlot={<PaperAirplaneIcon />} variant='plain'>
                Share
              </Button>
            </PostSharePopover>
            <Button leftSlot={post.resolution ? <RotateIcon /> : <CheckCircleIcon />} variant='plain' onClick={handlePostResolution}>
...
function Comments({ postId, replyingToCommentId, setReplyingToCommentId, getComments, timelineEvents, comments }: CommentsProps) {
    const { data: post } = useGetPost({ postId });
    const { isFetching, isFetchingNextPage, hasNextPage, fetchNextPage } = getComments;
    if (!post)
        return null;
    return (<div id={PAGE_COMMENTS_ID} className='relative flex w-full flex-1 flex-col gap-4 pt-4 transition-all'>
      ...
      <CommentsList post={post} comments={comments} timelineEvents={timelineEvents} replyingToCommentId={replyingToCommentId} setReplyingToCommentId={setReplyingToCommentId}/>
    </div>);
}
...
function ReplyComposer({ post, endOfCommentsRef, endOfCommentsInView, replyingToComment, setReplyingToCommentId, unseenComments, unseenCommentUsers }: ReplyComposerProps) {
    const canReplyInline = useBreakpoint('md');
    const isReplyingInStickyEditor = !!replyingToComment && !canReplyInline;
    return (<div>
      <div className={cn('h-px w-full shrink-0 border-t', {
      ...
      <div className='pb-safe-offset-3 sticky bottom-0 mx-auto flex w-full max-w-[--post-width] flex-1 flex-col gap-3 p-4'>
        {isReplyingInStickyEditor && (<div className='w-full'>
            <ReplyContent attachments={replyingToComment.attachments} author={replyingToComment.member.user} content={replyingToComment.body_html} onCancel={() => setReplyingToCommentId(null)}/>
          </div>)}

        <UnseenCommentsButton comments={unseenComments} users={unseenCommentUsers} onScrollToBottom={() => {
            scrollImmediateScrollableNodeToBottom(endOfCommentsRef.current);
        }}/>

        <div className='flex flex-1 flex-col gap-3'>
          {post.resolution && (<ResolutionHovercard post={post} side='top' align='center'>
              <span className='-mb-6'>
                <CommentComposerResolutionBanner />
              </span>
            </ResolutionHovercard>)}

          <PostCommentComposer postId={post.id} maxHeight='40vh' onCreated={(comment) => {
        ...
        }} placeholder={isReplyingInStickyEditor ? 'Write a reply...' : 'Write a comment...'} replyingToCommentId={isReplyingInStickyEditor ? replyingToComment?.id : undefined}/>
        </div>
      </div>
    </div>);
}

...
import { AtSignIcon, CheckSquareIcon, CodeIcon, FaceSmileIcon, Heading1Icon, Heading2Icon, Heading3Icon, HorizontalRuleIcon, KeyboardShortcut, OrderedListIcon, PlayIcon, PostIcon, QuoteIcon, TextAlignLeftIcon, UIText, UnorderedListIcon, UploadCloudIcon } from "../../../../../packages/ui/src/index.tsx";
...
export function SlashCommand({ editor, upload }: Props) {
    return (<SuggestionRoot editor={editor} char='/' pluginKey={pluginKey}>
      {COMMANDS.map((item) => {
            return (<SuggestionItem editor={editor} value={item.title} keywords={item.searchTerms} key={item.title} onSelect={({ editor, range }) => {

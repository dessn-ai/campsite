...
import { User } from "../../../../../packages/types/index.ts";
import { ANIMATION_CONSTANTS, Popover, PopoverContent, PopoverElementAnchor, PopoverPortal, RelativeTime, UIText } from "../../../../../packages/ui/src/index.tsx";
...
function CommentPreviewPopoverContent({ previewComment, editor, noteId, onExpand }: CommentPreviewPopoverContentProps) {
    ...
    if (!comment)
        return null;
    return (<>
      <PopoverElementAnchor element={previewComment && commentElement(previewComment.id, false, editor?.view.dom)} asChild/>
      <PopoverPortal forceMount>
        <PopoverContent asChild avoidCollisions side='top' align='center' sideOffset={8} onOpenAutoFocus={(evt) => evt.preventDefault()} onCloseAutoFocus={(evt) => evt.preventDefault()}>
          <m.div className={cn('dark:bg-elevated relative flex min-w-[32px] max-w-[250px] origin-[--radix-popover-content-transform-origin] gap-2 overflow-hidden rounded-xl bg-white p-1.5 shadow-md ring-1 ring-black/5 transition-shadow hover:shadow-lg dark:shadow-[inset_0px_1px_0px_rgba(255,255,255,0.10),_0px_2px_4px_rgba(0,0,0,0.5),_0px_0px_0px_1px_rgba(0,0,0,1)]', {
        ...
        animate: {
            ...
            <div className='flex flex-col'>
              ...
              {comment.body_html && (<div className='line-clamp-1 opacity-60'>
                  <CommentDescription subjectId={noteId} subjectType='Note' comment={comment} isEditing={false} isReply={false} setIsEditing={() => {
                return;
            }}/>
                </div>)}
            </div>

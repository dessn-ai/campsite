...
import { ActiveEditorComment, BlurAtTopOptions, getNoteExtensions, PostNoteAttachmentOptions } from "../../../../../packages/editor/src/index.ts";
...
import { MediaGalleryRenderer } from "../MediaGalleryRenderer.tsx";
...
import { LinkUnfurlRenderer } from "../LinkUnfurlRenderer.tsx";
import { NoteAttachmentRenderer } from "./Attachments/NoteAttachmentRenderer.tsx";
import { useUploadNoteAttachments } from "./Attachments/useUploadAttachments.ts";
...
export function useNoteEditor({ content, autofocus, editable, editorProps, onHoverComment, onActiveComment, onOpenAttachment, onBlurAtTop, provider }: NoteEditorOptions) {
    ...
    const editor = useEditor({
        ...
        editorProps: {
            attributes: {
                class: 'new-posts prose select-text focus:outline-none w-full relative note min-w-full px-[calc((100%-44rem)/2)]',
    ...
    useEffect(() => {
        if (!editor.commands.updateUser || !currentUser)
            return;

...
import { ONE_GB } from "../../../../../../packages/config/src/index.ts";
import { InlineAttachmentAttributes } from "../../../../../../packages/editor/src/extensions/index.ts";
import { Attachment } from "../../../../../../packages/types/index.ts";
import { MEDIA_GALLERY_VALIDATORS } from "../../utils.ts";
import { useScope } from "../../../../contexts/scope.tsx";
import { useCreateAttachment } from "../../../../hooks/useCreateAttachment.ts";
import { useCreateNoteAttachment } from "../../../../hooks/useCreateNoteAttachment.ts";
import { useGetCurrentOrganization } from "../../../../hooks/useGetCurrentOrganization.ts";
import { createFileUploadPipeline } from "../../../../utils/createFileUploadPipeline.ts";
import { apiClient, getTypedQueryData, setTypedQueryData } from "../../../../utils/queryClient.ts";
...
export function setOptimisticAttachment({ queryClient, scope, value }: {
    queryClient: QueryClient;
    scope: ReturnType<typeof useScope>['scope'];
    value: Attachment;
}) {
...
export function useUploadNoteAttachments({ noteId, enabled = true }: Props) {
    ...
    return useCallback(async ({ files, ...props }: UploadProps) => {
        if (!enabled)
            return;
        const isMultipleFiles = files.length > 1;
        const payloadIsValidGallery = files.every((file) => MEDIA_GALLERY_VALIDATORS.some((validator) => validator(file.type)));
        ...
        const pipeline = createFileUploadPipeline({
            ...
            maxFileSize,
            scope: `${scope}`,
            onFilesExceedMaxSize: () => toast.error(`File size must be less than ${Math.floor(maxFileSize / 1024 / 1024)}mb`),

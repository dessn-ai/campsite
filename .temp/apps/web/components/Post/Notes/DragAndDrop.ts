...
function nodeDOMAtCoords(coords: {
    ...
    return document.elementsFromPoint(coords.x, coords.y).find((elem: Element) => (elem.parentElement?.matches?.('.ProseMirror') && !elem.matches('ul, ol')) ||
...
function DragHandle(options: DragHandleOptions) {
    ...
    function handleClick(event: MouseEvent, view: EditorView) {
        ...
        if (pmNode && (pmNode.isText || pmNode.type.name === 'paragraph')) {
            view.dispatch(view.state.tr.setSelection(TextSelection.create(view.state.doc, nodePos, nodePos + pmNode.nodeSize)));
        }
        else {
            view.dispatch(view.state.tr.setSelection(NodeSelection.create(view.state.doc, nodePos)));
        }

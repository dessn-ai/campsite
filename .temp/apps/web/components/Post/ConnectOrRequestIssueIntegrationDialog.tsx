...
import { useLinearAuthorizationUrl } from "../../hooks/useLinearAuthorizationUrl.ts";
export function ConnectOrRequestIssueIntegrationDialog({ open, onOpenChange }: {
    open: boolean;
      ...
      <Dialog.Content className='flex flex-col p-0'>
        <div className='border-b p-3 pt-0'>
          <div className='flex w-full items-center gap-2'>
            ...
            <Button className='ml-auto' variant='primary' href={callbackUrl} externalLink allowOpener onClick={(e) => {
        ...
        <div className='bg-secondary flex flex-col gap-2 p-3'>
          ...
          {apps.map((app) => (<div key={app.name} className='flex w-full items-center gap-2'>
              <Image src={app.icon} width='36' height='36' alt={`${app.name} app icon`} role='presentation' aria-hidden/>
              <UIText weight='font-medium'>{app.name}</UIText>
              <Button className='ml-auto' variant='flat' onClick={() => {
                setFeedbackValue(`Integration request: ${app.name}\n\nShare more details about your toolkit and workflow...`);

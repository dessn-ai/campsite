...
import { OrganizationMember, Post } from "../../../../packages/types/index.ts";
import { CONTAINER_STYLES, GlobeIcon, Link, Popover, PopoverContent, PopoverPortal, PopoverTrigger, shortTimestamp, UIText } from "../../../../packages/ui/src/index.tsx";
...
import { useGetPostViews } from "../../hooks/useGetPostViews.ts";
import { flattenInfiniteData } from "../../utils/flattenInfiniteData.ts";
import { FollowUps } from "../FollowUp/index.ts";
...
export function PostViewersPopover({ post, children, side = 'bottom', align = 'end', modal = false, display = 'all' }: Props) {
    ...
    return (<Popover open={open} onOpenChange={setOpen} modal={modal}>
      ...
      <PopoverPortal>
        <PopoverContent side={side} align={align} sideOffset={8} onCloseAutoFocus={(event) => event.preventDefault()} className={cn('w-[320px]', CONTAINER_STYLES.base, CONTAINER_STYLES.shadows, 'bg-elevated rounded-lg dark:border dark:bg-clip-border')}>
          <div className='scrollbar-hide flex max-h-[400px] flex-col gap-0.5 overflow-y-scroll'>
            {(display === 'all' || display === 'follow-ups') && (<FollowUps followUps={post.follow_ups} showBorder={display !== 'follow-ups'}/>)}
...
function Views({ post }: {
    ...
    const { data: viewsData, hasNextPage, isError, isLoading, isFetching, isFetchingNextPage, fetchNextPage } = useGetPostViews({ postId: post.id });
    const views = useMemo(() => flattenInfiniteData(viewsData), [viewsData]);
    const { non_member_views_count } = post;
    const nonMemberViewersDescriptor = non_member_views_count > 0
        ? `${non_member_views_count} anonymous ${pluralize('view', non_member_views_count)}`
        : null;
    if (isLoading)
        return null;
    if (!post.views_count && !post.non_member_views_count)
        return null;
    return (<div className='p-1.5'>
      ...
      {post.non_member_views_count > 0 && (<div className='flex items-center gap-2 px-2 py-1.5'>
      ...
      <InfiniteLoader hasNextPage={!!hasNextPage} isError={!!isError} isFetching={!!isFetching} isFetchingNextPage={!!isFetchingNextPage} fetchNextPage={fetchNextPage}/>
    </div>);
}
export function ViewLink({ member, time }: {
    member: OrganizationMember;
    time?: string;
}) {
    const { scope } = useScope();
    return (<Link href={`/${scope}/people/${member.user.username}`}>
      <div className='hover:bg-tertiary flex items-center gap-2 rounded-md px-2 py-1.5'>
        ...
        <div className='flex flex-col'>
          ...
          {time && (<UIText size='text-xs' quaternary>

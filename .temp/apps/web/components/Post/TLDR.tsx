...
import { PlayIcon, WarningTriangleIcon } from "../../../../packages/ui/src/Icons/index.tsx";
import { LayeredHotkeys } from "../../../../packages/ui/src/index.tsx";
import { LoadingSpinner } from "../../../../packages/ui/src/Spinner/index.tsx";
...
import { useGetGeneratedTldr } from "../../hooks/useGetGeneratedTldr.ts";
...
export function TLDR({ open, postId, className, source }: Props) {
    ...
    return (<AnimatePresence initial={false}>
      {open && (<m.div id={id} initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} transition={{ duration: ANIMATION_DURATION / 1000 }} className={cn('overflow-hidden dark:shadow-[inset_0px_1px_0px_rgb(255_255_255_/_0.04),_inset_0px_0px_0px_1px_rgb(255_255_255_/_0.02),_0px_1px_2px_rgb(0_0_0_/_0.4),_0px_2px_4px_rgb(0_0_0_/_0.08),_0px_0px_0px_0.5px_rgb(0_0_0_/_0.24)]', className)}>
          {isPending && <TLDRLoading />}
          {isFailed && <TLDRError />}
          {isSuccess && !!getGeneratedTldr.data.html && (<PostTldrContent postId={postId} content={getGeneratedTldr.data.html} source={source}/>)}
          {isSuccess && !getGeneratedTldr.data.html && <TLDRNotEligible />}

          {hasResponse && !!getGeneratedTldr.data.response_id && (<div className='flex justify-center border-t px-3 py-1.5'>
...
function TLDRLoading() {
    return (<div className='text-tertiary flex items-center justify-center gap-2 p-5'>
      <LoadingSpinner />
...
function PostTldrContent({ content }: {
    postId: string;
    content: string;
    source: string;
}) {
    ...
    return (<div ref={ref} className='prose select-text px-3 py-4 focus:outline-none has-[ul]:pt-2'>
...
export function PostInlineSummary({ postId, source }: {
    postId: string;
    source: string;
}) {
    ...
    const isSuccess = getGeneratedTldr.isSuccess && getGeneratedTldr.data?.status === 'success';
    const hasResponse = isSuccess && !!getGeneratedTldr.data.html;
    return (<AccordionPrimitive.Root type='single' collapsible className='group flex flex-col' value={value} onValueChange={setValue}>
      <LayeredHotkeys keys='shift+s' options={{ preventDefault: true }} callback={() => {
            if (open) {
                setValue('');
            }
            else {
            ...
            if (!open && !inView) {
                queueMicrotask(() => document.getElementById(SUMMARY_ACCORDION_HEADER_ID)?.scrollIntoView({ behavior: 'smooth' }));
            }
        }}/>

...
import { Button, Checkbox, UIText } from "../../../../packages/ui/src/index.tsx";
...
export function DeletePostDialog({ post, open, onOpenChange }: Props) {
    ...
    function handleDelete() {
        if (post.has_iterations && !accepted)
            return;
    ...
    return (<Dialog.Root open={open} onOpenChange={onOpenChange} size='lg'>
      ...
      {post.has_iterations && (<Dialog.Content>
      ...
      <Dialog.Footer>
        <Dialog.TrailingActions>
          ...
          <Button variant='destructive' onClick={handleDelete} disabled={deletePostMutation.isPending || (!accepted && post.has_iterations)} loading={deletePostMutation.isPending} autoFocus>
            Delete post
          </Button>
        </Dialog.TrailingActions>
      </Dialog.Footer>

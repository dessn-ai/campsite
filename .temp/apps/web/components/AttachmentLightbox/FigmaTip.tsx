...
import { Button, CloseIcon, UIText } from "../../../../packages/ui/src/index.tsx";
import { useIsDesktopApp } from "../../../../packages/ui/src/hooks/index.tsx";
import { useFigmaEmbedLoaded } from "../../hooks/useFigmaEmbedLoaded.ts";
import { useFigmaEmbedSelected } from "../../hooks/useFigmaEmbedSelected.ts";
import { embedType } from "../Post/PostEmbeds/transformUrl.ts";
interface Props {
...
export function FigmaTip({ attachment }: Props) {
    ...
    const isFigmaAttachment = embedType(attachment.url) === 'figma' || (attachment.image && attachment.remote_figma_url && figmaEmbedSelected);
    ...
    return (<AnimatePresence>
      {showFigmaTip && (<m.div initial={{ opacity: 0, scale: 0.94 }} animate={{
                opacity: 1,
                scale: 1,
                transition: {
                    duration: 0.1
          ...
          <div className='flex flex-1 items-center gap-3'>
            ...
            {isDesktop && (<Button externalLink fullWidth variant='primary' href={`${attachment.app_url}?browser=true`}>
                Open in browser
              </Button>)}
          </div>
          <Button onClick={() => setShowFigmaTip(false)} variant='plain' className='absolute right-2 top-2' iconOnly={<CloseIcon />} accessibilityLabel='Close'/>
        </m.div>)}
    </AnimatePresence>);
}

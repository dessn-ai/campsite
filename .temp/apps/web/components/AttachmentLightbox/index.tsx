...
import { Attachment, Message, Note, Post } from "../../../../packages/types/index.ts";
import { Button, ChatBubbleIcon, CloseIcon } from "../../../../packages/ui/src/index.tsx";
...
import { displayCanvasCommentsAtom, newCommentCoordinatesAtom } from "../CanvasComments/CanvasComments.tsx";
import { zoomAtom } from "../ZoomPane/atom.ts";
import { FigmaTip } from "./FigmaTip.tsx";
import { FigmaToggleButton } from "./FigmaToggleButton.tsx";
import { FileMenu } from "./FileDropdown.tsx";
import { Gallery } from "./Gallery.tsx";
import { LightboxAttachmentRenderer } from "./LightboxAttachmentRenderer.tsx";
import { NoteLightboxComments } from "./NoteAttachmentLightboxComments.tsx";
import { ToggleCommentsButton } from "./ToggleCommentsButton.tsx";
import { ZoomSelect } from "./ZoomSelect.tsx";
export const filterLightboxableAttachments = (attachments: Attachment[], galleryId?: string) => attachments.filter((a) => {
    if (a.principle || a.origami || a.stitch)
        return false;
    if (galleryId && a.gallery_id !== galleryId)
        return false;
    return true;
});
interface Props {
...
export function AttachmentLightbox(props: Props) {
    ...
    return (<Dialog.Root open={open} onOpenChange={(open) => {
        ...
        }} size='cover' portalContainer={portalContainer} onPointerDownOutside={(e) => {
            e.stopPropagation();
            e.preventDefault();
        }} onInteractOutside={(e) => {
            e.stopPropagation();
            ...
            if ((e.target as HTMLElement).tagName === 'DIV' && (e.target as HTMLElement).isContentEditable)
                return;
            if (e.key === 'ArrowRight') {
                nextAttachment('next');
            }
        ...
        }} visuallyHiddenTitle='Attachment details' disableDescribedBy>
...
function InnerAttachmentLightbox({ subject, selectedAttachmentId, attachments, viewOnly = false, onClose, onSelectAttachment }: Props) {
    ...
    return (<ScopeProvider atoms={[zoomAtom, hoveredCanvasCommentAtom, displayCanvasCommentsAtom, newCommentCoordinatesAtom]}>
      <m.header className={cn('drag flex h-14 flex-row items-center justify-between border-b pl-4 pr-3 text-sm', {
        ...
        <div className='flex items-center gap-1'>
          {!viewOnly && isNote(subject) && (<Button className='max-lg:hidden' variant='plain' onClick={() => setSidebarOpen((prev) => !prev)} leftSlot={<ChatBubbleIcon />} tooltip={sidebarOpen ? 'Hide sidebar' : 'Show sidebar'} accessibilityLabel={sidebarOpen ? 'Hide sidebar' : 'Show sidebar'}>
      ...
      <div className='flex flex-1 flex-col overflow-y-auto lg:flex-row lg:overflow-hidden'>
        <div className='flex flex-1 flex-col overflow-hidden'>
          <div className='bg-tertiary dark:bg-secondary relative flex h-auto flex-1 overflow-hidden'>
            <LightboxAttachmentRenderer key={attachment.id} attachment={attachment} preventNewComment={viewOnly || !isPostAttachment}/>
            <FigmaTip attachment={attachment}/>
          </div>
          {attachments && attachments.length > 1 && (<Gallery selectedAttachmentId={selectedAttachmentId} attachments={attachments} onSelectAttachment={(attachment) => {
                if (attachment.id === selectedAttachmentId)
                    return;
                onSelectAttachment(attachment);
            }}/>)}
        </div>
        {!viewOnly && isNote(subject) && (<NoteLightboxComments note={subject} attachment={attachment} isOpen={sidebarOpen} onOpenChange={setSidebarOpen}/>)}
      </div>
    </ScopeProvider>);
}

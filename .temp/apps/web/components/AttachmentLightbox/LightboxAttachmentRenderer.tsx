...
import { Attachment, Post } from "../../../../packages/types/index.ts";
import { Button, DownloadIcon, ExternalLinkIcon, Link, PauseIcon, PlayIcon, UIText } from "../../../../packages/ui/src/index.tsx";
import { cn } from "../../../../packages/ui/src/utils/index.ts";
import { CanvasComments } from "../CanvasComments/CanvasComments.tsx";
import { Lottie } from "../Lottie.tsx";
import { stableId } from "../Post/PostReorderableAttachments.tsx";
import { LinkAttachment } from "../Thread/Bubble/AttachmentCard/LinkAttachment.tsx";
import { useFigmaEmbedSelected } from "../../hooks/useFigmaEmbedSelected.ts";
...
export function LightboxAttachmentRenderer({ attachment, preventNewComment, post }: Props) {
    ...
    if (!isRenderable(attachment)) {
        ...
        return (<div className='flex flex-1 flex-col items-center justify-center gap-1'>
...
function GifRenderer({ attachment }: Props) {
    ...
    <div className='m-auto flex h-full w-full items-center justify-center'>
      <video muted autoPlay playsInline loop key={stableId(attachment)} width={attachment.width} height={attachment.height} controls={false} preload='auto' className='h-auto max-h-full w-auto max-w-full'>
...
function VideoRenderer({ attachment, post }: Props) {
    ...
    if (!src)
        return null;
    return (<video key={stableId(attachment)} loop={(attachment.duration ?? 0) < 60000} // loop if less than one minute
     width={attachment.width} height={attachment.height} ref={videoRef} controls={true} preload='auto' className='focus:online-none aspect-video h-full w-full bg-black focus:border-0 focus-visible:outline-none'>
...
function OrigamiRenderer({ attachment }: Props) {
    ...
    return (<div className='flex flex-1 items-center justify-center'>
      <div className='relative inline-flex w-[90%] flex-col items-center justify-center space-y-6 p-6 md:w-[50%]' key={stableId(attachment)}>
        ...
        <UIText tertiary>
          <Link href='https://fb.me/getorigamistudio' target='_blank' rel='noopener noreferrer' className='hover:text-primary'>
            Origami Studio
          </Link>
          {' · '}
          <Link href='https://apps.apple.com/us/app/origami-live/id942636206' target='_blank' rel='noopener noreferrer' className='hover:text-primary'>
            Origami Live
          </Link>
        </UIText>
      </div>
...
function PrincipleRenderer({ attachment }: Props) {
    ...
    return (<div className='flex flex-1 items-center justify-center'>
      <div className='relative inline-flex w-[90%] flex-col items-center justify-center space-y-6 p-6 md:w-[50%]' key={stableId(attachment)}>
        ...
        <UIText tertiary>
          <Link href='https://principleformac.com/' target='_blank' rel='noopener noreferrer' className='hover:text-primary'>
            Principle for Mac
          </Link>
          {' · '}
          <Link href='https://apps.apple.com/us/app/principle-mirror/id991911319' target='_blank' rel='noopener noreferrer' className='hover:text-primary'>
            Principle for iOS
          </Link>
        </UIText>
      </div>
...
function LottieRenderer({ attachment }: Props) {
    ...
    if (!attachment.download_url)
        return null;
    return (<div onClick={togglePlay} className='group relative inline-flex h-full w-full items-center justify-center object-contain p-4'>
      <Lottie key={stableId(attachment)} url={attachment.download_url} onFrame={handleEnterFrame} onLoad={setAnimationItem} className='h-full'/>
      ...
      <div className='bg-elevated absolute bottom-4 left-4 z-10 flex w-[calc(100%-32px)] items-center justify-center gap-3 rounded-lg p-4 align-middle opacity-0 transition-all duration-300 group-hover:opacity-100'>
        <Button variant='primary' onClick={togglePlay} key={`${isPlaying}`} iconOnly={animationItem?.isPaused ? <PlayIcon /> : <PauseIcon />} accessibilityLabel={animationItem?.isPaused ? 'Play' : 'Pause'}/>
        <Slider defaultValue={[0]} value={[frame]} max={100} step={1} onClick={(e: MouseEvent) => e.stopPropagation()} onValueChange={(value: number[]) => {

...
import { Attachment, Note } from "../../../../packages/types/index.ts";
...
import { NoteAttachmentComments } from "../NoteComments/index.tsx";
interface Props {
    note: Note;
...
export function NoteLightboxComments({ note, attachment, isOpen, onOpenChange }: Props) {
    ...
    return (<>
      ...
      <OuterContainer isOpen={isOpen || !isLargeViewport} width={width}>
        <NoteAttachmentComments note={note} attachmentId={attachment?.id} onSidebarOpenChange={onOpenChange} hideAttachment/>
      </OuterContainer>
    </>);
}
function OuterContainer({ children, isOpen, width }: {
    children: React.ReactNode;
    isOpen: boolean;
    width: string;
}) {
    return (<m.aside initial={false} animate={{ width: isOpen ? width : 0 }} transition={{ type: 'spring', stiffness: 500, damping: 40 }} aria-disabled={!isOpen} className={cn('4xl:max-w-[500px] flex flex-none flex-col border-t focus:outline-none focus:ring-0 lg:w-full lg:max-w-[400px] lg:border-l lg:border-t-0', {
            invisible: !isOpen
        })}>

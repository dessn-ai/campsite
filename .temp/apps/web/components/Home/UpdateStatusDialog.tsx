...
import { OrganizationsOrgSlugMembersMeStatusesPostRequest } from "../../../../packages/types/generated.ts";
import { Button } from "../../../../packages/ui/src/Button/index.tsx";
import { Checkbox } from "../../../../packages/ui/src/Checkbox/index.tsx";
import { ChevronDownIcon, InformationIcon } from "../../../../packages/ui/src/Icons/index.tsx";
import { Link } from "../../../../packages/ui/src/Link/index.tsx";
import { Select } from "../../../../packages/ui/src/Select/index.ts";
import * as Dialog from "../../../../packages/ui/src/Dialog/index.tsx";
import { UIText } from "../../../../packages/ui/src/Text/index.tsx";
import { TextField } from "../../../../packages/ui/src/TextField/index.tsx";
import { Tooltip } from "../../../../packages/ui/src/Tooltip/index.tsx";
import { DateAndTimePicker } from "../DateAndTimePicker/index.tsx";
import { FullPageLoading } from "../FullPageLoading.tsx";
import { getTimeRemaining } from "../MemberStatus.tsx";
import { ReactionPicker } from "../Reactions/ReactionPicker.tsx";
import { useScope } from "../../contexts/scope.tsx";
import { useCreateStatus } from "../../hooks/useCreateStatus.ts";
import { useDeleteStatus } from "../../hooks/useDeleteStatus.ts";
...
import { useRecentStatuses } from "../../hooks/useRecentStatuses.ts";
import { useStatusIsExpired } from "../../hooks/useStatusIsExpired.ts";
import { useUpdateStatus } from "../../hooks/useUpdateStatus.ts";
import { isStandardReaction } from "../../utils/reactions/index.ts";
...
export function UpdateStatusDialog({ open, onOpenChange }: {
    open: boolean;
    ...
    return (<>
      <Dialog.Root aria-describedby='Update your status' open={open} onOpenChange={onOpenChange} size='lg' align='top'>
        ...
        {isLoadingMember && (<Dialog.Content className='p-24'>
            <FullPageLoading />
          </Dialog.Content>)}
...
export function StatusPickerContent({ onOpenChange }: {
    onOpenChange: (open: boolean) => void;
}) {
    ...
    const [expiresIn, setExpiresIn] = useState<Expiration | null>(currentStatus?.expiration_setting ?? defaultState.expiration_setting);
    const [expiresAt, setExpiresAt] = useState<Date | undefined>(currentStatus?.expires_at ? new Date(currentStatus.expires_at) : defaultState.expires_at);
    const [willPauseNotifications, setWillPauseNotifications] = useState((currentUser?.notifications_paused && currentStatus?.pause_notifications) ?? defaultState.pause_notifications);
    ...
    return (<>
      <Dialog.Content className='p-0'>
        <div className='scrollbar-hide flex max-h-[40vh] flex-1 flex-col overflow-hidden overflow-y-auto px-2 py-3'>
          {suggestedStatuses?.map((preset) => (<button key={`${preset.message}`} className='hover:bg-tertiary flex h-10 cursor-pointer items-center gap-1.5 rounded-lg px-2 py-3 text-sm' onClick={() => {
                if (currentStatus) {
                    updateStatus.mutate({
                        org: `${scope}`,
                        emoji: preset.emoji,
        ...
        <div className='flex flex-col gap-3 border-t p-4'>
          <div className='relative'>
            <div className='absolute left-1.5 top-1.5 z-[1]'>
              <ReactionPicker onReactionSelect={(reaction) => {
            if (!isStandardReaction(reaction))
                return;
            setEmoji(reaction.native);
        }} trigger={<Button variant='plain' accessibilityLabel='Update status emoji' className='group/emoji' iconOnly={<span className='text-lg'>{emoji}</span>}/>}/>
            </div>
            <TextField autoFocus={!isMobile} value={message} onChange={setMessage} placeholder='What’s your status?' additionalClasses='bg-transparent rounded-md text-[15px] h-10.5 dark:bg-transparent pl-12 pr-24' onKeyDown={(e) => {
            if (e.key === 'Enter') {
                e.preventDefault();
                onSave();
            }
        }}/>
            {message && (<div className='absolute right-1.5 top-1.5 z-[1]'>
                <Select options={[
                { value: '30m', label: '30m' },
                { value: '1h', label: '1h' },
                { value: '4h', label: '4h' },
            ...
            ]} align='end' value={expiresIn || '30m'} onChange={(value) => {
                if (value === 'custom') {
            ...
            <div className='mt-1.5 flex flex-1 items-center gap-1'>
              <div className='flex flex-1 justify-between gap-3'>
                <label className='flex items-center gap-1'>
                  ...
                  <Tooltip label='Silence notifications while this status is active. Anything you miss will be in your inbox to
                    review later.'>
                    <span>
                ...
                {willPauseNotifications && (<UIText tertiary>
      ...
      <Dialog.Footer className='border-t-0 pt-0'>
        ...
        <Dialog.TrailingActions>
          {hasStatus && !isDirty && (<Button variant='flat' type='submit' onClick={() => {
          ...
          {(isDirty || !hasStatus) && (<Button variant='primary' type='submit' onClick={onSave} disabled={updateStatus.isPending || !isDirty || !message}>
              Update status
            </Button>)}
        </Dialog.TrailingActions>
      </Dialog.Footer>

      <CustomExpirationCalendarDialog open={customExpirationCalendarDialogOpen} onOpenChange={setCustomExpirationCalendarDialogOpen} initialDate={expiresAt ?? addHours(new Date(), 1)} onChange={(date) => {
            setExpiresIn('custom');
            setExpiresAt(date);
        }}/>
    </>);
}
function CustomExpirationCalendarDialog({ open, onOpenChange, initialDate, onChange }: {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    initialDate: Date;
    ...
    return (<Dialog.Root size='fit' open={open} onOpenChange={onOpenChange} visuallyHiddenTitle='Custom status expiration date' visuallyHiddenDescription='Select a date for your status to expire'>
      <Dialog.Content className='place-self-center p-6'>
        <div className='flex h-full w-full flex-col gap-3'>
          <DateAndTimePicker value={date} onChange={setDate}/>
          <Button fullWidth disabled={date < new Date()} className='py-1' variant='primary' onClick={() => {

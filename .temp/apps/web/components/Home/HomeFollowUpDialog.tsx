...
import { Button, Calendar, ClockIcon, CloseIcon, Command, UIText } from "../../../../packages/ui/src/index.tsx";
...
import { getFollowUpDates } from "../FollowUp/index.ts";
import { useFollowUpActions } from "../../hooks/useFollowUpActions.ts";
...
export function HomeFollowUpDialog({ title, id, type, viewerFollowUp, open, onOpenChange, onBeforeCreate }: Props) {
    ...
    const { customFrom, customTo } = useMemo(() => ({ customFrom: addDays(new Date(), 1), customTo: addYears(new Date(), 1) }), []);
    function close() {
    ...
    return (<Dialog.Root open={open} onOpenChange={(open) => {
        ...
        }} size='lg' align='top' visuallyHiddenDescription='Select a follow up time or date'>
      <Dialog.Header className='pb-0'>
        <Dialog.Title>Follow up</Dialog.Title>
        <UIText tertiary>{title}</UIText>
      </Dialog.Header>

      {customDate ? (<div className='mt-3 flex flex-col items-center gap-2 p-3.5'>
          <Calendar initialFocus fromDate={customFrom} toDate={customTo} mode='single' selected={customDate} onSelect={(date) => setCustomDate(date)} className='w-full' classNames={{
                months: '',
                month: 'space-y-2',
        ...
        </div>) : (<Command className='flex flex-1 flex-col overflow-hidden' loop>
          <Command.List className='scrollbar-hide overflow-y-auto p-3'>
            {viewerFollowUp && (<HighlightedCommandItem onSelect={() => {
            ...
            {dates?.map((date) => (<HighlightedCommandItem key={date.label} onSelect={() => {
                    onBeforeCreate?.();
                    upsert(date.date);
                    close();
                }} className='h-10 gap-2 rounded-lg pl-2 pr-3.5'>
            ...
            <HighlightedCommandItem onSelect={() => setCustomDate(customFrom)} className='h-10 gap-2 rounded-lg pl-2 pr-3.5'>
              <ClockIcon className='text-tertiary'/>

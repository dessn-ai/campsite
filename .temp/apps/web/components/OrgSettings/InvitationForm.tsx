...
import { Button, ButtonPlusIcon, Select, TextField, TrashIcon, UIText } from "../../../../packages/ui/src/index.tsx";
import * as Dialog from "../../../../packages/ui/src/Dialog/index.tsx";
import { cn, emailRegex } from "../../../../packages/ui/src/utils/index.ts";
...
import { InvitationConfirmationDialog } from "./InvitationConfirmationDialog.tsx";
...
export function InvitationForm({ defaultCount = 1, onInvitationsSent }: InvitationFormProps) {
    ...
    function handleRoleChange(id: string, role: string) {
        setInvitations(invitations.map((invitation) => (invitation.id === id ? { ...invitation, role, projects: [] } : invitation)));
    }
    function handleEmailChange(id: string, value: string) {
        setInvitations(invitations.map((invitation) => (invitation.id === id ? { ...invitation, email: value } : invitation)));
    }
    function handleProjectIdsChange(id: string, projectIds: Set<string>) {
        setInvitations(invitations.map((invitation) => invitation.id === id
    ...
    return (<form onSubmit={handleOnSubmit} className='flex flex-col gap-3'>
      <div className='flex flex-col gap-3 max-sm:divide-y'>
        {invitations.map((invitation, index) => (<Invitation key={invitation.id} invitation={invitation} index={index} defaultCount={defaultCount} handleOnSubmit={handleOnSubmit} handleRoleChange={handleRoleChange} handleEmailChange={handleEmailChange} handleProjectIdsChange={handleProjectIdsChange} handlePaste={handlePaste} onRemoveInvitation={onRemoveInvitation}/>))}
      </div>
      ...
      <div className='flex justify-end'>
        <InvitationConfirmationDialog invitations={invitations} open={isDialogOpen} onCancel={() => setIsDialogOpen(false)} onSuccess={() => {
...
function Invitation({ invitation, index, defaultCount, handleOnSubmit, handleRoleChange, handleEmailChange, handleProjectIdsChange, handlePaste, onRemoveInvitation }: {
    ...
    return (<div className='flex flex-col gap-1'>
      <div className='relative flex flex-col gap-3 sm:flex-row'>
        <div className='grow'>
          ...
          <TextField type='email' id={invitation.id} name={invitation.id} value={invitation.email} autoFocus={index > defaultCount - 1 && index > 0} placeholder='Email address' onCommandEnter={handleOnSubmit} onPaste={(event) => handlePaste(invitation.id, event)} onChange={(value) => handleEmailChange(invitation.id, value)}/>
        </div>
        ...
        <div className='flex items-end gap-3'>
          <div className='w-full sm:w-[130px]'>
            ...
            <Select align='end' variant='base' showChevron options={roleOptions} popoverWidth={300} onChange={(role) => handleRoleChange(invitation.id, role)} value={invitation.role}/>
          </div>

          <Button variant='flat' iconOnly={<TrashIcon />} disabled={index === 0} accessibilityLabel='Remove' onClick={() => onRemoveInvitation(invitation.id)}/>
        </div>
      </div>
      {invitation.role === 'guest' && (<>
          ...
          <MemberProjectsManagementDialog open={projectsDialogOpen} onOpenChange={setProjectsDialogOpen} projectIds={new Set(invitation.projects.map((project) => project.id))} onProjectIdsChange={(projectIds) => handleProjectIdsChange(invitation.id, projectIds)}/>
        </>)}
    </div>);
}
function MemberProjectsManagementDialog({ open, onOpenChange, projectIds, onProjectIdsChange }: {
    open: boolean;
    onOpenChange: (open: boolean) => void;
    projectIds: Set<string>;
    onProjectIdsChange: (projectIds: Set<string>) => void;
}) {
    return (<Dialog.Root open={open} onOpenChange={onOpenChange} align='top' disableDescribedBy>
      <Dialog.Header>

...
import { SlackChannel } from "../../../../packages/types/index.ts";
import { AlertIcon, HashtagIcon, LockIcon, Select, SelectOption, SelectTrigger, SlackIcon } from "../../../../packages/ui/src/index.tsx";
import { useCreateSlackChannelSync } from "../../hooks/useCreateSlackChannelSync.ts";
import { useGetSlackChannel } from "../../hooks/useGetSlackChannel.ts";
import { useGetSlackChannels } from "../../hooks/useGetSlackChannels.ts";
import { useRemoteDataSelectFilter } from "../../hooks/useRemoteDataSelectFilter.ts";
...
function SlackChannelPicker(props: Props) {
    ...
    const prependActiveChannel = (channels: SlackChannel[]) => activeChannel ? [activeChannel, ...channels.filter((c) => c.id !== activeChannel.id)] : channels;
    ...
    const handleOpenChange = (open: boolean) => {
        if (!open || syncInitiated)
            return;
        syncChannels.mutate();
        setSyncInitiated(true);
    };
    if (getChannels.error) {
        return (<div className='flex items-center justify-center space-x-2 p-1 text-sm'>
        <AlertIcon />
        <span>Unable to find your team’s Slack channels</span>
      </div>);
    }
    return (<ChannelPicker activeId={activeId} channels={prependActiveChannel(channels)} customFilter={remoteDataSelectFilter} onChange={onChange} onQueryChange={setQuery} onOpenChange={handleOpenChange} leftSlot={includeSlackIcon && <SlackIcon size={16}/>} disabled={getChannels.isLoading}/>);
}
...
function ChannelPicker(props: ChannelPickerProps) {
    ...
    return (<Select typeAhead disabled={props.disabled} options={options} value={activeChannel.value} customFilter={customFilter} onChange={handleOnChange} onQueryChange={onQueryChange} onOpenChange={onOpenChange}>

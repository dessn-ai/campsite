...
import { Command, ConditionalWrap, UIText, useCommand } from "../../../../packages/ui/src/index.tsx";
import { NoteRow } from "./NoteRow.tsx";
import { filterAtom, sortAtom } from "./NotesIndexDisplayDropdown.tsx";
    ...
    return (<ConditionalWrap condition={needsCommandWrap} wrap={(children) => (<SubjectCommand>
      ...
      {Object.entries(groups).map(([date, notes]) => {
            ...
            <div className='-mx-2 flex flex-col gap-px py-2'>
              {notes.map((note) => (<NoteRow note={note} key={note.id} hideProject={hideProject}/>))}
            </div>
          </div>);
        })}
    </ConditionalWrap>);

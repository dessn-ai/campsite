...
import { Button, Command, LoadingSpinner, NoteIcon, UIText } from "../../../../packages/ui/src/index.tsx";
...
import { NoteIndexFilterType } from "./NotesIndexDisplayDropdown.tsx";
...
export function NotesHoverList({ children, side = 'right', align = 'start', sideOffset = 0, alignOffset = 0, disabled: _disabled = false }: {
    ...
    const notes = useMemo(() => groupByDate(flattenInfiniteData(getNotes.data) || [], (note) => note.created_at), [getNotes.data]);
    ...
    return (<HoverCard open={open} onOpenChange={setOpen} disabled={disabled} targetHref={href}>
      ...
      <HoverCard.Content side={side} align={align} sideOffset={sideOffset} alignOffset={alignOffset}>
        ...
        {!hasNotes && !getNotes.isLoading && (<div className='flex flex-1 items-center justify-center px-6 py-12'>
...
function NewNoteButton({ onSuccess }: {
    onSuccess?: ({ id }: {
        id: string;
    }) => void;
}) {

...
import { CheckIcon, ChevronDownIcon, GridIcon, ListIcon, SwitchIcon } from "../../../../packages/ui/src/Icons/index.tsx";
...
export const filterAtom = atomFamily((scope: CookieValueTypes) => atomWithWebStorage<NoteIndexFilterType>(`${scope}:notes-index-filter`, 'for-me'));
type NoteIndexSortType = 'last_activity_at' | 'created_at';
export const sortAtom = atomFamily(({ scope, filter }: {
    scope: CookieValueTypes;
    filter: string;
}) => atomWithWebStorage<NoteIndexSortType>(`${scope}:notes-index-sort:${filter}`, 'last_activity_at'), (a, b) => a.scope === b.scope && a.filter === b.filter);
export function NotesIndexDisplayDropdown({ iconOnly = false }: {
    iconOnly?: boolean;
}) {
    ...
    return (<>
      ...
      <DropdownMenu open={dropdownIsOpen} onOpenChange={setDropdownIsOpen} items={items} align='end' trigger={<Button variant='plain' leftSlot={iconOnly ? undefined : <ChevronDownIcon />} iconOnly={iconOnly ? <SwitchIcon /> : undefined} accessibilityLabel='Display dropdown' size='sm' tooltip='Display and sort' tooltipShortcut={shortcut}>

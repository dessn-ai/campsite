...
import { Message, MessageThread, OrganizationMember } from "../../../../packages/types/index.ts";
import { Button, MoonFilledIcon, Tooltip, UIText } from "../../../../packages/ui/src/index.tsx";
import { cn } from "../../../../packages/ui/src/utils/index.ts";
import { useCreateThreadNotificationForce } from "../../hooks/useCreateThreadNotificationForce.ts";
import { dateToEpoch, MS_IN_DAY } from "../../utils/dateToEpoch.ts";
import { longTimestamp, longTimestampFromDate } from "../../utils/timestamp.ts";
import { editModeAtom } from "../Chat/atoms.tsx";
import { HTMLRenderer } from "../HTMLRenderer/index.tsx";
import { Bubble } from "./Bubble/index.tsx";
...
function groupMessagesByUser(messages: Message[]) {
    ...
    messages.forEach((message) => {
        if (message.call && message.discarded_at)
            return;
...
function insertTimestampsBetweenGroups(groups: MessageGroup[], hasNextPage: boolean = false) {
    const groupsWithTimestamps: (MessageGroup | {
        timestamp: Date;
    } | {
        day: string;
    })[] = [];
    if (!hasNextPage && groups.length > 0) {
        ...
        if (firstCreatedAt)
            groupsWithTimestamps.push({ timestamp: new Date(firstCreatedAt) });
    }
...
export function Messages({ thread, messages, hasNextPage, onNewMessage }: Props) {
    ...
    useEffect(() => {
        ...
        if (localLast?.id === lastMessageId.current)
            return;
        if (lastMessageId.current)
            onNewMessage(localLast);
    ...
    if (!thread)
        return null;
    return (<div className={cn('relative flex flex-1 flex-col gap-3 px-3 pb-3 lg:px-4', {
...
function DNDIndicator({ thread }: {
    ...
    if (!otherMember.user.notifications_paused)
        return;
    return (<div className={cn('mx-auto flex flex-col items-center justify-center gap-1 px-8 text-center transition-opacity')}>
      <div className='flex items-center gap-1.5'>
        <MoonFilledIcon className={cn({
        ...
        <span className={cn('break-anywhere text-xs [text-wrap:balance]', {
            'text-violet-500 dark:text-violet-400': !thread.viewer_can_force_notification,
            'text-tertiary': thread.viewer_can_force_notification
        })}>
      ...
      {thread.viewer_can_force_notification && (<Button variant='text' className='text-xs text-violet-500 dark:text-violet-400' onClick={() => forceNotification({ threadId: thread.id }, {
                onSuccess: () => {
...
function TimestampHeader({ timestamp, editMode }: {
    timestamp: Date;
    editMode: boolean;
}) {
    ...
    return (<Tooltip label={longTimestampFromDate(timestamp)}>
      <div className={cn('my-2 flex items-center justify-center gap-1 transition-opacity', {
            'opacity-30': editMode
        })}>
        {!groupIsWithinToday && (<>
...
function DayHeader({ day }: {
    ...
    return (<div className='bg-quaternary dark:bg-tertiary -mx-3 my-8 flex h-px items-center justify-center lg:-mx-4 lg:px-4'>
...
function SystemMessages({ messages, editMode }: {
    messages: Message[];
    editMode: boolean;
}) {
    return (<div className={cn('mx-auto flex flex-col items-center justify-center gap-4 px-8 text-center transition-opacity', {
            'opacity-30': editMode
        })}>
      {messages.map((message) => {
            return (<Tooltip disableHoverableContent key={message.created_at} label={longTimestamp(message.created_at)}>
            <HTMLRenderer as='span' text={message.content} 
...
function ViewerMessages({ messages, thread }: {
    messages: Message[];
    thread: MessageThread;
}) {
    return (<div className='m-0 flex w-full flex-col self-end p-0'>
      {messages.map((message) => {
...
function OtherMessages({ group, thread }: {
    group: MessageGroup;
    thread: MessageThread;
}) {
    return (<div className='flex flex-col'>
      {thread && thread.other_members.length > 1 && (<div className={cn('ml-13.5 lg:ml-13', {
      ...
      <div className='m-0 flex w-full flex-col self-start p-0'>
        {group.messages.map((message) => {
            const position = group.messages.length === 1
                ? 'only'

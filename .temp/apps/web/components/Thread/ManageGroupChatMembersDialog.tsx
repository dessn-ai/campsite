...
import { MessageThread, SyncOrganizationMember } from "../../../../packages/types/index.ts";
import { Avatar, Button, Checkbox, Command, isMetaEnter, Link, SearchIcon, UIText } from "../../../../packages/ui/src/index.tsx";
...
import { useUpdateThreadOtherMembers } from "../../hooks/useUpdateThreadOtherMembers.ts";
interface Props {
...
export function ManageGroupChatMembersDialog({ thread, open, onOpenChange }: Props) {
    ...
    const membersHaveChanged = selectedMemberIds.length !== thread.other_members.length ||
    ...
    return (<Dialog.Root open={open} onOpenChange={handleOpenChange} size='lg' align='top' disableDescribedBy>
      ...
      <Command className='flex min-h-[30dvh] flex-1 flex-col overflow-hidden' loop>
        <div className='flex items-center gap-3 border-b px-3'>
          ...
          <Command.Input autoFocus placeholder='Search people...' value={query} onValueChange={setQuery} className='w-full border-0 bg-transparent py-3 pl-0 pr-4 text-[15px] placeholder-gray-400 outline-none focus:border-black focus:border-black/5 focus:ring-0' onKeyDownCapture={onKeyDownCapture}/>
        </div>
        ...
        <Command.List className='scrollbar-hide overflow-y-auto'>
          <Command.Group className='p-3'>
            ...
            {members?.map((member) => (<HighlightedCommandItem key={`${member.user.display_name}-${member.user.username}`} onClick={() => onSelectMember(member)} onSelect={() => onSelectMember(member)} className='h-10 gap-3 rounded-lg'>

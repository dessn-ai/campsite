...
import { getLocalSrcAtom } from "../../../../hooks/useUploadChatAttachments.tsx";
interface Props {
...
export function ImageAttachment({ attachment, selfSize, cover = false, onError, maxHeight }: Props) {
    ...
    return (<div className={cn({ 'flex h-full w-full items-center justify-center': !cover, content: cover })}>
      <Image alt={attachment.name ?? 'Image attachment'} src={src} width={width} height={height} draggable={false} className={cn('max-w-full', {

...
export function VideoAttachment({ attachment, selfSize, cover = false, autoplay = true, maxHeight }: Props) {
    ...
    return (<video key={attachment.id} muted loop={attachment.duration < 60000} // loop if less than one minute
     controls={true} preload='metadata' draggable={false} playsInline autoPlay={autoplay} className={cn('relative max-w-full', {

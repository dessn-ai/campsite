...
export function GifAttachment({ attachment, selfSize, cover = false, maxHeight = '44rem' }: Props) {
    ...
    return isBlob ? (<Image unoptimized src={url} draggable={false} className={cn('relative z-[1] max-w-full', {
    ...
    style={{ width, height, maxHeight }} alt=''/>) : (<video key={attachment.id} muted loop autoPlay playsInline controls={false} preload='metadata' draggable={false} className={cn('relative max-w-full', {

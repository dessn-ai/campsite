...
import { PhotoHideIcon } from "../../../../../../packages/ui/src/index.tsx";
import { GifAttachment } from "./GifAttachment.tsx";
import { ImageAttachment } from "./ImageAttachment.tsx";
import { LinkAttachmentStatic } from "./LinkAttachmentStatic.tsx";
import { LottieAttachment } from "./LottieAttachment.tsx";
import { OrigamiAttachment } from "./OrigamiAttachment.tsx";
import { PrincipleAttachment } from "./PrincipleAttachment.tsx";
import { StitchAttachment } from "./StitchAttachment.tsx";
import { VideoAttachment } from "./VideoAttachment.tsx";
...
function attachmentShouldCover(attachment: Attachment) {
    if (!attachment.video && !attachment.gif)
        return false;
...
export function AttachmentCard({ attachment, autoplay }: Props) {
    ...
    if (hasErrored) {
        return (<div className='bg-secondary text-tertiary flex h-full w-full items-center justify-center'>
    ...
    if (attachment.video) {
        return (<div className='relative flex max-h-[44rem] w-full items-center justify-center' style={{ aspectRatio: `${attachment.width} / ${attachment.height}` }}>
        {attachment.preview_thumbnail_url && !cover && (<Image aria-hidden='true' alt='Preview thumbnail' src={attachment.preview_thumbnail_url} width={attachment.width} height={attachment.height} className='absolute inset-0 z-0 opacity-10 blur-xl' onError={handleError}/>)}
...
export function Accessory({ label }: {
    label: string;
}) {
    return (<div className='dark:bg-elevated rounded-md bg-black px-2 py-1 text-center font-mono text-[11px] text-xs font-semibold text-white'>
      {label}
    </div>);
}

...
import { ReplyIcon, UIText } from "../../../../../packages/ui/src/index.tsx";
...
export function ReplyPreview({ message, thread, position }: ReplyPreviewProps) {
    ...
    if (!message.reply)
        return null;
    return (<div className='flex max-w-[calc(min(80%,_36rem))]'>
      <div className={cn('flex flex-col gap-1', {
        ...
        <div className={cn('text-quaternary flex items-center gap-1', {
          ...
          <UIText size='text-xs' className='line-clamp-1'>
            Replying to{' '}
            {message.reply.viewer_is_sender ? (message.viewer_is_sender ? ('yourself') : ('you')) : (<UIText element='span' size='text-xs' weight='font-medium'>
                {message.reply.sender_display_name}
              </UIText>)}
          </UIText>
        </div>

        <div className={cn('bg-quaternary relative flex-none rounded-[14px]', {
          ...
          {message.reply?.has_content ? (<div className='px-3.5 py-1.5 lg:px-3'>
              <div ref={replyTextRef} className={cn('text-tertiary chat-prose reply-prose max-w-md text-xs', !replyOpen && 'break-anywhere line-clamp-1 break-all')} dangerouslySetInnerHTML={{ __html: message.reply.content }}/>
            </div>) : (message.reply?.last_attachment && (<div>
                {isRenderable(message.reply.last_attachment) ? (<div className='w-20 max-w-[80px] opacity-50'>
                    <div className='max-h-full self-center' style={{
                    aspectRatio: `${message.reply.last_attachment.width || 1} / ${message.reply.last_attachment.height || 1}`
                }}>
                      <div className={cn('flex shrink-0 items-center justify-center', 'absolute inset-0')}>
                        <div className={cn('relative h-full w-full overflow-hidden rounded-[14px]', {
                    'rounded-br': message.viewer_is_sender,
                    'rounded-bl': !message.viewer_is_sender
                })}>
                  ...
                  </div>) : (<div className='max-w-[160px] opacity-70'>

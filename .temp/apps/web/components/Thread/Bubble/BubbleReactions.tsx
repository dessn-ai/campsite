...
function getClasses(hasReacted: boolean) {
    return cn('flex justify-center pointer-events-auto items-center', 'pl-1.5 pr-[7px] h-5.5 text-[11px] gap-[5px]', 'group rounded-full font-medium ring-1 min-w-[32px]', {
        'bg-blue-100/70 dark:bg-blue-900/40 hover:bg-blue-100 dark:hover:bg-blue-900/60 text-blue-900 dark:text-blue-400': hasReacted,
...
export function BubbleReactions({ message, thread }: Props) {
    ...
    if (message.discarded_at)
        return null;
    if (!message.grouped_reactions || message.grouped_reactions.length === 0)
        return null;
    ...
    return (<div className={cn('ring-primary h-5.5 bg-primary flex flex-wrap items-center gap-0.5 rounded-full px-px shadow-sm ring-2', {
            'flex-row-reverse': message.viewer_is_sender
        })}>
      <Reactions reactions={message.grouped_reactions} onReactionSelect={handleReactionSelect} getClasses={getClasses}/>
    </div>);
}

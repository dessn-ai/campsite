...
import { Link, Tooltip, UIText } from "../../../../../packages/ui/src/index.tsx";
import { cn } from "../../../../../packages/ui/src/utils/index.ts";
import { chatThreadPlacementAtom, editModeAtom } from "../../Chat/atoms.tsx";
import { MemberAvatar } from "../../MemberAvatar/index.tsx";
import { RichLinkCard } from "../../RichLinkCard.tsx";
import { RichTextRenderer } from "../../RichTextRenderer/index.tsx";
import { Attachments } from "./Attachments.tsx";
import { MessageCallBubble } from "./MessageCallBubble.tsx";
...
import { createMessageStateAtom, useRetryCreateMessage } from "../../../hooks/useCreateMessage.ts";
import { containsOnlyReactions } from "../../../utils/reactions/containsOnlyReactions.ts";
import { longTimestamp } from "../../../utils/timestamp.ts";
import { Engagements } from "./Engagements.tsx";
import { isMessageOptimistic } from "./isMessageOptimistic.ts";
...
export function Bubble({ message, thread, position }: Props) {
    ...
    return (<div className={cn('flex flex-col', {
      ...
      <div className='flex w-full gap-2'>
        {/* avatar column */}
        {shouldRenderAvatar && (<Link href={`/${scope}/people/${message.sender.user.username}`} className='-translate-y-1 self-end'>
            <MemberAvatar member={message.sender} size='base'/>
          </Link>)}

        {!shouldRenderAvatar && isGroupOrIntegrationDm(thread) && !message.viewer_is_sender && (<div className='w-8 flex-none'/>)}

        <div className={cn('group/bubble relative flex flex-1 flex-col transition-opacity', {
            'opacity-30': editMode && editMode.id !== message.id,
            'items-end': message.viewer_is_sender,
            'items-start': !message.viewer_is_sender,
            'mb-0.5': message.grouped_reactions.length === 0
        })} onContextMenu={(evt) => {
            if (isOptimistic || canHover)
                return;
            evt.preventDefault();
        }} {...longPressProps}>
          <div className={cn('relative flex w-full flex-1 flex-col items-end gap-0.5', !message.viewer_is_sender && 'items-start', {
            ...
            {!message.discarded_at && message.unfurled_link && !message.attachments.length && (<div className={cn('relative flex w-full ring-2 ring-[--bg-primary] sm:max-w-md', {
                'justify-start': !message.viewer_is_sender,
                'justify-end': message.viewer_is_sender
            }, getBorderRadiusClasses(message.reply?.id ? 'middle' : position === 'first' || position === 'only' ? 'first' : 'middle', message.viewer_is_sender))}>
                <RichLinkCard className={getBorderRadiusClasses(message.reply?.id ? 'middle' : position === 'first' || position === 'only' ? 'first' : 'middle', message.viewer_is_sender)} url={message.unfurled_link}/>
              </div>)}

            {!!message.attachments.length && (<Attachments message={message} thread={thread} overflowState={[overflowOpen, setOverflowOpen]}/>)}

            {(message.has_content || message.discarded_at || message.call) && (<div className={cn('flex w-full items-center justify-end gap-1.5', !message.viewer_is_sender && 'flex-row-reverse')}>
                {message.call && (<MessageCallBubble thread={thread} call={message.call} className={roundedClasses} message={message}/>)}
      ...
      {message.optimistic_id && (<StatusIndicators message={message} position={position} optimisticId={message.optimistic_id}/>)}
    </div>);
}
function TextBubble({ message, thread, position, className }: {
    message: Message;
    thread: MessageThread;
    position: Position;
    className: string;
    ...
    return (<Tooltip align={message.viewer_is_sender ? 'end' : 'start'} label={longTimestamp(message.created_at, { month: 'short' })}>
      <div className={cn('chat-prose relative select-text whitespace-pre-wrap break-words', className, {
            ...
            'px-2.5': position === 'only' && hasReactionsOnly && !message.viewer_is_sender && isGroupOrIntegrationDm(thread),
            'pt-1': position === 'first' && hasReactionsOnly && !message.viewer_is_sender && isGroupOrIntegrationDm(thread),
...
function EditedIndicator({ message, position, thread }: {
    message: Message;
    position: Position;
    thread: MessageThread;
}) {
    if (!message.updated_at || message.updated_at === message.created_at || message.discarded_at)
        return null;
    return (<div className={cn('flex', {
      ...
      <Tooltip align={message.viewer_is_sender ? 'end' : 'start'} label={`Edited ${longTimestamp(message.updated_at, { month: 'short' })}`}>
        <div className={cn('text-quaternary flex items-center', {
            'items-start': !message.viewer_is_sender,
            'items-end': message.viewer_is_sender
        })}>
...
function StatusIndicators({ message, position, optimisticId }: {
    message: Message;
    position: Position;
    optimisticId: string;
}) {
    ...
    if (!isOptimistic || !state)
        return null;
    return (<div className={cn('flex self-end pr-3.5 lg:pr-3', {
            'pb-1.5': position !== 'last' &&
                position !== 'only' &&
                (state.status === 'error' || (state.status === 'pending' && showPending))
        })}>
      {state.status === 'error' && (<button disabled={retryMutation.isPending} onClick={() => {
      ...
      {state.status === 'pending' && showPending && (<UIText size='text-xs' className='text-quaternary'>
          Still sending...
        </UIText>)}
    </div>);
}

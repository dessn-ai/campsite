...
import { BubbleReactions } from "./BubbleReactions.tsx";
import { SharedPost } from "./SharedPost.tsx";
interface Props {
    message: Message;
...
export function Engagements({ message, thread }: Props) {
    if (message.discarded_at)
        return null;
    const hasReactionsOrSharedPost = (message.grouped_reactions && message.grouped_reactions.length > 0) || message.shared_post_url;
    if (!hasReactionsOrSharedPost)
        return null;
    return (<div className={cn('-mb-0.5 flex -translate-y-1.5 flex-row', 'z-10', // ensure reactions are above attachments only bubbles
        {

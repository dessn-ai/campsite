...
import { MessageThreadMembership } from "../../../../packages/types/index.ts";
import { Button, RadioGroup, RadioGroupItem, UIText } from "../../../../packages/ui/src/index.tsx";
import * as Dialog from "../../../../packages/ui/src/Dialog/index.tsx";
import { useUpdateThreadMembership } from "../../hooks/useUpdateThreadMembership.ts";
...
export function ThreadNotificationsSettingsDialog({ membership, threadId, open, onOpenChange }: Props) {
    ...
    function handleSave() {
        if (level === null)
            return;
        updateThread({ notification_level: level }, {
            onSuccess: () => {
    ...
    return (<Dialog.Root open={open} onOpenChange={onOpenChange} size='sm'>
      ...
      <div className='px-4 pb-6'>
        <RadioGroup aria-label='Notification settings' value={level} className='flex flex-col gap-3' onValueChange={(value) => setLevel(value as MessageThreadMembership['notification_level'])}>

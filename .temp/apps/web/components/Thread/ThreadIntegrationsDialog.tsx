...
import { MessageThread, OauthApplication } from "../../../../packages/types/index.ts";
...
import { useCreateThreadOauthApp } from "../../hooks/useCreateThreadOauthApp.ts";
import { useDeleteThreadOauthApp } from "../../hooks/useDeleteThreadOauthApp.ts";
import { useGetOauthApplications } from "../../hooks/useGetOauthApplications.ts";
import { useGetThreadOauthApps } from "../../hooks/useGetThreadOauthApps.ts";
import { useHash } from "../../hooks/useHash.ts";
export function ThreadIntegrationsDialog({ open, onOpenChange, thread }: {
    open: boolean;
    ...
    function onAddOauthApp(oauthApp: OauthApplication) {
        createOauthApp.mutate({ oauth_application_id: oauthApp.id }, {
            onSuccess: () => {
    ...
    return (<Dialog.Root open={open} onOpenChange={onOpenChange}>
      ...
      <Dialog.Content>
        <div className='flex min-h-[100px] flex-col'>
          {isFetching ? (<LazyLoadingSpinner />) : apps.length > 0 ? (<div className='flex-1 space-y-4 py-1'>
              {apps.map((oauthApp) => (<div className='flex items-center gap-3' key={oauthApp.id}>
                  ...
                  <div className='flex gap-1'>
                    {threadOauthApps?.find((app) => app.id === oauthApp.id) ? (<Button variant='destructive' type='button' onClick={() => onRemoveOauthApp(oauthApp)}>
                        Remove from thread
                      </Button>) : (<Button variant='base' type='button' onClick={() => onAddOauthApp(oauthApp)}>
                        Add to thread
                      </Button>)}
                  </div>
                </div>))}
            </div>) : (<EmptyState message="Your organization doesn't have any integrations."/>)}
        </div>
      </Dialog.Content>

import { FormEvent, forwardRef, KeyboardEvent, memo, useCallback, useEffect, useImperativeHandle, useMemo, useRef, useState } from 'react';
...
import { getChatExtensions } from "../../../../packages/editor/src/index.ts";
import { MessageThread, OrganizationMember } from "../../../../packages/types/index.ts";
import { ArrowUpIcon, Button, CheckIcon, LayeredHotkeys, Tooltip } from "../../../../packages/ui/src/index.tsx";
import { cn, isMetaEnter } from "../../../../packages/ui/src/utils/index.ts";
import { EMPTY_HTML, EMPTY_JSON } from "../../atoms/markdown.ts";
import { EditorBubbleMenu } from "../EditorBubbleMenu/index.ts";
import { MentionList } from "../MarkdownEditor/MentionList.tsx";
import { ReactionList } from "../MarkdownEditor/ReactionList.tsx";
import { InlineRelativeTimeRenderer } from "../RichTextRenderer/handlers/RelativeTime.tsx";
import { ComposerActions } from "./ComposerActions.tsx";
import { useTypingIndicator } from "./hooks/useTypingIndicator.tsx";
import { useUnfocusedPaste } from "./hooks/useUnfocusedPaste.tsx";
import { MobileTypingIndicator, TypingIndicator } from "./TypingIndicator.tsx";
import { ADD_ATTACHMENT_SHORTCUT, ComposerEditorRef } from "./utils/index.ts";
import { useAlphaNumericKeyPress } from "../../hooks/useAlphaNumericKeyPress.ts";
...
import { GalleryThumbnail } from "../AttachmentLightbox/GalleryThumbnail.tsx";
import { attachmentsAtom, chatThreadPlacementAtom, editModeAtom, inReplyToAtom, removeAttachmentAtom, shouldMinimizeComposerActionsAtom } from "../Chat/atoms.tsx";
import { useControlClickLink } from "../MarkdownEditor/ControlClickLink.ts";
import { PostComposerRemoveButton } from "../PostComposer/PostComposerRemoveButton.tsx";
...
export const Composer = forwardRef<HTMLFormElement, Props>(function Composer({ children, thread, canSend, onMessage, onEditLastMessage, onScrollToBottom, onFocus, onBlur, autoFocus = true, id, onPaste, onUpload, dropzone }, ref) {
    ...
    const onEditorSubmit = useCallback(() => {
        ...
        if (!canSend)
            return false;
        if (!hasContent || !canSendAttachments)
            return false;
    ...
    const onEditorUpdate = useCallback((editor: CoreEditor) => {
    ...
    const onUnfocusedPaste = useCallback((event: React.ClipboardEvent<HTMLElement>) => {
        const text = event.clipboardData.getData('text/plain');
        if (text)
            return editorRef.current?.editor()?.chain().focus('end');
        onPaste(event);
    }, [onPaste]);
    ...
    const onKeyPress = useCallback((key: string) => {
        if (hasManuallyBlurredViaEscape.current)
            return;
    ...
    return (<>
      <LayeredHotkeys keys={ADD_ATTACHMENT_SHORTCUT} callback={dropzone.open} options={{ enableOnContentEditable: true, enableOnFormTags: true }}/>

      <MobileTypingIndicator channelName={thread?.channel_name} threadId={thread?.id}/>
      <form ref={ref} onSubmit={onSubmit} onPasteCapture={onPaste} className={cn('bg-primary relative z-10 flex flex-col gap-2 px-3 py-2', {
        ...
        {inReplyTo && (<ReplyContent content={inReplyTo.content} author={inReplyTo.sender.user} attachments={inReplyTo.attachments} onCancel={() => setInReplyTo(null)}/>)}

        <div className='flex items-end gap-2'>
          <ComposerActions editorRef={editorRef} onUpload={onUpload} dropzone={dropzone}/>

          <div className='flex flex-1 flex-col'>
            <div className={cn('relative flex min-h-[36px] flex-1 flex-col rounded-[18px]', 'bg-elevated border-1', dropzone.isDragActive &&
            'border-blue-500 bg-blue-50 ring-2 ring-blue-100 hover:bg-blue-100/20 dark:bg-blue-900/20 dark:ring-blue-600 dark:ring-opacity-20')} data-filled={hasContent}>
              <Attachments />

              <ComposerEditor ref={editorRef} initialValue={initialValue} autoFocus={autoFocus} id={id} channelName={thread?.channel_name} onUpdate={onEditorUpdate} onSubmit={onEditorSubmit} onArrowUp={onEditorArrowUp} onFocus={() => {
            hasManuallyBlurredViaEscape.current = false;
            onFocus?.();
        }} onBlur={onBlur} defaultMentions={thread?.other_members}/>

              <div className='absolute bottom-1 right-1 flex h-7 w-7 items-center justify-center'>
                <Button round variant={editMode ? 'important' : 'primary'} type='submit' iconOnly={editMode ? <CheckIcon size={24} strokeWidth='2'/> : <ArrowUpIcon size={22} strokeWidth='2.5'/>} accessibilityLabel='Send' disabled={!canSend || !hasContent || !canSendAttachments} onMouseDown={(e) => e.preventDefault()} className='h-7 w-7' tooltip='Send' tooltipShortcut='enter'/>
              </div>
            </div>
...
const Attachments = memo(function Attachments() {
    ...
    if (attachments.length === 0)
        return null;
    return (<ul className='flex flex-wrap items-end gap-1.5 px-3 pb-0.5 pt-2.5 lg:pb-0'>
      {attachments.map((attachment) => (<Tooltip key={attachment.id} label={attachment.name} delayDuration={0} align='start' side='top' alignOffset={16}>
          <li className={cn('group/remove-container bg-elevated relative grid w-20 max-w-[80px] rounded-xl after:absolute after:inset-0 after:rounded-xl after:border after:border-black/5 after:content-[""] dark:after:border-white/5', attachment.height > attachment.width && 'max-h-[10em]')}>
            <div className='max-h-full self-center overflow-hidden rounded-xl' style={{
            ...
            <PostComposerRemoveButton accessibilityLabel='Remove attachment' onClick={() => {
                if (!attachment.optimistic_id)
                    return;
...
const ComposerEditor = forwardRef<ComposerEditorRef, ComposerEditorProps>(function ComposerEditor(props, ref) {
    const { initialValue, autoFocus = true, id, channelName, defaultMentions, onUpdate, onSubmit, onArrowUp, onFocus, onBlur } = props;
    const threadPlacement = useAtomValue(chatThreadPlacementAtom);
    ...
    const editor = useEditor({
        ...
        editorProps: {
            attributes: {
                class: cn('scrollbar-hide break-words focus:ring-0 focus:outline-none !dark:bg-transparent bg-quaternary border-transparent !bg-transparent focus:border-transparent focus:outline-0 focus:ring-0 max-h-[60vh] text-base lg:text-sm chat-prose overflow-y-auto select-auto', 'pr-10 py-[6px] lg:py-[7px] pl-3')
            }
        },
        extensions,
    ...
    return (<>
      ...
      <EditorBubbleMenu editor={editor} enableHeaders={false} enableLists={false} enableBlockquote={false} enableUnderline={false} enableCodeBlock={false}/>
      <EditorContent id={id} editor={editor} onKeyUp={onKeyUp} onKeyDownCapture={onKeyDownCapture} onFocus={onFocus} onBlur={onBlur}/>
    </>);
});

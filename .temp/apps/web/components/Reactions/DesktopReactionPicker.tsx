...
import { cn, TextField } from "../../../../packages/ui/src/index.tsx";
...
import { formatReactionData, isStandardReaction, StandardReaction } from "../../utils/reactions/index.ts";
import { ALL_REACTION_CATEGORIES, getReactionCategoryLabel } from "../../utils/reactions/data.ts";
const COLUMNS = 9;
...
function isReactionRow(row: RowType): row is {
    type: 'reactions';
    size: number;
    reactions: (StandardReaction | SyncCustomReaction)[];
} {
...
export function DesktopReactionPicker({ showCustomReactions, onReactionSelect }: DesktopReactionPickerProps) {
    ...
    const { rows, stickyRowIndexes } = useMemo(() => {
        const categories = ALL_REACTION_CATEGORIES.map((categoryId) => {
            ...
            if (!showCustomReactions && categoryId === 'custom')
                return;
            // If we are searching, only show the `search` category
            if (query)
                return categoryId === 'frequent' ? { id: 'search', reactions: reactionSearchResults } : undefined;
            if (categoryId === 'frequent')
                return { id: 'frequent', reactions: frequentlyUsedReactions };
     ...
     */
    const setActivePosition = ({ row, column }: {
        row: number;
        column: number;
    }) => {
        ...
        if (!element)
            return;
        element.style.removeProperty(`--active-position-${activePositionRef.current.row}-${activePositionRef.current.column}`);
    ...
    const handleKeyDown = (e: React.KeyboardEvent) => {
         ...
         */
        if (!isUpMove(e) && !isDownMove(e) && !isLeftMove(e) && !isRightMove(e))
            return;
        /**
         * If we are at start position, allow left/right moves that are
         * contained within the input selection.
         */
        if (deepEqual(currentPosition, START_POSITION) && isLeftMove(e))
            return;
        if (deepEqual(currentPosition, START_POSITION) && isRightMove(e) && inputSelectionStart !== query.length)
            return;
        e.preventDefault();
        function handleArrowUp({ column }: {
            column: number;
        }) {
            ...
            const nextRowIndex = rows[currentPosition.row - 1]?.type === 'header' ? currentPosition.row - 2 : currentPosition.row - 1;
        ...
        function handleArrowDown({ column }: {
            column: number;
        }) {
    ...
    return (<div className='relative isolate flex h-full flex-col overflow-hidden focus:outline-0'>
      <div className='bg-elevated z-20 px-2 pt-2'>
        <TextField ref={inputRef} value={query} onChange={(value) => {
            setQuery(value);
            if (value.trim())
                setActivePosition(SEARCH_POSITION);
            else
                setActivePosition(START_POSITION);
        }} placeholder='Search' additionalClasses='bg-quaternary h-8 focus:bg-primary border-transparent rounded px-2' onKeyDown={handleKeyDown}/>
      </div>
        ...
        <div style={{
          ...
          {virtualizer.getVirtualItems().map((virtualRow) => {
            ...
            if (rowData.type === 'header') {
                return (<h2 key={virtualRow.index} className='bg-elevated text-secondary z-10 w-full whitespace-nowrap px-1.5 pb-1 pt-4 text-xs font-medium' style={{
                        position: 'absolute',
                        top: 0,
                        left: 0,
                        width: '100%',
            ...
            if (rowData.type === 'reactions') {
                return (<div key={virtualRow.index} className='grid w-full grid-flow-col grid-rows-1 place-content-start items-center' style={{
                        position: 'absolute',
                        top: 0,
                        left: 0,
                        width: '100%',
                  ...
                  {rowData.reactions.map((reaction, columnIndex) => (<button title={reaction.name} data-row={virtualRow.index} data-column={columnIndex} tabIndex={-1} key={reaction.id} className={cn('hover:!bg-quaternary flex aspect-square shrink-0 items-center justify-center rounded-md font-[emoji] text-xl leading-none transition-colors duration-0 ease-out')} style={{
                            width: REACTION_SIZE,
                            height: REACTION_SIZE,
                            backgroundColor: `var(--active-position-${virtualRow.index}-${columnIndex})`
                        }} onClick={() => handleReactionSelect(reaction)}>
                      {isStandardReaction(reaction) ? (<span className='mt-1'>{reaction.native}</span>) : (<Image className='h-5 w-5 object-contain' src={reaction.file_url ?? ''} alt={reaction.name} width={20} height={20}/>)}
                    </button>))}
                </div>);
            }
            return null;

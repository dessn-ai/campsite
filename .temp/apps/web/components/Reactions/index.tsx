...
import { GroupedReaction, SyncCustomReaction } from "../../../../packages/types/index.ts";
import { ANIMATIONS, Tooltip } from "../../../../packages/ui/src/index.tsx";
import { useEmojiMartData } from "../../hooks/reactions/useEmojiMartData.ts";
import { findEmojiMartEmoji, formatReactionName, StandardReaction } from "../../utils/reactions/index.ts";
interface Props {
      ...
      <MotionConfig transition={{ duration: 0.2 }}>
        <AnimatePresence initial={false}>
          <>
            ...
            return (<span key={reaction.emoji ?? reaction.custom_content?.id}>
                  <Tooltip key={reaction.emoji} label={<p className='max-w-[200px]'>
                        {reaction.tooltip}
                        {reactionLabel && (<>
                            {' '}
                    ...
                    <motion.button {...ANIMATIONS} onClick={() => {
                    if (reaction.emoji && emojiMartData) {
                      ...
                      {reaction.custom_content && (<Image className='mb-px h-[15px] w-[15px] object-contain' src={reaction.custom_content.file_url} alt={reaction.custom_content.name} width={16} height={16}/>)}

                      {reaction.reactions_count > 0 && (<span className='font-mono leading-none'>{reaction.reactions_count}</span>)}
                    </motion.button>
                  </Tooltip>
                </span>);
        })}

...
import { CONTAINER_STYLES, DismissibleLayer, Popover, PopoverContent, PopoverPortal as PopoverPortalPrimitive, PopoverTrigger, useBreakpoint, useControllableState } from "../../../../packages/ui/src/index.tsx";
...
import { StandardReaction } from "../../utils/reactions/index.ts";
import { DesktopReactionPicker } from "./DesktopReactionPicker.tsx";
import { MobileReactionPicker } from "./MobileReactionPicker.tsx";
...
export function ReactionPicker({ onReactionSelect, trigger, custom: showCustomReactions, onClose, open: openProp, onOpenChange: onOpenChangeProp, modal = true, align }: ReactionPickerProps) {
    ...
    return (<Root open={open} onOpenChange={onOpenChange} modal={modal}>
      ...
      <Portal>
        <>
          <Overlay />
          <DismissibleLayer>
            <Content align={align} asChild={isPopover} onCloseAutoFocus={(e) => {
            if (!onClose)
                return;
            e.preventDefault();
            onClose();
        ...
        : {})} style={isPopover
            ...
            : undefined} className={cn(isPopover && [
            ...
            'bg-elevated relative h-[424px] overflow-hidden border bg-clip-border dark:shadow-[0_0_0_1px_black]'
              ...
              {!isPopover ? (<MobileReactionPicker showCustomReactions={showCustomReactions} onReactionSelect={handleReactionSelect}/>) : (<DesktopReactionPicker showCustomReactions={showCustomReactions} onReactionSelect={handleReactionSelect}/>)}
            </Content>
          </DismissibleLayer>
        </>
      </Portal>

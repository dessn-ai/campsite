...
import { NoteSharePermissionsRow, Props } from "./NoteSharePermissions.tsx";
export function NotePeoplePermissions({ note, permissions }: Props) {
    ...
    return (<>
      <UIText size='text-xs' weight='font-medium' tertiary className={cn((note.viewer_can_edit || note.project_permission !== 'none') && 'mt-3')}>
        People with access
      </UIText>

      <ul className='flex flex-col gap-3'>
        {!note.viewer_is_author && (<li className='flex items-center justify-between'>
        ...
        {renderPermissions.map((permission) => (<NoteSharePermissionsRow key={permission.id} permission={permission} note={note}/>))}
      </ul>
    </>);
}

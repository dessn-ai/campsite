...
import { useDeleteNotePermission } from "../../hooks/useDeleteNotePermission.ts";
import { useGetCurrentUser } from "../../hooks/useGetCurrentUser.ts";
import { useUpdateNotePermission } from "../../hooks/useUpdateNotePermissions.ts";
import { NotePeoplePermissionSelect, permissionActionToLabel } from "./NotePeoplePermissionSelect.tsx";
...
export function NoteSharePermissionsRow({ permission, note }: NoteSharePermissionsRowProps) {
    ...
    return (<li className='flex items-center justify-between gap-3'>
      ...
      <div className='flex items-center gap-1'>
        {note.viewer_can_edit && permission.user.id !== currentUser?.id ? (<NotePeoplePermissionSelect allowNone={note.viewer_can_edit} selected={permission.action} onChange={(value) => {
                if (value === 'none') {
                    deletePermission({ noteId: note.id, permissionId: permission.id });
                    return;
                }
                else {
                    updatePermission({ noteId: note.id, permissionId: permission.id, permission: value }, {
                        onSuccess: () => {

...
import { Note, Permission } from "../../../../packages/types/index.ts";
...
import { useCreateNotePermissions } from "../../hooks/useCreateNotePermissions.ts";
import { useSyncedMembers } from "../../hooks/useSyncedMembers.ts";
import { apiErrorToast } from "../../utils/apiErrorToast.ts";
import { NotePeoplePermissionSelect, PERMISSION_ACTIONS } from "./NotePeoplePermissionSelect.tsx";
interface Props {
    note: Note;
...
export function NoteAddPersonPermission({ note, permissions }: Props) {
    ...
    function handleSubmit(e?: FormEvent<HTMLFormElement> | MouseEvent<HTMLButtonElement> | KeyboardEvent<HTMLDivElement>) {
        e?.preventDefault();
    ...
    return (<form onSubmit={handleSubmit} className='flex w-full flex-1 flex-col gap-2'>
      <OrganizationMemberMultiSelect onChange={handleChange} options={options} value={selectedOptions} className='flex-1 rounded-md border' placeholder='Share with specific people...' noOptionsMessage={() => 'Nobody found'} onInputChange={setQuery}/>
      {selectedOptions.length > 0 && (<div className='flex flex-1 flex-row justify-end gap-2'>
          <NotePeoplePermissionSelect allowNone={false} selected={selectedPermission} onChange={(value) => {

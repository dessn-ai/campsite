...
import { NoteFilledIcon, PostFilledIcon, VideoCameraFilledIcon, WarningTriangleIcon } from "../../../packages/ui/src/Icons/index.tsx";
...
import { useRerenderNodeViewOnBlur } from "./Post/Notes/useRerenderNodeViewOnBlur.ts";
import { useGetResourceMention } from "../hooks/useGetResourceMention.ts";
export function InlineResourceMentionRenderer(props: NodeViewWrapperProps) {
    ...
    return (<NodeViewWrapper as='span' className={cn({
            'rounded-md outline outline-2 outline-blue-500 focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-blue-500': props.editor.options.editable && props.selected && props.editor.isFocused
        }, '[.drag-node_&]:outline-none')} draggable={false} data-drag-handle={false}>
      <Link href={href}>
...
function Container({ children }: React.PropsWithChildren) {
    return (<span className='rounded bg-black/[0.04] decoration-clone py-0.5 pl-px pr-[3px] align-baseline text-[15px] hover:bg-black/[0.08] dark:bg-white/10 dark:hover:bg-white/[0.14]'>
      {children}
    </span>);
}
...
export function ResourceMentionView({ href }: {
    href: string;
}) {
...
export function ResourceMentionIcon({ type, size }: {
    type: 'call' | 'note' | 'post';
    size?: number;
}) {
    if (type === 'call') {

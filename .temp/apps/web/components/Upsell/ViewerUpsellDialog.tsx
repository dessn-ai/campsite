...
import { Avatar, Button, UIText } from "../../../../packages/ui/src/index.tsx";
...
export function ViewerUpsellDialog({ open, onOpenChange, icon, title }: Props) {
    ...
    return (<Dialog.Root open={open} onOpenChange={onOpenChange} size='xl' visuallyHiddenTitle={title} disableDescribedBy>
      <Dialog.Content>
        <div className='flex flex-col justify-center gap-4 px-1 pb-2 pt-6'>
          ...
          {admins && (<div className='bg-tertiary flex flex-col gap-3 rounded-lg p-4'>
              <UIText weight='font-medium'>Send an email to an admin to request a member role</UIText>
              {admins.map((member) => (<div className='flex-1 text-sm' key={member.id}>
                  <div className='flex items-center gap-3'>
                    ...
                    <Button href={`mailto:${member.user.email}?subject=Campsite member role request from ${currentUser?.display_name} (${currentUser?.email})&body=Hi ${member.user.display_name},%0D%0A%0D%0AI would like to request a member role on our team’s Campsite to write my own posts.%0D%0A%0D%0AHere's a link to manage team roles in our org settings: ${WEB_URL}/${organization?.slug}/people%0D%0A%0D%0AThanks!`} variant='primary'>
                      Email
                    </Button>
                  </div>
                </div>))}

...
import { AttachIcon, Button, CloseIcon, Link, PlanetIcon, TextField, UIText } from "../../../../packages/ui/src/index.tsx";
import * as Dialog from "../../../../packages/ui/src/Dialog/index.tsx";
import { isMetaEnter } from "../../../../packages/ui/src/utils/index.ts";
import { useScope } from "../../contexts/scope.tsx";
import { useCreateFeedback } from "../../hooks/useCreateFeedback.ts";
import { useGetCurrentOrganization } from "../../hooks/useGetCurrentOrganization.ts";
import { useIsCampsiteScope } from "../../hooks/useIsCampsiteScope.ts";
import { useStoredState } from "../../hooks/useStoredState.ts";
import { useFileUploadMutation } from "../../hooks/useUploadFile.ts";
import { apiErrorToast } from "../../utils/apiErrorToast.ts";
import { filesFromClipboardData } from "../../utils/filesFromClipboardData.ts";
import { transformFile } from "../../utils/transformFile.ts";
import { TransformedFile } from "../../utils/types.ts";
import { FeedbackFilePreview } from "./FeedbackFilePreview.tsx";
...
export function FeedbackDialog() {
    ...
    function onFileUploadComplete(file: TransformedFile, file_path: string | null, property: 'key' | 'preview_file_path') {
        setScreenshot({
            ...file,
            [property]: file_path
        });
    ...
    const { mutate: fileUploadMutation, isPending } = useFileUploadMutation(onFileUploadComplete, onFileUploadError, 'key');
    const formRef = useRef<HTMLFormElement>(null);
    const onPaste = useCallback(async (event: ClipboardEvent<HTMLFormElement>) => {
    ...
    const onDrop = useCallback(async (acceptedFiles: File[]) => {
        ...
        if (!acceptedFile) {
            return toast.error('Attachments must be an image (.jpg, .jpeg, .png) or video (.webm, .mp4, .mov) and be less than 100mb');
        }
    ...
    return (<div {...getRootProps()}>
      <Dialog.Root open={feedbackState.open} onOpenChange={setFeedbackDialogOpen} size='2xl' align={isMobile ? 'top' : 'center'} visuallyHiddenDescription='Share feedback, feature requests, or report a bug'>
        <Dialog.Header>
        ...
        <Dialog.Content>
          <form onPasteCapture={onPaste} ref={formRef} onSubmit={handleSubmit} className='flex flex-col gap-3 pt-0.5'>
            <TextField placeholder='Share feedback, feature requests, or report a bug...' multiline minRows={isMobile ? 5 : 6} maxRows={isMobile ? 5 : 12} autoFocus name='feedback' onKeyDownCapture={handleCommandEnter} value={feedbackState.value} onChange={setFeedbackValue}/>

            {screenshot && (<div className='flex flex-row items-center space-x-3'>
                <FeedbackFilePreview file={screenshot} reorderable={false} onRemove={() => setScreenshot(null)}/>
              </div>)}

            {!screenshot && (<div className='justify-self-start'>
        ...
        {showInsidersUpsell && (<div className='bg-secondary pb-4.5 hidden items-start gap-3 border-t p-3 lg:flex'>
            ...
            <div className='flex flex-col gap-3 pt-1'>
              ...
              <Button className='self-start' externalLink href='https://app.campsite.com/guest/5r72qd3xuW1x4EHwHpw4' variant='primary' onClick={() => setUpsellDismissed(true)}>
                Join insiders
              </Button>
            </div>
            <div className='ml-auto'>
              <Button variant='plain' size='sm' onClick={() => setUpsellDismissed(true)} iconOnly={<CloseIcon />} accessibilityLabel='Dismiss'/>
            </div>
          </div>)}

        <Dialog.Footer>
          <Dialog.LeadingActions>
            <Link target='_blank' rel='noopener noreferrer' href='https://twitter.com/trycampsite' className='text-tertiary hover:text-secondary group hidden items-center justify-center gap-2 px-2 hover:text-opacity-100 sm:flex'>
              <Image src='/img/services/twitter.png' width={16} height={16} className='rounded-md grayscale group-hover:grayscale-0' alt='Twitter logo'/>
          ...
          <Dialog.TrailingActions>
            ...
            <Button disabled={createFeedback.isPending || feedbackState.value == null || feedbackState.value.length === 0} loading={createFeedback.isPending || isPending} type='submit' variant='primary' onClick={handleSubmit}>
              Send feedback
            </Button>
          </Dialog.TrailingActions>
        </Dialog.Footer>

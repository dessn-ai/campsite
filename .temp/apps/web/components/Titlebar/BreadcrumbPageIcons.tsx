...
import { ChatBubbleIcon, HashtagIcon, HomeIcon, NoteFilledIcon, PostDraftIcon, PostFilledIcon, ProjectIcon, SquircleIconContainer, UIText, UserCircleFilledIcon, VideoCameraFilledIcon } from "../../../../packages/ui/src/index.tsx";
...
export function ProjectAccessoryBreadcrumbIcon({ project }: {
    ...
    return (<SquircleIconContainer className='text-transparent' size='small'>
      {showAccessory && (<UIText className='text-primary relative z-10 mt-0.5 font-["emoji"] text-[18px] leading-none'>
          {project?.accessory}
        </UIText>)}
      {!showAccessory &&
            (isChatProject ? (<ChatBubbleIcon className='text-primary' size={24}/>) : (<ProjectIcon className='text-primary' size={24}/>))}
    </SquircleIconContainer>);
}
...
export function PostBreadcrumbIcon() {
    return (<SquircleIconContainer className='text-gray-800 dark:text-gray-700' size='small'>

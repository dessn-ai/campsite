import { Comment, Post } from "../../../../packages/types/generated.ts";
...
import { commentComposerId } from "./CommentComposer.tsx";
import { useCommentLocalDraft } from "./hooks/useCommentLocalDraft.tsx";
...
export function CommentMobileReplyComposer({ post, comment, replyingToCommentId, setReplyingToCommentId }: CommentMobileReplyComposerProps) {
    ...
    return (<div className='relative isolate flex flex-1 scroll-m-32 gap-3 p-3'>
      <button onClick={toggleReplyComposer} className='absolute inset-0 z-50 cursor-text' disabled={comment.is_optimistic}/>
      ...
      <p className={cn('flex items-center gap-2', {
        ...
        {showAddReplyButton ? (<CommentReplyPlaceholder commentId={comment.id}/>) : (showCancelReplyButton && 'Cancel reply')}
      </p>
    </div>);
}

...
import { CheckIcon, ChevronSelectExpandIcon, ChevronSelectIcon, UIText } from "../../../../packages/ui/src/index.tsx";
interface CommentResolvedTombstoneProps {
    comment: Comment;
    showResolvedComment: boolean;
    setShowResolvedComment: (show: boolean) => void;
}
export function CommentResolvedTombstone({ comment, showResolvedComment, setShowResolvedComment }: CommentResolvedTombstoneProps) {
    return (<div className='flex p-1'>
      <button onClick={() => setShowResolvedComment(!showResolvedComment)} className='hover:bg-tertiary bg-primary dark:bg-secondary dark:hover:bg-tertiary group flex flex-1 items-center gap-3 rounded-md p-2'>

...
import { scrollCommentsIntoView } from "../InlinePost/InlinePostEngagements.tsx";
...
export function useAutoScrollComments({ onSidebarOpenChange, comments, canvasCommentId }: {
    ...
    const highlightComment = useCallbackRef((id: string, timeout: number = 3000) => {
        if (highlightedCommentHash.current === id)
            return;
    ...
    const handleHashChange = useCallbackRef(() => {
        ...
        if (scrolledIntoViewHash.current === window.location.hash)
            return;
...
export const CommentsList = ({ comments = [], timelineEvents = [], post, replyingToCommentId, setReplyingToCommentId }: Props) => {
    ...
    return (<div ref={ref} id='#comments' className={cn('flex flex-col-reverse', {
      ...
      {mixedItems.map((mixedItem) => {
            ...
            return (<CommentComponent key={comment.id} post={post} comment={comment} replyingToCommentId={replyingToCommentId} setReplyingToCommentId={setReplyingToCommentId}/>);
        })}
    </div>);
};

...
import { LayeredHotkeys } from "../../../../packages/ui/src/DismissibleLayer/LayeredHotkeys.tsx";
import { Switch } from "../../../../packages/ui/src/Switch/index.tsx";
...
export function CommentListHeader({ post, className }: CommentListHeaderProps) {
    ...
    return (<div id='comment-list-header' className={cn('flex scroll-mt-4 items-center gap-1.5', 'h-7.5', // style the same height as a <Button> to avoid layout shift when resolved comments are toggled
        className)}>
      ...
      {post?.viewer_is_organization_member && (<Switch label='Notifications' checked={post.viewer_has_subscribed} onChange={handleSubscribe}/>)}
    </div>);
}

...
import { LoadingSpinner } from "../../../../packages/ui/src/index.tsx";
...
import { MULTI_VALUE_PICKER_STYLES } from "../../utils/multiValuePickerStyles.ts";
export interface OrganizationMemberMultiSelectOptionType {
    value: string;
    label: string;
    member: SyncOrganizationMember;
}
export function organizationMemberToMultiSelectOption(member: SyncOrganizationMember): OrganizationMemberMultiSelectOptionType {
    return {
        value: member.id,
...
export function OrganizationMemberMultiSelect({ className, ...rest }: React.ComponentProps<typeof StateManagedSelect<OrganizationMemberMultiSelectOptionType, true>>) {
    return (<Select isMulti components={{ MultiValueContainer, Menu, LoadingIndicator, Option: CustomOption }} onKeyDown={handleKeyDown} className={cn('tag-picker', className)} styles={MULTI_VALUE_PICKER_STYLES} {...rest}/>);
}
...
const MultiValueContainer = (props: MultiValueGenericProps<OrganizationMemberMultiSelectOptionType>) => {
    ...
    return (<MotionConfig transition={{ duration: 0.1 }}>
      <AnimatePresence>
        <motion.div key={props.data.id} layout initial={{ opacity: 0.5, scale: 0.96 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0.5, scale: 0.96 }} style={{ originX: 0.5 }} className={cn(props.data.member && 'max-w-[150px] truncate')}>

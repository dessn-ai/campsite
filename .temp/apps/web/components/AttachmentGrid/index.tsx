...
import { LottieAttachment } from "../Thread/Bubble/AttachmentCard/LottieAttachment.tsx";
import { useVideoTimestamp } from "../../hooks/useVideoTimestamp.ts";
...
function MediaThumbnail({ attachment, imageClassName = '', containerClassName = '', index = 0, mediaCount = 0, isSingle = false, hideVideoTimestamp = false, autoPlayVideo = true, setSelectedPostAttachmentId }: {
    ...
    return (<div className={cn('group relative', borderRadiusClassName(index, mediaCount), {
            'row-span-2': index === 2 && mediaCount === 3
        }, containerClassName)}>
      <ConditionalWrap 
    // a single video should just display the player and not link to the post
    condition={!(isSingle && attachment.video)} wrap={(c) => <button onClick={() => setSelectedPostAttachmentId(attachment.id)}>{c}</button>}>
        <>
          {(attachment.image || attachment.gif) && (<img src={src} alt={attachment.name ?? ''} className={cn(imageClassName, 'm-0 object-top', borderRadiusClassName(index, mediaCount), {
          ...
          {attachment.video && (<video ref={videoRef} muted autoPlay={autoPlay} playsInline key={src} controls={showVideoControls} preload='auto' className={cn('focus:online-none focus:border-0 focus-visible:outline-none', borderRadiusClassName(index, mediaCount), {
          ...
          {attachment.video && !isSingle && !hideVideoTimestamp && (<div className='absolute bottom-0 left-0 font-mono'>
...
export function AttachmentGrid({ postId, attachments, autoPlayVideo }: AttachmentGalleryProps) {
    ...
    return (<>
      ...
      <div className={cn('relative w-full', {
            'ring-neutral-150 bg-neutral-150 grid auto-rows-fr gap-px rounded-md ring-1 dark:bg-neutral-700/50 dark:ring-neutral-700/50': mediaCount >= 2,
        ...
        {mediaToDisplay.map((media, index) => (<MediaThumbnail key={media.id} attachment={media} index={index} mediaCount={mediaCount} isSingle={mediaCount === 1} autoPlayVideo={autoPlayVideo} setSelectedPostAttachmentId={setSelectedPostAttachmentId}/>))}
        {hasMore && (<div className='absolute bottom-0 right-0'>
...
function AttachmentAccessory({ children }: React.PropsWithChildren) {
    return (<div className='m-1 flex h-6 items-center justify-center rounded-md bg-black px-2 text-xs font-bold text-white'>

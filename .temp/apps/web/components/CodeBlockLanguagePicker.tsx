...
import { ALIAS_TO_LANGUAGE } from "../../../packages/editor/src/index.ts";
import { CheckIcon, ChevronDownIcon, cn, CONTAINER_STYLES, Popover, PopoverContent, PopoverElementAnchor, PopoverPortal, SelectCommandContainer, SelectCommandEmpty, SelectCommandGroup, SelectCommandInput, SelectCommandItem, SelectCommandList, SelectCommandSeparator, SelectOption } from "../../../packages/ui/src/index.tsx";
interface Props {
...
export function CodeBlockLanguagePicker({ editor }: Props) {
    ...
    <div className='absolute'>
      <button ref={setElement} className='text-quaternary hover:text-secondary flex translate-y-full flex-row items-center px-2 py-1 text-xs' onClick={() => setOpen(true)} 
    ...
    style={{ visibility: 'hidden' }} type='button'>
      ...
      <Popover open={open} onOpenChange={(open) => {
        ...
        <PopoverPortal>
          <PopoverContent className={cn('scrollable min-w-[365px] max-w-[365px]', CONTAINER_STYLES.base)} side='bottom' align='end' onKeyDown={() => setInitialOpen(false)} asChild>
            ...
            return (<SelectCommandItem className={cn('justify-between', {
                    '!bg-white/10': initialOpen && isSelected,
                    '!bg-transparent !shadow-none': initialOpen && !isSelected
                })} key={`${option.value}${option.label}`} value={option.label.toString()} title={option.label.toString()} onSelect={() => {
                    editor
                        ?.chain()
                        .focus()

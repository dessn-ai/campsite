...
import { SearchCall, SearchMixedItem, SearchNote, SearchPost } from "../../../../packages/types/index.ts";
import { Button, CloseIcon, LayeredHotkeys, LazyLoadingSpinner, NoteFilledIcon, SearchIcon, UIText, VideoCameraFilledIcon } from "../../../../packages/ui/src/index.tsx";
...
import { PeopleIndexMemberRow } from "../People/PeopleIndexMemberRow.tsx";
import { ProjectTag } from "../ProjectTag.tsx";
import { SearchHighlights } from "./SearchHighlights.tsx";
import { SearchResult } from "./SearchResult.tsx";
import { SearchResultPostItem } from "./SearchResultPostItem.tsx";
import { BreadcrumbTitlebar, BreadcrumbTitlebarContainer } from "../Titlebar/BreadcrumbTitlebar.tsx";
import { useScope } from "../../contexts/scope.tsx";
import { SelectGroupItemFn, useGroupedListNavigation } from "../../hooks/useListNavigation.ts";
import { useSearchMixed } from "../../hooks/useSearchMixed.ts";
import { useSearchOrganizationMembers } from "../../hooks/useSearchOrganizationMembers.ts";
import { flattenInfiniteData } from "../../utils/flattenInfiniteData.ts";
import { addRecentSearchAtom, RecentSearches, recentSearchesAtom } from "./RecentSearches.tsx";
export function SearchIndex() {
    ...
    const showEmptyState = !isLoading && !!query && (isPeopleFocusWithoutResults || isOtherFocusWithoutResults || isNoFocusWithoutAnyResults);
    return (<IndexPageContainer>
      ...
      <IndexPageContent>
        {!isLoading && (<>
            {showRecent && (<RecentSearches recentSearches={recentSearches} onFocus={(index) => selectItem({ itemIndex: index, groupIndex: 0 })} onPointerMove={(index) => selectItem({ itemIndex: index, scroll: false, groupIndex: 0 })}/>)}

            {showEmptyState && (<EmptyState message='Nothing found' icon={<SearchIcon size={72} className='text-quaternary opacity-50'/>}/>)}

            {showPeople && (<div className='flex flex-col'>
                {(!focus || !query) && (<div className='flex items-center gap-4 py-2'>
                ...
                <ul className='@container -mx-2 flex flex-col gap-px py-2'>
                  {members.map((member, index) => (<PeopleIndexMemberRow id={getItemRowDOMId({ id: member.id })} member={member} key={member.id} onFocus={() => selectItem({ itemIndex: index, groupIndex: 1 })} onPointerMove={() => selectItem({ itemIndex: index, scroll: false, groupIndex: 1 })}/>))}
                </ul>
              </div>)}

            {!!getSearch.data?.items.length && focus !== 'people' && (<div>
                {showPeople && (<div className='flex items-center gap-4 py-2'>
                    <UIText weight='font-medium' tertiary>
                      Top results
                    </UIText>
                    <div className='flex-1 border-b'/>
                  </div>)}
                <SearchResultsList items={getSearch.data.items} callMap={getSearch.data.callsMap} postMap={getSearch.data.postsMap} noteMap={getSearch.data.notesMap} selectItem={selectItem}/>
              </div>)}
          </>)}
      </IndexPageContent>
    </IndexPageContainer>);
...
export function SearchField({ query, isLoading, mobile }: SearchFieldProps) {
    ...
    return (<>
      ...
      <form onSubmit={onSubmit} onKeyDownCapture={(e) => {
        ...
        <div className={cn('relative flex flex-1 items-center gap-2', {
          ...
          <input ref={ref} className='flex-1 border-none bg-transparent p-0 text-sm outline-none ring-0 focus:ring-0' placeholder='Search...' role='searchbox' autoComplete='off' autoCorrect='off' spellCheck={false} type='text' autoFocus={!query} value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => {
            if (e?.key === 'Escape') {
                if (text)
                    return setText('');
                if (!text)
          ...
          <Button round iconOnly={<CloseIcon size={16} strokeWidth='2'/>} onClick={() => {
            setText('');
            ref.current?.focus();
        }} href={`/${scope}/search`} onMouseDown={(e) => e.preventDefault()} accessibilityLabel='Clear search' variant='plain' className={cn('absolute right-2 top-1/2 h-6 w-6 -translate-y-1/2', {
...
export function FilterButton({ focus, children, fullWidth }: PropsWithChildren & {
    focus?: string;
    fullWidth?: boolean;
}) {
    const router = useRouter();
    ...
    return (<Button size={fullWidth ? 'base' : 'sm'} onClick={() => router.replace({ query: focus ? { ...query, f: focus } : query }, undefined, { shallow: true })} variant={isFocused ? 'flat' : 'plain'} fullWidth={fullWidth}>
      {children}
    </Button>);
}
export const getItemRowDOMId = (item: {
    id: string;
}) => `search-item-${item.id}`;
function SearchResultsList({ items, callMap, postMap, noteMap, selectItem }: {
    ...
    return (<ul className='@container -mx-2 flex flex-col gap-px py-2'>
      {items.map((item, index) => (<SearchResultItem key={item.id} item={item} callMap={callMap} postMap={postMap} noteMap={noteMap} onFocus={() => selectItem({ itemIndex: index, groupIndex: 2 })} onPointerMove={() => selectItem({ itemIndex: index, scroll: false, groupIndex: 2 })}/>))}
    </ul>);
}
...
function CallItem({ call, highlights, titleHighlight, ...rest }: ItemProps & {
    call: SearchCall;
}) {
    const { scope } = useScope();
    return (<SearchResult href={`/${scope}/calls/${call.id}`} id={call.id} className={!highlights?.length ? 'items-center' : 'items-start'} {...rest}>
...
function NoteItem({ note, highlights, titleHighlight, ...rest }: ItemProps & {
    note: SearchNote;
}) {
    const { scope } = useScope();
    return (<SearchResult className={!highlights?.length ? 'items-center' : 'items-start'} href={`/${scope}/notes/${note.id}`} id={note.id} {...rest}>
      ...
      <div className='flex flex-1 items-center gap-3'>
        ...
        {note.project && (<div className='hidden items-center gap-1 self-start pt-0.5 md:flex'>

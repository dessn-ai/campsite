...
import { Badge, cn, ConditionalWrap, FollowUpTag, HighlightedCommandItem, UIText } from "../../../../packages/ui/src/index.tsx";
...
import { PostTldrPopover } from "../Post/PostTldrPopover.tsx";
...
import { getPostFallbackTitle } from "../../utils/project.ts";
...
export const CompactPost = memo(({ post, display = 'default', hideProject = false }: CompactPostProps) => {
    ...
    const linkTarget = !unreadPost && post.latest_comment_path && !post.viewer_is_latest_comment_author
    ...
    return (<div className='@xl:px-4 group relative flex min-h-12 items-center gap-3 rounded-lg p-3'>
      <PostOverflowMenu type='context' post={post} onTldrOpen={() => setTldrOpen(true)}>
        <HighlightedCommandItem className='absolute inset-0 z-0' value={encodeCommandListSubject(post, { href: linkTarget, pinned: display === 'pinned' })} onSelect={handleSelect}/>
      </PostOverflowMenu>

      <div className='mt-0.5 flex items-start self-start'>
        <ConditionalWrap condition={post.viewer_is_organization_member && !isIntegration} wrap={(c) => (<MemberHovercard username={post.member.user.username as string}>
      ...
      <div className='@xl:flex-row @xl:items-center @xl:gap-3 flex flex-1 flex-col-reverse items-start gap-0.5'>
            ...
            <div className='flex flex-1 flex-col gap-0.5'>
              <div className='flex flex-shrink items-center'>
                ...
                <AnimatePresence initial={false}>
                  {unreadPost && (<m.div initial={{ opacity: 0, marginRight: -10 }} animate={{ opacity: 1, marginRight: 10 }} exit={{ opacity: 0, marginRight: -10 }} className='h-2.5 w-2.5 flex-none rounded-full bg-blue-500'/>)}
                </AnimatePresence>

                {title && (<UIText primary weight='font-medium' className='break-anywhere mr-2 line-clamp-1 text-[15px]'>
                    {title}
                  </UIText>)}
              </div>
              ...
              <div className='flex items-center'>
                ...
                {display !== 'pinned' && hasComments && (<span className={cn('h-4.5 mr-2 mt-px flex items-center justify-center self-start rounded px-1.5 text-[10px] font-semibold uppercase', {
                ...
                {description && (<UIText tertiary className='break-anywhere line-clamp-1 flex-1'>
                    {description}
                  </UIText>)}
        ...
        {!hideProject && (<div className='@xl:self-center self-start'>
...
function VersionBadge({ post }: {
    post: Post;
}) {
    if (!post.has_parent)
        return null;
    return (<Badge className={cn('mr-1.5 shrink-0 rounded font-mono', {
            relative: !isMobile
        })}>
      v{post.version}
    </Badge>);
}
function DescriptionStatusBadges({ post }: {
    post: Post;
}) {
    ...
    if (!viewersFeedbackRequest && !anyFeedbackRequest && !viewerFollowUp && !post.resolution)
        return null;
    return (<span className='mr-1.5 flex items-center gap-1.5'>

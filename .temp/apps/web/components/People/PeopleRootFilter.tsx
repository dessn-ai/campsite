...
export function PeopleRootFilter() {
    ...
    if (!viewerIsAdmin)
        return null;
    return (<div className='ml-2 flex flex-1 items-center gap-1'>
      ...
      <Button size='sm' tooltip='Active' tooltipShortcut='1' onClick={() => setRootFilter('active')} variant={rootFilter === 'active' ? 'flat' : 'plain'}>
        Active
      </Button>
      <Button size='sm' tooltip='Invited' tooltipShortcut='2' onClick={() => setRootFilter('invited')} variant={rootFilter === 'invited' ? 'flat' : 'plain'}>
        Invited
      </Button>
      <Button size='sm' tooltip='Deactivated' tooltipShortcut='3' onClick={() => setRootFilter('deactivated')} variant={rootFilter === 'deactivated' ? 'flat' : 'plain'}>
        Deactivated
      </Button>
    </div>);
}

import { OrganizationMember, SyncOrganizationMember } from "../../../../packages/types/index.ts";
import { Button, UIText } from "../../../../packages/ui/src/index.tsx";
import * as Dialog from "../../../../packages/ui/src/Dialog/index.tsx";
import { useScope } from "../../contexts/scope.tsx";
import { useDeactivateOrganizationMember } from "../../hooks/useDeactivateOrganizationMember.ts";
...
export function DeactivateMemberDialog({ member, open, onOpenChange }: Props) {
    ...
    async function handleOnRemove() {
        await deactivateMemberMutation.mutate({ id: member.id }, {
            onSuccess() {
                onOpenChange(false);
            }
        });
    }
    return (<Dialog.Root open={open} onOpenChange={onOpenChange} size='sm' disableDescribedBy>
      <Dialog.Header>
      ...
      <Dialog.Content>
        <div className='flex flex-col gap-3'>
          <UIText tertiary>{`Deactivating ${member.user.display_name} will keep their post history, but they will no longer have access to the ${currentOrganization?.name} organization.`}</UIText>
      ...
      <Dialog.Footer>
        <Dialog.TrailingActions>
          ...
          <Button variant='destructive' onClick={handleOnRemove} disabled={deactivateMemberMutation.isPending} autoFocus>

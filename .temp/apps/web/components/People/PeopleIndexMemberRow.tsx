...
import { Badge, Link, UIText } from "../../../../packages/ui/src/index.tsx";
...
export function PeopleIndexMemberRow({ id, member, onFocus, onPointerMove, children }: {
    id?: string;
    ...
    return (<li className={cn('group relative flex items-center gap-3 rounded-md py-2 pl-3 pr-1.5', 'data-[state="open"]:bg-tertiary', {
      ...
      <Link id={id} onFocus={onFocus} onPointerMove={onPointerMove} href={`/${scope}/people/${member.user.username}`} className='absolute inset-0 z-0 rounded-lg focus:ring-0'/>
      ...
      <div className='pointer-events-none flex flex-1 items-center gap-3'>
        ...
        <div className='flex flex-col'>
          ...
          {!isCommunity && (<UIText tertiary className='line-clamp- truncate'>
              {member.user.email}
            </UIText>)}
        </div>
      </div>

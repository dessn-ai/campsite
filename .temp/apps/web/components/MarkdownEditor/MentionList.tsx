...
import { Mention } from "../../../../packages/editor/src/extensions/index.ts";
import { OrganizationMember, SyncOrganizationMember } from "../../../../packages/types/generated.ts";
...
import { SuggestionItem, SuggestionRoot, useSuggestionEmpty } from "../SuggestionList.tsx";
...
export function MentionList({ editor, defaultMentions, modal }: Props) {
    return (<SuggestionRoot modal={modal} editor={editor} char='@' allow={({ state, range }) => {
            ...
            return allow;
        }} minScore={0.2} contentClassName='p-0 max-h-[min(480px,var(--radix-popover-content-available-height))] w-[min(350px,var(--radix-popover-content-available-width))]' listClassName='p-1'>
...
function InnerMentionList({ editor, defaultMentions }: Pick<Props, 'editor' | 'defaultMentions'>) {
    ...
    return items.map((member) => (<SuggestionItem key={member.id} editor={editor} value={member.user.username} keywords={[member.user.display_name !== member.user.username ? member.user.display_name : '']} 
    // downrank guest members
    scoreModifier={member.role === 'guest' ? 0.3 : 1} onSelect={({ editor, range }) => editor.commands.insertMention({
            range,
            id: member.id,
      ...
      <MemberAvatar displayStatus member={{
            user: member.user
        }} size='sm'/>

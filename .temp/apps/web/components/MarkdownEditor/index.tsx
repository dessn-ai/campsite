...
import { BlurAtTopOptions, focusAtStartWithNewline, getMarkdownExtensions, MediaGallery, MediaGalleryItem } from "../../../../packages/editor/src/index.ts";
import { Attachment } from "../../../../packages/types/generated.ts";
...
import { InlineResourceMentionRenderer } from "../InlineResourceMentionRenderer.tsx";
import { MentionList } from "./MentionList.tsx";
import { ReactionList } from "./ReactionList.tsx";
import { ResourceMentionList } from "./ResourceMentionList.tsx";
import { useCreateLinkAttachment } from "./useCreateLinkAttachment.tsx";
import { MediaGalleryRenderer } from "../Post/MediaGalleryRenderer.tsx";
import { SlashCommand } from "../Post/Notes/SlashCommand.tsx";
...
import { setsAreEqual } from "../../utils/setsAreEqual.ts";
import { CodeBlockLanguagePicker } from "../CodeBlockLanguagePicker.tsx";
import { EditorBubbleMenu } from "../EditorBubbleMenu/index.ts";
import { LinkUnfurlRenderer } from "../Post/LinkUnfurlRenderer.tsx";
import { useUploadNoteAttachments } from "../Post/Notes/Attachments/useUploadAttachments.ts";
import { PostInlineAttachmentRenderer } from "../Post/PostInlineAttachmentRenderer.tsx";
import { useControlClickLink } from "./ControlClickLink.ts";
import { useEditorFileHandlers } from "./useEditorFileHandlers.ts";
...
const MarkdownEditor = forwardRef<MarkdownEditorRef, Props>((props, ref) => {
    const { id = '', content, placeholder, onClick, onChangeDebounced, onChangeDebounceMs = 300, onEmptyDidChange, autoFocus = false, isSubmitted, minHeight = '48px', maxHeight, textSize = 'base', defaultMentions, onInlineAttachmentsChange, containerClasses = 'px-3 py-2.5', enableInlineAttachments = false, enableInlineLinks = false, enableSyntaxHighlighting = false, initialAttachments, appendBubbleMenuTo, onBlurAtTop, disableSlashCommand = false, disableMentions = false, disableReactions = false } = props;
    ...
    const editor = useEditor({
        ...
        editorProps: {
            attributes: {
                id: id,
                class: cn('prose editing focus:outline-none w-full max-w-full overflow-hidden select-auto', {
                    'text-sm': textSize === 'sm'
                }, containerClasses),
    ...
    return (<>
      ...
      <EditorContent className='flex flex-1' editor={editor} onClick={onClick} onFocus={props.onFocus} onBlur={props.onBlur} onPaste={onPaste} onDrop={onDrop}/>
      <div className={cn('mx-auto h-[2px] max-w-[44rem] bg-blue-500', {

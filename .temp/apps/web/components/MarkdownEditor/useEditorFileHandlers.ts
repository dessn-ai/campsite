...
export function useEditorFileHandlers({ editor, enabled = true, upload }: FileDropProps) {
    ...
    const handleDrop = useCallback((editor: TTEditor, props: DropProps) => {
        if (!editor)
            return false;
    ...
    const onDrop = useCallback((event: DragEvent<HTMLDivElement>) => {
        ...
        if (editor.view.dragging && !(isMacOs ? event.altKey : event.ctrlKey))
            return;
        handleDrop(editor, event);
    ...
    const uploadAndAppendAttachments = useCallback(async (files: File[]) => {
        if (!editor || !enabled)
            return;

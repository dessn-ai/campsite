...
import { UsersMeNotificationSchedulePutRequest } from "../../../../packages/types/generated.ts";
...
export function NotificationScheduleDayButtons() {
    const { setValue, watch, formState: { errors } } = useFormContext<NotificationScheduleFormSchema>();
...
function DayButton({ children, onClick, isActive }: {
    children: React.ReactNode;
    onClick: () => void;
    isActive: boolean;
}) {
    return (<Button onClick={onClick} variant={isActive ? 'important' : 'base'} className='h-8 w-8 text-xs' round>
      {children}
    </Button>);
}

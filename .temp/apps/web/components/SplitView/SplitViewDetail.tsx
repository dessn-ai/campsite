...
import { useIsSplitViewAvailable } from "./hooks/index.ts";
import { debouncedSelectedSplitViewSubjectAtom, selectedSplitViewSubjectAtom } from "./utils/index.ts";
...
export function SplitViewDetail({ fallback, fallbackWidth = '0px' }: SplitViewDetailProps) {
    ...
    return (<>
      ...
      <AnimatePresence initial={false}>
        {show && (<m.div className='bg-primary z-10 flex flex-col overflow-hidden max-lg:hidden' transition={{ duration: 0.2 }} {...(debouncedSelectedSplitViewSubject
            ? {
            ...
            {debouncedSelectedSplitViewSubject ? (<m.div className='flex flex-1 flex-col overflow-hidden' initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                {debouncedSelectedSplitViewSubject?.subjectType === 'post' && (<PostView postId={debouncedSelectedSplitViewSubject.id}/>)}
                {debouncedSelectedSplitViewSubject?.subjectType === 'note' && (<NoteView noteId={debouncedSelectedSplitViewSubject.id}/>)}
                {debouncedSelectedSplitViewSubject?.subjectType === 'call' && (<CallView callId={debouncedSelectedSplitViewSubject.id}/>)}
              </m.div>) : fallback ? (<m.div className='flex flex-1 flex-col overflow-hidden' initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                {fallback}
              </m.div>) : null}
          </m.div>)}
      </AnimatePresence>

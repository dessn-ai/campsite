...
import { CheckIcon, Link, RelativeTime, Tooltip, UIText } from "../../../../packages/ui/src/index.tsx";
import { cn } from "../../../../packages/ui/src/utils/index.ts";
import { AttachmentLightbox } from "../AttachmentLightbox/index.tsx";
import { CommentResolvedTombstone } from "../Comments/CommentResolvedTombstone.tsx";
import { enumerateCommentElements } from "../Comments/CommentsList.tsx";
...
import { getImmediateScrollableNode } from "../../utils/scroll.ts";
import { scrollInputIntoView } from "../../utils/scrollInputIntoView.ts";
import { longTimestamp } from "../../utils/timestamp.ts";
import { hoveredCanvasCommentAtom } from "../CanvasComments/CanvasComment.tsx";
import { CommentDescription } from "../Comments/CommentDescription.tsx";
import { CommentEngagements } from "../Comments/CommentEngagements.tsx";
import { CommentHoverActions } from "../Comments/CommentHoverActions.tsx";
import { KeepInView } from "../KeepInView.tsx";
import { commentElement } from "../Post/Notes/CommentRenderer.tsx";
import { activeNoteEditorAtom } from "../Post/Notes/types.ts";
interface Props {
    comment: Comment;
    note: Note;
...
export function CommentComponent({ comment, note, hideAttachment = false, highlightPopover = false }: Props) {
    ...
    if (!showResolvedComment && comment.resolved_at) {
        return (<CommentResolvedTombstone comment={comment} showResolvedComment={showResolvedComment} setShowResolvedComment={setShowResolvedComment}/>);
    }
    return (<div className='pb-3'>
      <div className='relative'>
        <div className={cn('dark:before:border-gray-750 before:absolute before:-bottom-2.5 before:left-6 before:top-12 before:block before:w-3.5 before:rounded-bl-2xl before:border-b before:border-l', {
            'before:-bottom-2 before:rounded-bl-none before:border-b-0': highlightPopover
        })}/>
        <CommentDetails comment={comment} note={note} hideAttachment={hideAttachment} highlightPopover={highlightPopover}/>
        {!!mixedItems.length && (<div className='pl-9'>
            {mixedItems.map((mixedItem) => {
                if (mixedItem.type === 'timeline-event') {
                    const timelineEvent = mixedItem as TimelineEvent;
                    return (<div key={timelineEvent.id} className='-ml-3 pb-4 pt-1'>
      ...
      <div className={cn('scroll-m-32 pl-9', { 'pb-4': isReplyComposerVisible })} id={replyComposerContainerId}>
        <div className='px-3'>
          <div className='flex gap-2'>
            {!isReplyComposerVisible && !comment.parent_id && !highlightPopover && (<button className='text-tertiary hover:text-primary text-sm font-medium' onClick={showReplyComposer} disabled={comment.is_optimistic}>
                Write a reply
              </button>)}

            <KeepInView className='flex-1' disabled={!isReplyComposerVisible}>
              <NoteCommentComposer autoFocus placeholder='Write a reply...' closeComposer={() => setIsReplyComposerVisible(false)} onSubmitting={() => setIsReplyComposerVisible(false)} noteId={note.id} replyingToCommentId={comment?.id} open={isReplyComposerVisible} onCreated={(comment) => {
            setTimeout(() => {
...
const CommentDetails = (props: CommentProps) => {
    ...
    return (<>
      <AttachmentLightbox subject={note} selectedAttachmentId={openAttachmentId} onClose={() => setOpenAttachmentId(undefined)} onSelectAttachment={({ id }) => setOpenAttachmentId(id)}/>
    ...
    */
    data-comment-id={highlightPopover ? undefined : `#comment-${comment.id}`} className={cn('initial:gap-3 group isolate flex flex-1 scroll-mt-1 p-3 transition-colors duration-150', {
            'gap-2 pt-1': isReply
        })} onMouseOver={highlightPopover ? () => setHoveredCanvasCommentId(comment.id) : undefined} onMouseLeave={highlightPopover ? () => setHoveredCanvasCommentId(undefined) : undefined}>
            ...
            <span>
              <Link href={`/${scope}/people/${comment.member.user.username}`} className='relative'>
                <>
                  ...
                  {comment.resolved_at && (<Tooltip label={`Resolved by ${comment.resolved_by?.user.display_name}`}>
          ...
          <>
            <div className='relative z-50 -mb-1 -mt-1.5 flex items-center'>
              <div className='flex-1 flex-col space-x-1.5 truncate align-baseline leading-none'>
                ...
                <Link href={`/${scope}/notes/${note.id}#comment-${comment.id}`} className={cn('text-tertiary hover:text-primary text-sm', {
            'pointer-events-none': comment.is_optimistic
              ...
              <div className='min-h-[34px]'>
                {!isEditing && (<CommentHoverActions comment={comment} subjectId={note.id} subjectType='note' isEditing={isEditing} setIsEditing={setIsEditing} isInCanvas={highlightPopover}/>)}
              </div>
            </div>
            ...
            <div className='space-y-2'>
              <div className='flex justify-between gap-4'>
                <div className='-mt-1.5 flex max-w-full flex-1 flex-col gap-2'>
                  {!!comment.note_highlight && !highlightPopover && (<button className='relative mt-1 select-auto text-start' onClick={() => {
                if (!activeNodeEditor)
                    return;
                ...
                if (markIsOutOfView && commentMarkElement instanceof HTMLElement) {
                    ...
                    scrollableParent.scrollTo({
                      ...
                      <span className='prose text-secondary line-clamp-3 pl-3 leading-normal before:absolute before:bottom-0 before:left-0 before:top-0 before:w-[3px] before:rounded-full before:bg-yellow-300 dark:before:bg-yellow-500' dangerouslySetInnerHTML={{ __html: comment.note_highlight }}></span>
                    </button>)}

                  <CommentDescription subjectId={note.id} subjectType='Note' isEditing={isEditing} setIsEditing={setIsEditing} comment={comment} isReply={isReply}/>
                </div>
                {!isEditing && comment.attachment_thumbnail_url && !hideAttachment && !highlightPopover && (<div className='flex-none pt-1'>
                    {comment.attachment_id && (<button className='group/canvas-comment relative flex rounded-lg' onClick={() => setOpenAttachmentId(comment.attachment_id ?? undefined)}>
...
const Reply = ({ reply, note }: ReplyProps) => {
    return (<div className='relative rounded-md transition-colors duration-150' id={`#comment-${reply.id}`}>
      <svg className='dark:text-gray-750 absolute -left-3 top-2.5 text-gray-200' width='13' height='13' viewBox='0 0 13 13' fill='none' xmlns='http://www.w3.org/2000/svg'>

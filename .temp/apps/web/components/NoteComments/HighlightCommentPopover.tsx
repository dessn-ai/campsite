...
import { ActiveEditorComment } from "../../../../packages/editor/src/index.ts";
import { Comment, Note } from "../../../../packages/types/index.ts";
import { Button, CONTAINER_STYLES, Popover, PopoverContent, PopoverElementAnchor, PopoverPortal } from "../../../../packages/ui/src/index.tsx";
...
import { cleanFragmentForHighlight, commentElement } from "../Post/Notes/CommentRenderer.tsx";
...
export function HighlightCommentPopover(props: Props) {
    ...
    const noteHighlight = useMemo(() => {
        if (!editor || !activeComment || !activeComment.newCommentRange)
            return;
    ...
    const onServerCreate = useCallback((data: Comment) => {
        if (!isNewComment) {
    ...
    return (<>
      <Dialog.Root open={dismissDialogOpen} onOpenChange={setDismissDialogOpen} size='lg' visuallyHiddenTitle='Discard comment'>
        <Dialog.Header>
      ...
      <Popover open={open} onOpenChange={(open) => {
        ...
        <PopoverPortal>
          <PopoverContent className={cn('scrollable min-w-[365px] max-w-[365px]', CONTAINER_STYLES.base)} avoidCollisions side='top' align='center' onCloseAutoFocus={(evt) => {
            evt.preventDefault();
            editor.chain().focus().run();
        }} sideOffset={8} addDismissibleLayer>
            {open && (<div key={activeComment.id} className={cn('bg-elevated flex w-full flex-col rounded-xl shadow-lg shadow-black/20 ring-1 ring-black/[0.04] dark:shadow-[inset_0px_1px_0px_rgba(255,255,255,0.04),_0px_2px_12px_rgba(0,0,0,0.4),_0px_0px_0px_1px_rgba(0,0,0,0.8)] dark:ring-white/[0.02]', 'origin-[--radix-popover-content-transform-origin]')}>
                ...
                'border-t': !isNewComment
                  ...
                  <NoteCommentComposer autoFocus={isNewComment} noteId={noteId} noteHighlight={noteHighlight} onCreated={onServerCreate} onSubmitting={() => setSubmitting(true)} onEmptyChange={setIsEmpty} replyingToCommentId={!isNewComment ? activeComment?.id : undefined} display='inline' placeholder={isNewComment ? 'Write a comment...' : 'Write a reply...'}/>
                </div>
              </div>)}
          </PopoverContent>
        </PopoverPortal>
...
function FetchingCommentComponent({ commentId, note }: {
    commentId: string;
    note: Note;
}) {

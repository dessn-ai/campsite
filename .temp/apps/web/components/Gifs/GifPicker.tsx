...
import { Gif } from "../../../../packages/types/index.ts";
import { CONTAINER_STYLES, Popover, PopoverContent, PopoverPortal as PopoverPortalPrimitive, PopoverTrigger, TextField, useBreakpoint, useControllableState } from "../../../../packages/ui/src/index.tsx";
import { cn } from "../../../../packages/ui/src/utils/index.ts";
import { InfiniteLoader } from "../InfiniteLoader.tsx";
import { useDownloadGif } from "../../hooks/useDownloadGif.ts";
import { useGetGifs } from "../../hooks/useGetGifs.ts";
...
export function GifPicker({ trigger, onClose, onGifSelect, open: openProp, onOpenChange: onOpenChangeProp }: GifPickerProps) {
    ...
    return (<Root open={open} onOpenChange={onOpenChange} modal>
      ...
      <Portal>
        <>
          <Overlay />
          <Content asChild={isPopover} onCloseAutoFocus={(e) => {
            if (!onClose)
                return;
            e.preventDefault();
            onClose();
        ...
        : {})} className={cn(isPopover && [
            ...
            'bg-elevated fixed inset-x-0 bottom-0 h-[424px] rounded-t-xl focus:outline-none focus:ring-0')} style={isPopover
            ...
            <div className='relative isolate flex h-full flex-col overflow-hidden focus:outline-0'>
              {!isPopover && (<div className='mx-auto mt-2 h-1 w-8 shrink-0 rounded-full bg-[--text-primary] opacity-20'/>)}
              <div className='bg-elevated z-20 px-2 py-2'>
                <TextField value={query} onChange={(value) => {
            setQuery(value);
        }} 
    /**
     ...
     */
    placeholder='Search Tenor' additionalClasses={cn('bg-quaternary focus:bg-primary border-transparent', {
            ...
            'h-10 rounded-lg px-3 text-base': !isPopover
              ...
              <div className='grid grid-cols-3 gap-1 overflow-y-scroll md:w-[320px]'>
                {gifs?.map((gif) => (<button className='contents' key={gif.id} onClick={() => handleGifSelect(gif)}>
                    <Image unoptimized className='h-fulls aspect-square w-full object-cover' draggable={false} src={gif.url} width={gif.width} height={gif.height} alt={gif.description}/>
                  </button>))}

                <div className='col-span-3'>
                  <InfiniteLoader hasNextPage={!!getGifs.hasNextPage} isError={!!getGifs.isError} isFetching={!!getGifs.isFetching} isFetchingNextPage={!!getGifs.isFetchingNextPage} fetchNextPage={getGifs.fetchNextPage}/>
                </div>
              </div>
            </div>
          </Content>

...
import { Command } from "../../../../packages/ui/src/Command/Command.tsx";
import { SearchIcon } from "../../../../packages/ui/src/Icons/index.tsx";
...
export function ProjectsManagement({ query, setQuery, initialProjectIds, addedProjectIds, onAddedProjectIdsChange, removedProjectIds, onRemovedProjectIdsChange }: MemberProjectsManagementProps) {
    ...
    function onSelectProject(project: SyncProject) {
        ...
        if (initialProjectIds?.has(project.id)) {
            if (!removedProjectIds || !onRemovedProjectIdsChange)
                return;
    ...
    return (<Command className='flex min-h-[30dvh] flex-1 flex-col overflow-hidden' loop>
      <div className='flex items-center gap-3 border-b px-3'>
        <div className='flex h-6 w-6 items-center justify-center'>
          <SearchIcon className='text-quaternary'/>
        </div>
        <Command.Input autoFocus placeholder='Search channels...' value={query} onValueChange={setQuery} className='w-full border-0 bg-transparent py-3 pl-0 pr-4 text-[15px] placeholder-gray-400 outline-none focus:border-black focus:border-black/5 focus:ring-0' onKeyDownCapture={onKeyDownCapture}/>
      </div>
      ...
      <Command.List className='scrollbar-hide overflow-y-auto'>
        <Command.Group className='p-3'>
          ...
          {projects?.sort(initiallySelectedProjectsFirst).map((project) => (<HighlightedCommandItem key={project.id} onClick={() => onSelectProject(project)} onSelect={() => onSelectProject(project)} className='h-10 gap-3 rounded-lg'>

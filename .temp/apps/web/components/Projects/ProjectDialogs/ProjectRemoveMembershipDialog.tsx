...
import { Project, SyncUser } from "../../../../../packages/types/generated.ts";
...
export function ProjectRemoveMembershipDialog({ project, user, open, onOpenChange }: RemoveProjectMembershipDialogProps) {
    const router = useRouter();
    const { scope } = useScope();
    const { data: currentUser } = useGetCurrentUser();
    const deleteProjectMembershipMutation = useDeleteProjectMembership(project.id);
    const handleDelete = () => {
        deleteProjectMembershipMutation.mutate({ user: user }, {
            onSuccess: () => {
                if (currentUser?.id === user.id && project.private)
    ...
    return (<Dialog.Root open={open} onOpenChange={onOpenChange}>
      ...
      <Dialog.Footer>
        <Dialog.TrailingActions>
          ...
          <Button variant='destructive' onClick={handleDelete} disabled={deleteProjectMembershipMutation.isPending} autoFocus>

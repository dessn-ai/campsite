...
export function ProjectRemoveLastPrivateProjectMembershipDialog({ project, open, onOpenChange }: ProjectRemoveLastPrivateProjectMembershipDialogProps) {
    ...
    return (<Dialog.Root open={open} onOpenChange={onOpenChange} size='sm'>
      ...
      <Dialog.Footer>
        ...
        <Dialog.TrailingActions>
          <Button variant='flat' className='hover:bg-red-500 hover:text-white dark:hover:bg-red-500 dark:hover:text-white' onClick={() => deleteProjectMutation.mutate(project.id, {
            onSuccess: handleCleanup
        })}>
            Leave & Delete
          </Button>
          {!project.archived && (<Button variant='primary' onClick={() => archiveProjectMutation.mutate(project.id, {
                onSuccess: handleCleanup
            })}>
              Archive
            </Button>)}

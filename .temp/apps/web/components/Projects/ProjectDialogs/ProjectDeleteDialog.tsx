...
export function ProjectDeleteDialog({ project, open, onOpenChange }: ProjectDeleteDialogProps) {
    ...
    const preventDelete = project.posts_count > 0 && deleteMatch.trim().toLowerCase() !== project.name.trim().toLowerCase();
    ...
    return (<Dialog.Root open={open} onOpenChange={onOpenChange} size='sm'>
      ...
      {project.posts_count > 0 && (<Dialog.Content>
      ...
      <Dialog.Footer>
        <Dialog.TrailingActions>
          ...
          <Button variant='destructive' onClick={() => deleteProjectMutation.mutate(project.id, {

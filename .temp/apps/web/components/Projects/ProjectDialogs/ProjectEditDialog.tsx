...
import { Project, SyncCustomReaction } from "../../../../../packages/types/index.ts";
import { Button, Checkbox, LockIcon, MinusIcon, MutationError, PlusIcon, ProjectIcon, RadioGroup, RadioGroupItem, TextField, UIText } from "../../../../../packages/ui/src/index.tsx";
...
import { SlackBroadcast } from "../SlackBroadcast.tsx";
...
export function ProjectEditDialog({ open, onOpenChange, project }: ProjectEditDialogProps) {
    ...
    if (!project.viewer_can_update) {
        return (<ViewerUpsellDialog open={open} onOpenChange={onOpenChange} icon={<ProjectIcon size={28}/>} title='Channel editing is available to members'/>);
    }
    return (<Dialog.Root open={open} onOpenChange={onOpenChange} size='lg' disableDescribedBy>
      <Dialog.Header>
          ...
          <div className='flex flex-col gap-3'>
            <div className='flex w-full items-end gap-2 pt-2.5'>
              <div className='group/accessory relative'>
                {accessory && (<button onClick={handleRemoveAccessory} className='text-primary bg-elevated absolute -right-1 -top-1 flex h-3.5 w-3.5 items-center justify-center rounded-full opacity-0 shadow ring-1 ring-gray-200 hover:bg-red-500 hover:text-white hover:ring-red-500 group-hover/accessory:opacity-100 dark:bg-gray-600 dark:ring-gray-700 dark:hover:bg-red-500'>
                    <MinusIcon size={16} strokeWidth='2.5'/>
                  </button>)}
                {!accessory && (<span className='text-primary pointer-events-none absolute -right-1 -top-1 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-white shadow ring-1 ring-gray-200 group-hover/accessory:bg-blue-500 group-hover/accessory:text-white group-hover/accessory:ring-blue-500 dark:bg-gray-600 dark:ring-gray-700'>
                ...
                <ReactionPicker trigger={<div className='dark:bg-quaternary hover:bg-quaternary text-secondary hover:text-primary hover:border-primary group flex h-8 w-8 cursor-pointer list-none items-center justify-center rounded-md border p-1 text-xs font-medium'>
                      {accessory ? (<UIText className='font-["emoji"]' size='text-base'>
                          {accessory}
                        </UIText>) : project.private ? (<LockIcon />) : (<ProjectIcon />)}
                    </div>} onReactionSelect={handleReactionSelect}/>
              </div>

              <div className='w-full'>
                <TextField type='text' value={name} onChange={setName} placeholder='Channel name' maxLength={32} minLength={2} onCommandEnter={handleSubmit}/>
              </div>
            </div>
            <TextField type='text' value={description ?? undefined} onChange={(value) => setDescription(value)} placeholder='Description (optional)' maxLength={280} minLength={2} minRows={3} maxRows={6} multiline onCommandEnter={handleSubmit}/>

            {!project.personal && (<SlackBroadcast open={open} isAdmin={viewerIsAdmin} slackChannelId={slackChannelId} setSlackChannelId={setSlackChannelId} setSlackChannelIsPrivate={setSlackChannelIsPrivate}/>)}
            ...
            <div className='text-secondary flex w-full items-start gap-2'>
              ...
              <div className='flex-1'>
                ...
                {showVisibility ? (<div className='items-center gap-1 pt-2'>
                    <RadioGroup loop aria-label='Visibility' value={isPrivate ? 'private' : 'public'} className='flex flex-col gap-3' orientation='vertical' onValueChange={(value) => {
                ...
                if (value === 'private')
                    setIsDefault(false);
                      ...
                      <RadioGroupItem id='public' value='public'>
                        ...
                        {showDefault && (<div className='text-secondary mt-2 w-full gap-2'>
                ...
                isPrivate &&
                (noMembers ? (<div className='mt-2 justify-center gap-2 rounded-lg bg-red-50 p-2.5 text-red-900 dark:bg-red-300/10 dark:text-red-200'>
                              <UIText inherit>Private channels must have at least one member.</UIText>
                            </div>) : (<div className='mt-2 justify-center gap-2 rounded-lg bg-amber-50 p-2.5 text-amber-900 dark:bg-amber-300/10 dark:text-amber-200'>
                              <UIText inherit>
            ...
            {updateProject.isError && (<div className='flex flex-col text-sm text-red-500'>
      ...
      <Dialog.Footer>
        <Dialog.TrailingActions>
          ...
          <Button variant='primary' onClick={handleSubmit} disabled={!name || updateProject.isPending || (isPrivate && noMembers)}>

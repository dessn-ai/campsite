...
export function SlackBroadcast({ isAdmin, slackChannelId, setSlackChannelId, setSlackChannelIsPrivate, open }: Props) {
    ...
    if (!canShow)
        return null;
    return (<div className='flex w-full flex-col gap-1.5 pb-2'>
      <UIText secondary size='text-xs' weight='font-medium'>
        Cross-post to Slack
      </UIText>
      {hasIntegrationWithScopes ? (<SlackChannelPicker onChange={(channel) => {
                setSlackChannelId(channel?.id ?? null);
                setSlackChannelIsPrivate(!!channel?.is_private);
            }} includeSlackIcon activeId={slackChannelId}/>) : (<ConnectSlackButton href={slackBroadcastsAuthorizationUrl}/>)}

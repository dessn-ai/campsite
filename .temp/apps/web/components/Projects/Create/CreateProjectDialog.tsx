...
import { Project, SyncCustomReaction, SyncOrganizationMember } from "../../../../../packages/types/index.ts";
import { Button, ChatBubbleFilledIcon, LockIcon, MinusIcon, PlusIcon, PostFilledIcon, ProjectIcon, RadioGroup, RadioGroupItem, Switch, TextField, UIText } from "../../../../../packages/ui/src/index.tsx";
import * as Dialog from "../../../../../packages/ui/src/Dialog/index.tsx";
import { OrganizationMemberMultiSelect, OrganizationMemberMultiSelectOptionType, organizationMemberToMultiSelectOption } from "../../OrganizationMember/OrganizationMemberMultiSelect.tsx";
...
import { isStandardReaction, StandardReaction } from "../../../utils/reactions/index.ts";
interface Props {
    open: boolean;
...
const projectComposerAtom = atom((get) => ({
    step: get(stepAtom),
    name: get(nameAtom),
...
type Action = {
    ...
    type: 'set-members';
    payload: SyncOrganizationMember[];
} | {
    type: 'reset';
};
export function CreateProjectDialog({ open, onOpenChange, onCreate }: Props) {
    ...
    function handleOpenChange(value: boolean) {
        ...
        if (!!state.name && !value)
            return setShowDiscardWarning(true);
        // reset if closing
        if (!value)
            dispatch({ type: 'reset' });
        onOpenChange(value);
    ...
    if (!currentOrganization?.viewer_can_create_project) {
        return (<ViewerUpsellDialog open={open} onOpenChange={onOpenChange} icon={<ProjectIcon size={28}/>} title='Channel creation is available to members'/>);
    }
    const dialogDescription = state.step === 'name'
        ? 'Channels keep your team’s posts organized. Use them to group conversations by project, team, or topic. '
        : state.name;
    return (<>
      <Dialog.Root open={showDiscardWarning} size='sm' 
    // force user to click cancel or discard
    onOpenChange={() => undefined} disableDescribedBy>
        <Dialog.Header>
...
function FormatOption({ icon, title, description, active, onChange }: {
    ...
    return (<label htmlFor={`format-${title}`} className='dark:bg-quaternary flex cursor-pointer flex-col items-center justify-center gap-0.5 rounded-lg border px-2 py-3 text-center focus-within:ring-2 focus-within:ring-blue-100 has-[:checked]:border-blue-500 dark:focus-within:ring-blue-600/40'>
      <input type='radio' checked={active} onChange={(e) => onChange(e.target.checked)} className='sr-only' name='format' id={`format-${title}`}/>
...
function NameStep() {
    ...
    return (<div className='flex flex-col gap-4 pt-2.5'>
      <div className='flex w-full items-start gap-2.5'>
        <div className='group/accessory relative'>
          {state.accessory && (<button onClick={() => dispatch({ type: 'set-accessory', payload: '' })} className='text-primary bg-elevated absolute -right-1 -top-1 flex h-3.5 w-3.5 items-center justify-center rounded-full opacity-0 shadow ring-1 ring-gray-200 hover:bg-red-500 hover:text-white hover:ring-red-500 group-hover/accessory:opacity-100 dark:bg-gray-600 dark:ring-gray-700 dark:hover:bg-red-500'>
              <MinusIcon size={16} strokeWidth='2.5'/>
            </button>)}

          {!state.accessory && (<span className='text-primary pointer-events-none absolute -right-1 -top-1 flex h-3.5 w-3.5 items-center justify-center rounded-full bg-white shadow ring-1 ring-gray-200 group-hover/accessory:bg-blue-500 group-hover/accessory:text-white group-hover/accessory:ring-blue-500 dark:bg-gray-600 dark:ring-gray-700'>
              <PlusIcon size={13} strokeWidth='2.5'/>
            </span>)}
          <ReactionPicker trigger={<div className='dark:bg-quaternary hover:bg-quaternary text-secondary hover:text-primary hover:border-primary group flex h-8 w-8 cursor-pointer list-none items-center justify-center rounded-md border p-1 text-xs font-medium'>
                {state.accessory ? (<UIText className='font-["emoji"]' size='text-base'>
                    {state.accessory}
                  </UIText>) : state.private ? (<LockIcon />) : (<ProjectIcon />)}
              </div>} onReactionSelect={handleReactionSelect}/>
        </div>
        <div className='flex flex-1 flex-col gap-2'>
          <div className='flex-1'>
            <TextField type='text' minLength={2} maxLength={32} autoFocus={!state.name} value={state.name} placeholder='Channel name' onChange={(payload) => dispatch({ type: 'set-name', payload })} onKeyDownCapture={handleKeyDown}/>
          </div>
          {typeof state.description === 'undefined' && (<button onClick={() => dispatch({ type: 'set-description', payload: '' })} className='text-tertiary hover:text-primary px-2 text-left'>
              <UIText inherit size='text-xs'>
                + Add description (optional)
              </UIText>
            </button>)}
          {typeof state.description !== 'undefined' && (<div className='flex-1'>
              <TextField type='text' autoFocus={!!state.name && !state.description} multiline minRows={2} maxRows={2} maxLength={280} value={state.description} placeholder='Add description (optional)' onChange={(payload) => dispatch({ type: 'set-description', payload })} onKeyDownCapture={handleKeyDown}/>
            </div>)}
          {hasChatChannels && (<div>
              <div className='grid grid-cols-2 gap-2 pt-2'>
                <FormatOption icon={<PostFilledIcon className='text-secondary' size={24}/>} title='Posts' description='Topic-based discussions' active={state.format === 'posts'} onChange={() => dispatch({ type: 'set-format', payload: 'posts' })}/>
                <FormatOption icon={<ChatBubbleFilledIcon className='text-secondary' size={24}/>} title='Chat' description='Real-time conversations' active={state.format === 'chat'} onChange={() => dispatch({ type: 'set-format', payload: 'chat' })}/>
              </div>
            </div>)}
          {!isCommunity && (<div className='flex items-center gap-1 pt-2'>
              <RadioGroup loop aria-label='Channel visibility' value={state.private ? 'private' : 'public'} className='flex flex-col gap-3' orientation='vertical' onValueChange={(value) => {
...
function InviteStep() {
    ...
    return (<div className='flex flex-col gap-3'>
      ...
      <div className='min-h-10'>
        {!state.inviteAll && (<OrganizationMemberMultiSelect autoFocus={state.members.length === 0 && state.private} openMenuOnFocus value={value} options={options} onChange={handleChange} className='bg-quaternary flex-1 rounded-md border' placeholder={state.private ? 'Invite people...' : 'Add people...'} loadingMessage={() => 'Searching people...'} noOptionsMessage={() => 'Nobody found'}/>)}
...
function AllSpecificToggle() {
    ...
    if (state.private)
        return null;
    if (userIsOnlyOrgMember)
        return null;
    if (isCommunity)
        return null;
    return (<RadioGroup loop aria-label='Default channel membership' defaultValue={state.inviteAll ? 'all' : 'specific'} className='flex flex-col gap-3' orientation='vertical' onValueChange={(value) => dispatch({ type: 'set-invite-all', payload: value === 'all' ? true : false })}>
      <RadioGroupItem id='all' value='all'>
        <UIText secondary>
          Add {allOrgMembers.length > 1 && 'all'} {pluralize('member', allOrgMembers.length, true)} of{' '}
          <span className='text-primary font-semibold'>{currentOrganization?.name}</span>
        </UIText>
        {members.some((member) => member.role === 'guest') && (<UIText size='text-xs' tertiary>
            Excludes guests
          </UIText>)}
      </RadioGroupItem>
...
function DefaultProjectToggle() {
    ...
    if (!viewerIsAdmin)
        return null;
    return (<div className='bg-tertiary flex flex-col gap-1 rounded-lg p-3'>
...
function InviteActions({ onCreate }: {
    onCreate?: (project: Project) => void;
}) {
    const [state, dispatch] = useAtom(projectComposerAtom);
    const { members: allOrgMembers } = useSyncedMembers();

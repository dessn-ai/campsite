...
import { LinkIcon } from "../../../../../packages/ui/src/index.tsx";
interface Props {
    url: string;
    title: string;
}
export function BookmarkFavicon(props: Props) {
    ...
    if (!src)
        return <LinkIcon />;
    return (<Image className='pointer-events-none h-[18px] w-[18px] flex-none rounded-md' src={src} width={20} height={20} draggable={false} alt={`Favicon for ${props.title}`}/>);
}

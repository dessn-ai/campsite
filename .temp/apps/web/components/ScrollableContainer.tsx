 ...
 */
export function ScrollableContainer({ className, children, disableScrollRestoration = false, disableStableGutter = false, ...props }: ScrollableContainerProps) {
    const ref = useRef<HTMLDivElement>(null);
    useScrollRestoration(ref, { enabled: !disableScrollRestoration });
    return (<div ref={ref} className={cn('flex w-full flex-1 flex-col overflow-y-auto', 'focus:outline-none focus:ring-0', !disableStableGutter && '[scrollbar-gutter:stable]', className)} {...props}>
      {children}
    </div>);
}

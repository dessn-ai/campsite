...
import { IntegrationTeam } from "../../../packages/types/index.ts";
import { AlertIcon, LinearIcon, LockIcon, Select, SelectOption, SelectTrigger, SelectValue } from "../../../packages/ui/src/index.tsx";
import { useCreateLinearTeamSync } from "../hooks/useCreateLinearTeamSync.ts";
...
export function LinearTeamPicker({ onChange, activeId, onKeyDownCapture }: LinearTeamPickerProps) {
    ...
    if (getTeams.error) {
        return (<div className='flex items-center justify-start space-x-1 text-sm'>
        <AlertIcon />
        <span>Unable to load your organization’s Linear teams</span>
      </div>);
    }
    return (<TeamPicker activeId={activeId} teams={getTeams.data ?? []} onChange={onChange} onKeyDownCapture={onKeyDownCapture} onOpenChange={handleOpenChange} disabled={getTeams.isLoading}/>);
}
...
function TeamPicker(props: TeamPickerProps) {
    ...
    return (<Select typeAhead disabled={props.disabled} options={options} value={activeTeam?.value ?? ''} onChange={handleOnChange} onOpenChange={onOpenChange}>

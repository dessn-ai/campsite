...
import { Link, RelativeTime, UIText } from "../../../../packages/ui/src/index.tsx";
...
export function ExistingThreadListItem({ thread, isSelected = false }: Props) {
    ...
    return (<Link href={`/${thread.organization_slug}/chat/${thread.id}`} className={cn('dark:focus:bg-tertiary group relative flex w-full flex-none scroll-mt-12 items-center gap-3 rounded-lg py-3 pl-2 pr-4 focus-within:border-none focus-within:outline-none focus-within:ring-0 focus:border-none focus:outline-none focus:ring-0', {
            'bg-tertiary': isSelected,
            'hover:bg-tertiary': !isSelected
        })} onMouseOver={() => setHovering(true)} onMouseOut={() => setHovering(false)} onClick={(e) => {
      ...
      <div className='flex flex-1 flex-col'>
        ...
        <UIText inherit className={cn('break-anywhere -mb-0.5 line-clamp-1 min-w-0 flex-1 break-all', {

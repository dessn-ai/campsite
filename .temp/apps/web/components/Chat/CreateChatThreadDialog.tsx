...
import { MessageThread, OauthApplication, SyncOrganizationMember } from "../../../../packages/types/index.ts";
import { Avatar, Button, ChevronRightIcon, Command, PlusIcon, RadioGroup, RadioGroupItem, SearchIcon, TextField, UIText } from "../../../../packages/ui/src/index.tsx";
import { HighlightedCommandItem } from "../../../../packages/ui/src/Command/index.ts";
import * as Dialog from "../../../../packages/ui/src/Dialog/index.tsx";
import { isMetaEnter } from "../../../../packages/ui/src/utils/index.ts";
...
import { useCreateThread } from "../../hooks/useCreateThread.ts";
...
import { useGetOauthApplications } from "../../hooks/useGetOauthApplications.ts";
...
const createGroupChatAtom = atom((get) => ({
    step: get(step),
    title: get(titleAtom),
...
export function CreateChatThreadDialog({ open, onOpenChange }: Props) {
    ...
    return (<Dialog.Root open={open} onOpenChange={handleOpenChange} size='lg' align='top' disableDescribedBy>
      <Dialog.Header>
        <Dialog.Title>{title}</Dialog.Title>
      </Dialog.Header>

      {open && state.step === 'dm' && (<DMStep onSelect={() => handleOpenChange(false)} onCancel={() => handleOpenChange(false)}/>)}

      {open && state.step === 'group' && (<GroupStep onSuccess={(thread) => {
                handleOpenChange(false);
                router.push(`/${scope}/chat/${thread.id}`);
            }}/>)}
    </Dialog.Root>);
}
function DMStep({ onSelect, onCancel }: {
    onSelect: () => void;
    onCancel: () => void;
}) {
    const router = useRouter();
    ...
    return (<>
      <Command className='bg-elevated flex min-h-[30dvh] flex-1 flex-col overflow-hidden p-0 px-2' loop>
        <button className='hover:bg-quaternary focus:bg-quaternary group mb-2 flex items-center gap-3 rounded-lg px-2 py-2 text-left focus-within:ring-0' onClick={() => dispatch({ type: 'set-step', payload: 'group' })}>
          ...
          <UIText className='group-hover:text-primary group-focus-within:text-primary flex-1 text-blue-600 dark:text-blue-400' inherit>
            New group
          </UIText>
        ...
        {!isOnlyOrgMember && (<div className='-mx-2 flex items-center gap-3 border-b px-4'>
            <div className='flex h-6 w-6 items-center justify-center'>
              <SearchIcon className='text-quaternary'/>
            </div>
            <Command.Input autoFocus placeholder='Search people...' value={query} onValueChange={setQuery} className='w-full border-0 bg-transparent py-3 pl-0 pr-4 text-[15px] placeholder-gray-400 outline-none focus:border-black focus:border-black/5 focus:ring-0' onKeyDownCapture={(e) => {
                e.stopPropagation();
                if (e.key === 'Escape')
        ...
        <Command.List className='scrollbar-hide overflow-y-auto py-2'>
          ...
          {[...nonGuests, ...guests]?.map((member) => (<HighlightedCommandItem key={`${member.user.display_name}-${member.user.username}`} onClick={() => onSelectMember(member)} onSelect={() => onSelectMember(member)} className='h-10 gap-3 rounded-lg'>
          ...
          {messageableApps.map((app) => (<HighlightedCommandItem key={app.id} onClick={() => onSelectOauthApplication(app)} onSelect={() => onSelectOauthApplication(app)} className='h-10 gap-3 rounded-lg'>
    ...
    return (<>
      <Dialog.Content className='overflow-visible pt-1'>
        <div className='flex flex-col gap-5'>
          <TextField type='text' minLength={2} maxLength={32} autoFocus={!state.title} value={state.title} placeholder='Group name (optional)' onChange={(payload) => dispatch({ type: 'set-title', payload })} helpText='Name this group to make it easy for people to recognize.' onKeyDownCapture={onKeyDownCapture}/>

          <div className='flex flex-col gap-3'>
            {isOnlyOrgMember && (<div className='bg-tertiary flex flex-col gap-2 rounded-lg p-3'>
                <UIText secondary>
            ...
            {!isOnlyOrgMember && (<RadioGroup loop aria-label='Default channel membership' defaultValue={state.visibility} className='flex flex-col gap-3' orientation='vertical' onValueChange={(payload: Visibility) => dispatch({ type: 'set-visibility', payload })}>
                <div className='flex flex-col gap-1.5'>
                  <RadioGroupItem id='specific' value='specific'>
                    <UIText secondary>Add specific people</UIText>
                  </RadioGroupItem>
                  {state.visibility === 'specific' && (<div className='min-h-10 pl-8'>
                      <OrganizationMemberMultiSelect openMenuOnFocus value={value} options={options} onChange={handleChange} className='bg-quaternary flex-1 rounded-md border' placeholder='Add people...' loadingMessage={() => 'Searching people...'} noOptionsMessage={() => 'Nobody found'}/>
                    </div>)}
                </div>
                ...
                {state.visibility === 'personal' && (<div className='-mt-3 pl-8'>
      ...
      <Dialog.Footer>
        ...
        <Dialog.TrailingActions>
          <Button disabled={state.visibility === 'specific' && !state.members.length} variant='primary' onClick={handleSubmit} loading={createThread.isPending}>
            Create
          </Button>
        </Dialog.TrailingActions>
      </Dialog.Footer>

...
import { Suggestion, SuggestionOptions } from "../../../packages/editor/src/extensions/Suggestion.ts";
import { isMarkActiveInRange, isNodeActiveInRange, isRemoteTransaction, recreateTransform } from "../../../packages/editor/src/index.ts";
import { cn, Command, CommandProps, CommandRef, CONTAINER_STYLES, highlightedCommandItemStyles, Popover, PopoverAnchor, PopoverContent, PopoverPortal, UIText } from "../../../packages/ui/src/index.tsx";
...
type SuggestionProps = React.PropsWithChildren & Omit<React.ComponentPropsWithoutRef<'div'>, 'className'> & Pick<SuggestionOptions, 'editor' | 'pluginKey' | 'allow' | 'startOfLine' | 'allowedPrefixes' | 'allowSpaces'> & Pick<React.ComponentPropsWithoutRef<typeof Popover>, 'modal'> & Pick<React.ComponentPropsWithoutRef<typeof PopoverContent>, 'align' | 'side'> & Pick<CommandProps, 'filter' | 'minScore'> & {
    // force non-null char
    char: string;
...
export function SuggestionRoot(props: SuggestionProps) {
    ...
    const { children, editor, pluginKey, char, contentClassName, listClassName, startOfLine, allow, allowedPrefixes = DEFAULT_ALLOWED_PREFIXES, allowSpaces, align = 'start', side = 'bottom', 
    /**
     ...
     */
    modal = true, filter, onControlledQueryChange, minScore, ...etc } = props;
    ...
    return (<Popover open={open} onOpenChange={(open) => {
      ...
      <PopoverPortal>
        {open && (<PopoverContent side={side} align={align} 
        // retain focus on the editor
        onOpenAutoFocus={(e) => e.preventDefault()} className={cn(CONTAINER_STYLES.base, 'scrollbar-hide bg-elevated dark:border-primary-opaque flex max-h-60 min-w-[265px] scroll-p-1 flex-col gap-0.5 overflow-y-auto overflow-x-hidden rounded-[9px] border border-neutral-400/40 p-1 shadow-md outline-none focus:outline-none dark:shadow-[0px_0px_0px_0.5px_rgba(0,0,0,1),_0px_4px_4px_rgba(0,0,0,0.24)]', contentClassName)} avoidCollisions='autoPlacement' asChild>
            <Command ref={commandRef} shouldFilter={!onControlledQueryChange} loop 
        // disable default cmdk layered hotkeys so the editor keydown events control the component
        manualInputs minScore={minScore} filter={filter}>
              <Command.Input 
        // HACK to use cmdk with controlled input, just don't render the element
        value={query} className='hidden'/>
              <Command.List className={cn(CONTAINER_STYLES.base, 'flex flex-col outline-none focus:outline-none', listClassName)} {...etc}>
                ...
                <Command.Empty className='flex items-center gap-2 p-2 text-sm' onClick={() => {
                close();
...
type ItemProps = React.PropsWithChildren & Omit<React.ComponentPropsWithoutRef<typeof Command.Item>, 'onSelect'> & {
    editor: Editor;
    onSelect: (props: {
        editor: Editor;
        range: Range;
...
export function SuggestionItem({ children, editor, onSelect, className, ...etc }: ItemProps) {
    ...
    return (<Command.Item {...etc} className={cn(highlightedCommandItemStyles(), 'space-x-2 rounded-md p-1.5', className)} onSelect={() => {
            if (!range.current)
                return;

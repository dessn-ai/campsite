import { Attachment, Call, Comment, MessageThread, Note, Post, Project, ResourceMention, User } from "../../../packages/types/index.ts";
import { useQueryNormalizer } from "./normy/QueryNormalizerProvider.tsx";
import { pluckKeyValues } from "./pluckKeyValues.ts";
...
function normalizedData<Key extends keyof NormalizedDataTypes, TData = NormalizedDataTypes[Key]>(type: Key, data: {
    [normalizedIdKey]: string;
} & Partial<TData>) {
    return { [normalizedTypeNameKey]: type, ...data };
}
type UpdateFn<T> = (old: T) => Partial<T>;
export function setNormalizedData<Key extends keyof NormalizedDataTypes, TData = NormalizedDataTypes[Key]>({ queryNormalizer, type, id, update }: {
...
export function createNormalizedOptimisticUpdate<Key extends keyof NormalizedDataTypes, TData = NormalizedDataTypes[Key] & {
    [key: string]: any;
}>({ queryNormalizer, type, id, update }: {
    queryNormalizer: ReturnType<typeof useQueryNormalizer>;
    type: Key;
    id: string;
    update: Partial<TData> | UpdateFn<TData>;
...
export function getNormalizedData<Key extends keyof NormalizedDataTypes, TData = NormalizedDataTypes[Key]>({ queryNormalizer, type, id }: {
    queryNormalizer: ReturnType<typeof useQueryNormalizer>;
    type: Key;
    id: string;
}) {

import { figmaRegex } from "../../../packages/regex/src/index.ts";
import { Attachment } from "../../../packages/types/generated.ts";
/**
 * Converts an Attachment object into a more usable format
 */
export function getFileMetadata(attachment: Partial<Pick<Attachment, 'name' | 'file_type' | 'url' | 'download_url' | 'origami' | 'principle' | 'stitch' | 'link'>>) {
    ...
    if (origami)
        openUrl = downloadUrl?.replace('https', 'origami-public');
    if (principle)
        openUrl = downloadUrl?.replace('https', 'principle');
    if (stitch)
        openUrl = downloadUrl?.replace('https', 'stitch');
    if (figma) {
        openUrl = downloadUrl;

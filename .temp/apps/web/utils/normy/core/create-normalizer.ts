import { addOrRemoveDependencies } from "./add-or-remove-dependencies.ts";
import { defaultConfig } from "./default-config.ts";
import { denormalize } from "./denormalize.ts";
import { getDependenciesDiff } from "./get-dependencies-diff.ts";
import { getQueriesDependentOnMutation } from "./get-queries-dependent-on-mutation.ts";
import { mergeData } from "./merge-data.ts";
import { normalize } from "./normalize.ts";
import { Data, DataObject, NormalizedData, NormalizerConfig } from "./types.ts";
...
const isMutationObjectDifferent = (mutationData: Data, normalizedData: Data): boolean => {
    ...
    if (mutationData !== null &&
        ...
        return Object.entries(mutationData).some(([key, value]) => (normalizedData as DataObject)?.[key] !== undefined &&
...
export const createNormalizer = (normalizerConfig?: NormalizerConfig, initialNormalizedData?: NormalizedData) => {
    ...
    const setQuery = (queryKey: string, queryData: Data) => {
        ...
        const { addedDependencies, removedDependencies } = getDependenciesDiff(normalizedData.queries[queryKey] ? normalizedData.queries[queryKey].dependencies : [], Object.keys(normalizedObjectsData));
        normalizedData = {
            queries: {
    ...
    const getObjectById = <T extends Data>(id: string, exampleObject?: T): T | undefined => getQueryFragment(`@@${id}`, exampleObject);
    return {

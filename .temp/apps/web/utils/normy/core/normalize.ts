...
import { Data, DataObject, DataPrimitiveArray, NormalizerConfig, UsedKeys } from "./types.ts";
...
export const getDependencies = (data: Data, config = defaultConfig, usedKeys?: UsedKeys, path = ''): [
    DataObject[],
    UsedKeys
] => {
    usedKeys = usedKeys || {};
    if (Array.isArray(data)) {
        return [
            (data as DataObject[]).reduce((prev: DataObject[], current: Data) => [...prev, ...getDependencies(current, config, usedKeys, path)[0]], [] as DataObject[]),
            usedKeys
        ];
    }
    if (data !== null && typeof data === 'object' && !(data instanceof Date)) {
        ...
        return [
            Object.entries(data).reduce((prev, [k, v]) => [...prev, ...getDependencies(v, config, usedKeys, `${path}.${k}`)[0]], config.getNormalizationObjectKey(data) !== undefined ? [data] : []),
            usedKeys
        ];
    }
    return [[], usedKeys];
};
export const normalize = (data: Data, config = defaultConfig): [
    Data,
    {

...
import { createNormalizer, Data, NormalizerConfig } from "../core/index.ts";
...
export const createQueryNormalizer = (queryClient: QueryClient, normalizerConfig: Omit<NormalizerConfig, 'structuralSharing'> & {
    normalize?: boolean;
} = {}) => {
        ...
        subscribe: () => {
            ...
            unsubscribeMutationCache = queryClient.getMutationCache().subscribe((event) => {
                if (event.type === 'updated' &&
                    ...
                    (event.mutation.state?.context as {
                        optimisticData?: Data;
                    })?.optimisticData) {
                    updateQueriesFromData((event.mutation.state.context as {
                        optimisticData: Data;
                    }).optimisticData);
                }
                else if (event.type === 'updated' &&

...
import { isStandardReactionSkin, ReactionData } from "./data.ts";
...
export function hasReacted(groupedReactions: GroupedReaction[], reaction: StandardReaction | SyncCustomReaction): boolean {
    return groupedReactions.some((groupedReaction) => {
        if (!groupedReaction.viewer_reaction_id)
            return false;
        if (isStandardReaction(reaction))
            return reaction.native === groupedReaction.emoji;
        return reaction.id === groupedReaction.custom_content?.id;
    });
}
export function isExistingReaction(groupedReaction: GroupedReaction, reaction: StandardReaction | SyncCustomReaction): boolean {
    if (isStandardReaction(reaction))
        return reaction.native === groupedReaction.emoji;
    return reaction.id === groupedReaction.custom_content?.id;
}
export function findGroupedReaction(groupedReactions: GroupedReaction[], reaction: StandardReaction | SyncCustomReaction): GroupedReaction | undefined {

...
import { FollowUp, NotificationPage, OrganizationsOrgSlugFollowUpsIdPutRequest } from "../../../packages/types/index.ts";
...
import { createNormalizedOptimisticUpdate, setNormalizedData } from "./queryNormalization.ts";
/*
...
*/
export function clearNotificationsWithFollowUp({ id, type, queryClient }: {
    id: string;
    type: FollowUpType;
    ...
    setTypedInfiniteQueriesData(queryClient, getMembersMeNotifications.baseKey, (old) => {
        ...
        return {
            ...
            pages: old.pages.map((page) => {
                return {
                    ...page,
                    data: page.data.filter((notification) => notification.follow_up_subject?.id !== id && notification.follow_up_subject?.type !== type)
                };
            })
        };
    });
...
export function handleFollowUpDelete({ queryClient, queryNormalizer, followUpId, subjectId, subjectType }: PropsFollowUpDelete) {
...
export function handleFollowUpUpdate({ queryNormalizer, followUpId, subjectId, subjectType, updateData }: PropsFollowUpUpdate) {
    const type = normyTypeFromApiTypeName(subjectType);
    if (type)
        return createNormalizedOptimisticUpdate({
            queryNormalizer,

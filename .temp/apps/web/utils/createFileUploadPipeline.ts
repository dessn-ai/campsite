...
import { uuidExpression } from "../../../packages/regex/src/index.ts";
import { Attachment } from "../../../packages/types/generated.ts";
import { VALID_AUDIO_TYPES, VALID_IMAGE_TYPES, VALID_VIDEO_TYPES } from "../components/Post/utils.ts";
import { handleFile } from "../hooks/useUploadFile.ts";
import { getLottieThumbnailAndDuration } from "./getLottieThumbnailAndDuration.ts";
import { getVideoThumbnailAndDuration } from "./getVideoThumbnailAndDuration.ts";
import { fileIsLottie, generateImageValues } from "./transformFile.ts";
import { urlToHlsUrl } from "./urlToHlsUrl.ts";
type Upload = {
    file: File;
...
export async function createFileUploadPipeline({ files, scope, onFilesExceedMaxSize, maxFileSize, onAppend, onUpdate }: Props) {
    ...
    const promises = uploads.map(async ({ attachment, thumbnail, file }) => {
        ...
        if (thumbnail) {
            uploadPromises.push(handleFile({
                file: thumbnail,
                type: thumbnail.type,
                resource: 'Post',
        ...
        uploadPromises.push(handleFile({ file, type: file.type, resource: 'Post', orgSlug: scope as string }).then((path) => {
            if (path) {

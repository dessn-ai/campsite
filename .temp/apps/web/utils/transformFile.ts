...
import { isImage } from "../components/Post/utils.ts";
import { TransformedFile } from "./types.ts";
export function generateImageValues({ file, isOnDisk = false }: {
    file: File;
    isOnDisk?: boolean;
}) {
    return new Promise<{
...
const containsLottie = (json: Record<string, any>) => REQUIRED_LOTTIE_KEYS.every((field) => Object.prototype.hasOwnProperty.call(json, field));

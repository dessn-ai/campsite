...
import { OrganizationMember, TimelineEvent, TimelineEventPage } from "../../../../packages/types/generated.ts";
...
import { apiClient, setTypedInfiniteQueriesData } from "../queryClient.ts";
import { TimelineEventPostResolved, TimelineEventPostUnresolved, TimelineEventSubjectPinned, TimelineEventSubjectTitleUpdated, TimelineEventSubjectUnpinned } from "./types.ts";
const OPTIMISTIC_ID_PREFIX = 'temp';
const ROLLUP_WINDOW = 60 * 1000; // 60 seconds
...
export function createOptimisticTimelineEvent(props: {
...
function rollUpTimelineEvents({ optimisticTimelineEvent, pages }: {
    ...
    const last_matching_timeline_event = pages
        ...
        if (new Date(optimisticTimelineEvent.created_at).getTime() - new Date(timelineEvent.created_at).getTime() >=
            ROLLUP_WINDOW) {
            return false;
        }
        if (timelineEvent.member_actor?.id !== optimisticTimelineEvent.member_actor?.id)
            return false;
...
export function insertPostTimelineEvent({ queryClient, scope, postId, timelineEvent }: {
    ...
    if (!timelineEvent)
        return;
    setTypedInfiniteQueriesData(queryClient, getPostsTimelineEvents.requestKey({ orgSlug: `${scope}`, postId }), (old) => {
        if (!old)
            return old;
        return {
            ...old,

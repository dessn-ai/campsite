...
import { RAILS_API_URL, WEB_PUSH_PUBLIC_KEY } from "../../../packages/config/src/index.ts";
...
export const WebPushProvider: React.FC<Props> = ({ children }) => {
    ...
    useEffect(() => {
        if (canPush && 'serviceWorker' in navigator) {
            navigator.serviceWorker.register(`/service_worker.js?API_URL=${RAILS_API_URL}`).then((registration) => {

...
import { LayeredHotkeys } from "../DismissibleLayer/index.tsx";
import { cn } from "../utils/index.ts";
import { commandScore } from "../utils/commandScore.ts";
...
interface Store {
    ...
    snapshot: () => State;
    setState: <K extends keyof State>(key: K, value: State[K], opts?: any, event?: React.MouseEvent<HTMLDivElement> | React.PointerEvent<HTMLDivElement>) => void;
    emit: () => void;
}
interface Group {
    id: string;
...
const Command = React.forwardRef<CommandRef, CommandProps>(function Command(props, ref) {
    ...
    const ids = useLazyRef<Map<string, {
        value: string;
        keywords?: string[];
        modifier?: number;
    ...
    const { label, value, className, disablePointerSelection: _disablePointerSelection = false, vimBindings = true, focusSelection = false, disableAutoSelect = false, manualInputs = false, minScore = -1, 
    // prevent prop spreading invalid attributes
    shouldFilter: _shouldFilter, filter: _filter, onValueChange: _onValueChange, ...etc } = props;
    ...
    return (<>
      <LayeredHotkeys keys={['ArrowDown']} options={{ repeat: true, enabled: !manualInputs, ...hotkeyEditableOptions }} callback={(e) => next(e, 'none')}/>
      <LayeredHotkeys keys={['alt+ArrowDown']} options={{ repeat: true, enabled: !manualInputs, ...hotkeyEditableOptions }} callback={(e) => next(e, 'alt')}/>
      <LayeredHotkeys keys={['mod+ArrowDown']} options={{ repeat: true, enabled: !manualInputs, ...hotkeyEditableOptions }} callback={(e) => next(e, 'meta')}/>
      <LayeredHotkeys keys={['n', 'j']} options={{ repeat: true, enabled: !manualInputs && vimBindings }} callback={(e) => next(e, 'none')}/>
      <LayeredHotkeys keys={['alt+n', 'alt+j']} options={{ repeat: true, enabled: !manualInputs && vimBindings }} callback={(e) => next(e, 'alt')}/>
      <LayeredHotkeys keys={['ArrowUp']} options={{ repeat: true, enabled: !manualInputs, ...hotkeyEditableOptions }} callback={(e) => prev(e, 'none')}/>
      <LayeredHotkeys keys={['alt+ArrowUp']} options={{ repeat: true, enabled: !manualInputs, ...hotkeyEditableOptions }} callback={(e) => prev(e, 'alt')}/>
      <LayeredHotkeys keys={['mod+ArrowUp']} options={{ repeat: true, enabled: !manualInputs, ...hotkeyEditableOptions }} callback={(e) => prev(e, 'meta')}/>
      <LayeredHotkeys keys={['p', 'k']} options={{ repeat: true, enabled: !manualInputs && vimBindings }} callback={(e) => prev(e, 'none')}/>
      <LayeredHotkeys keys={['alt+p', 'alt+k']} options={{ repeat: true, enabled: !manualInputs && vimBindings }} callback={(e) => prev(e, 'alt')}/>
      <LayeredHotkeys keys={['Home']} options={{ enabled: !manualInputs, ...hotkeyEditableOptions }} callback={(e) => {
            if (!eventTargetIsCmdkInput(e))
                return;
            updateSelectedToIndex(0);
        }}/>
      <LayeredHotkeys keys={['End']} options={{ enabled: !manualInputs, ...hotkeyEditableOptions }} callback={(e) => {
            if (!eventTargetIsCmdkInput(e))
                return;
            last();
        }}/>
      <LayeredHotkeys keys={['Enter']} options={{ enabled: !manualInputs, ...hotkeyEditableOptions }} callback={(e) => {
            if (!eventTargetIsCmdkInput(e))
                return;
      ...
      <div ref={containerRef} tabIndex={-1} className={cn('outline-none', className)} {...etc} 
    // eslint-disable-next-line react/no-unknown-property
    cmdk-root=''>
        <label 
    // eslint-disable-next-line react/no-unknown-property
    cmdk-label='' htmlFor={context.inputId} id={context.labelId} 
    // Screen reader only
    style={srOnlyStyles}>
          {label}
        </label>
        {SlottableWithNestedChildren(props, (child) => (<StoreContext.Provider value={store}>
...
const Item = React.forwardRef<HTMLDivElement, ItemProps>(function Item(props, forwardedRef) {
    ...
    const render = useCmdk((state) => forceMount
        ? true
    ...
    function select(event?: React.MouseEvent<HTMLDivElement> | React.PointerEvent<HTMLDivElement>) {
        if (value.current)
            store.setState('value', value.current, true, event);
    }
    function deselect() {
        if (value.current)
            store.setState('value', undefined, true);
    }
    const { disabled, disableOnClick, style, value: _value, onSelect: _onSelect, forceMount: _forceMount, keywords: _keywords, scoreModifier: _scoreModifier, ...etc } = props;
    const handleClick = (e: React.MouseEvent<HTMLDivElement>) => {
        if (disabled || disableOnClick)
            return;
    ...
    return (<div ref={mergeRefs([ref, forwardedRef])} {...etc} 
    // must still mount a DOM element as getValidItems() uses mounted elements for filtering and sorting
    data-render={render} style={{ ...style, display: render ? undefined : 'none' }} id={id} 
    // eslint-disable-next-line react/no-unknown-property
    cmdk-item='' tabIndex={0} role='option' aria-disabled={Boolean(disabled)} aria-selected={Boolean(selected)} data-disabled={Boolean(disabled)} data-selected={Boolean(selected)} onPointerMove={disabled || context.getDisablePointerSelection() ? undefined : select} onFocus={disabled ||
            context.getDisablePointerSelection() ||
            // prevent re-entrant selection
            selected
            ? undefined
            : () => select()} onBlur={disabled || context.getDisablePointerSelection() || !selected ? undefined : deselect} onClick={handleClick} onAuxClick={handleClick}>
      {props.children}
    </div>);
});
/**
...
const Group = React.forwardRef<HTMLDivElement, GroupProps>(function Group(props, forwardedRef) {
    ...
    return (<div ref={mergeRefs([ref, forwardedRef])} {...etc} 
    // eslint-disable-next-line react/no-unknown-property
    cmdk-group='' role='presentation' hidden={render ? undefined : true}>
      {heading && (<div ref={headingRef} 
        // eslint-disable-next-line react/no-unknown-property
        cmdk-group-heading='' aria-hidden id={headingId}>
          {heading}
        </div>)}
      {SlottableWithNestedChildren(props, (child) => (<div 
        // eslint-disable-next-line react/no-unknown-property
        cmdk-group-items='' role='group' aria-labelledby={heading ? headingId : undefined}>
...
const Input = React.forwardRef<HTMLInputElement, InputProps>(function Input(props, forwardedRef) {
    ...
    const selectedItemId = React.useMemo(() => {
        if (!value)
            return;
        const item = context.listInnerRef.current?.querySelector(`${ITEM_SELECTOR}[${VALUE_ATTR}="${encodeURIComponent(value)}"]`);
    ...
    return (<input ref={forwardedRef} {...etc} 
    // eslint-disable-next-line react/no-unknown-property
    cmdk-input='' autoComplete='off' autoCorrect='off' spellCheck={false} aria-autocomplete='list' role='combobox' aria-expanded={true} aria-controls={context.listId} aria-labelledby={context.labelId} aria-activedescendant={selectedItemId ?? undefined} id={id} type='text' value={isControlled ? props.value : search} onChange={(e) => {
            if (!isControlled) {
...
const List = React.forwardRef<HTMLDivElement, ListProps>(function List(props, forwardedRef) {
    ...
    const context = useCommand()!;
    return (<div ref={mergeRefs([forwardedRef, context.listInnerRef])} {...etc} 
    // eslint-disable-next-line react/no-unknown-property
    cmdk-list='' role='listbox' aria-label={label} id={context.listId}>
      {SlottableWithNestedChildren(props, (child) => (<>{child}</>))}
    </div>);
});
/**
...
const Loading = React.forwardRef<HTMLDivElement, LoadingProps>(function Loading(props, forwardedRef) {
    ...
    return (<div ref={forwardedRef} {...etc} 
    // eslint-disable-next-line react/no-unknown-property
    cmdk-loading='' role='progressbar' aria-valuenow={progress} aria-valuemin={0} aria-valuemax={100} aria-label={label}>
      {SlottableWithNestedChildren(props, (child) => (<div aria-hidden>{child}</div>))}
    </div>);
});
...
function useValue(id: string, ref: React.RefObject<HTMLElement>, deps: (string | React.ReactNode | React.RefObject<HTMLElement>)[], aliases: string[] = [], scoreModifier: number = 1) {
...
function SlottableWithNestedChildren({ asChild, children }: {
    asChild?: boolean;
    children?: React.ReactNode;

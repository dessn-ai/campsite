...
export function Badge({ children, color = 'default', icon, className, tooltip }: {
    ...
    return (<ConditionalWrap condition={!!tooltip} wrap={(children) => <Tooltip label={tooltip}>{children}</Tooltip>}>
      <span className={cn('min-h-4.5 flex flex-none items-center justify-center rounded px-1.5 pb-px pt-0.5 uppercase', BG_STYLE[color], TEXT_STYLE[color], icon && 'gap-0.5 pl-1', className)}>
        {icon && icon}

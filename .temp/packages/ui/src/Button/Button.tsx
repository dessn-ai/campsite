...
import { Link, LinkProps } from "../Link/index.tsx";
...
export const Button = forwardRef<HTMLButtonElement & HTMLAnchorElement, ButtonProps>(({ id, align = 'center', accessibilityLabel, loading, children, disabled, href, iconOnly, variant, className, fullWidth, onFocus, leftSlot, rightSlot, onBlur, onClick, onKeyDownCapture, tooltip, tooltipShortcut, autoFocus, externalLink, allowOpener, type = 'button', download, role, round, size = 'base', 'aria-pressed': ariaPressed, title, isSelect, ...props }, ref) => {
    if (iconOnly && !accessibilityLabel) {
        throw new Error('You must provide an accessibilityLabel to a button with iconOnly');
    }
    if (href) {
        return (<Tooltip disableHoverableContent label={tooltip} shortcut={tooltipShortcut}>
          <Link id={id} href={href} className={cn('relative outline-none transition', buttonVariants({ variant, fullWidth, loading, size, round }), { '!pl-1': leftSlot && size === 'sm' }, { '!pl-2': leftSlot && size === 'base' }, { '!pl-4': leftSlot && size === 'large' }, { '!pr-1': rightSlot && size === 'sm' }, { '!pr-2': rightSlot && size === 'base' }, { '!pr-4': rightSlot && size === 'large' }, { 'w-6.5 p-0': iconOnly && size === 'sm' }, { 'w-7.5 p-0': iconOnly && size === 'base' }, { 'w-10 p-0': iconOnly && size === 'large' }, { 'px-2': !iconOnly && size === 'sm' }, { 'px-2.5': !iconOnly && size === 'base' }, { 'px-4.5': !iconOnly && size === 'large' }, { 'pointer-events-none opacity-50': disabled || loading }, className)} ref={ref} onFocus={onFocus} onBlur={onBlur} title={tooltip ? undefined : (title ?? accessibilityLabel)} aria-label={accessibilityLabel} onClick={(e) => {
                if (onClick)
            ...
            }} onKeyDownCapture={onKeyDownCapture} target={externalLink ? '_blank' : undefined} rel={externalLink ? (allowOpener ? 'opener' : 'noopener noreferrer') : undefined} role={role} draggable={false} {...props}>
            {iconOnly && iconOnly}
            <span className={cn('relative z-[2] flex items-center', {
                'gap-2': size === 'large',
                'gap-1.5': size === 'base',
                'gap-1': size === 'sm'
            })}>
    ...
    return (<Tooltip disableHoverableContent label={tooltip} shortcut={tooltipShortcut}>
        <button id={id} className={cn('outline-none', buttonVariants({ variant, fullWidth, loading, size, round }), { '!pl-1': leftSlot && size === 'sm' }, { '!pl-2': leftSlot && size === 'base' }, { '!pl-4': leftSlot && size === 'large' }, { '!pr-1': rightSlot && size === 'sm' }, { '!pr-2': rightSlot && size === 'base' }, { '!pr-4': rightSlot && size === 'large' }, { 'w-6.5 p-0': iconOnly && size === 'sm' }, { 'w-7.5 p-0': iconOnly && size === 'base' }, { 'w-10 p-0': iconOnly && size === 'large' }, { 'px-2': !iconOnly && size === 'sm' }, { 'px-2.5': !iconOnly && size === 'base' }, { 'px-4.5': !iconOnly && size === 'large' }, className)} ref={ref} type={type} onFocus={onFocus} onBlur={onBlur} title={tooltip ? undefined : (title ?? accessibilityLabel)} aria-label={accessibilityLabel} onClick={onClick} disabled={disabled} autoFocus={autoFocus} aria-pressed={ariaPressed} data-autofocus={autoFocus} onKeyDownCapture={onKeyDownCapture} {...props}>
          {iconOnly && iconOnly}
          <span className={cn('relative z-[2] flex items-center overflow-hidden', {

...
import { useIsDesktopApp } from "../hooks/index.tsx";
...
export function Root({ children, size = 'base', fillHeight = false, align = 'center', open, onOpenChange, onKeyDown, trigger, portalContainer, onPointerDownOutside, onInteractOutside = undefined, visuallyHiddenTitle, visuallyHiddenDescription, disableDescribedBy = false }: DialogProps) {
    ...
    return (<D.Root open={open} onOpenChange={dialogOpenChange} modal={portalContainer ? false : true}>
      {trigger && <D.Trigger asChild>{trigger}</D.Trigger>}

      {open && (<D.Portal forceMount container={container}>
          <D.Overlay asChild>
            <div className={cn('animate-backdrop fixed inset-0 bg-black/20 dark:bg-black/60')}/>
          </D.Overlay>

          <DismissibleLayer>
            <D.Content asChild ref={contentRef} 
        // must be an empty value to work
        disable-escape-layered-hotkeys='' onKeyDown={(e) => {
                onKeyDown?.(e);
            ...
            }} onInteractOutside={onInteractOutside} onPointerDownOutside={(e) => {
                if (onPointerDownOutside)
                    return onPointerDownOutside(e);
                ...
                const actuallyClickedInside = e.detail.originalEvent.clientX > contentRect.left &&
              ...
              <div onBlur={(event) => {
            ...
            }} className={cn('transition-all', 'fixed left-1/2 isolate w-full -translate-x-1/2 focus:outline-0', 'flex flex-col', maxWidth, align === 'top' && size !== 'cover'
                ? fillHeight
                ...
                : 'top-1/2 -translate-y-1/2', size !== 'cover' &&
                align !== 'top' &&
                (!isDesktopApp
                    ? 'max-h-[calc(100dvh-env(safe-area-inset-bottom,0)-env(safe-area-inset-top,0)-32px)]'
                    : 'max-h-[calc(100dvh-env(safe-area-inset-bottom,0)-env(safe-area-inset-top,0)-64px)]'), needsFullHeight ? 'h-screen' : fillHeight ? undefined : 'h-auto', {
                'bg-elevated shadow-popover rounded-lg dark:bg-gray-900 dark:shadow-[inset_0_0.5px_0_rgb(255_255_255_/_0.08),_inset_0_0_1px_rgb(255_255_255_/_0.24),_0_0_0_0.5px_rgb(0,0,0,1),0px_0px_4px_rgba(0,_0,_0,_0.08),_0px_0px_10px_rgba(0,_0,_0,_0.12),_0px_0px_24px_rgba(0,_0,_0,_0.16),_0px_0px_80px_rgba(0,_0,_0,_0.2)]': size !== 'cover',
                'pt-8': isDesktopApp && size === 'full',
                'bg-primary': size === 'cover'
            })} style={{
                maxHeight: visualHeight ? `calc(${visualHeight}px - 12vh)` : undefined
            }}>
                {visuallyHiddenTitle && (<VisuallyHidden asChild>
                    <D.Title>{visuallyHiddenTitle}</D.Title>
                  </VisuallyHidden>)}
                {visuallyHiddenDescription && (<VisuallyHidden asChild>
...
export function Footer(props: DefaultProps & {
    variant?: 'primary' | 'secondary';
}) {
    const { variant = 'primary', className, ...rest } = props;
    return (<footer className={cn('flex items-center rounded-b-lg border-t p-3', className, {
...
export function CloseButton({ className, ...props }: DefaultProps) {
    ...
    tabIndex={-1} asChild>
      <Button className={cn('absolute right-3 top-3', className)} variant='plain' iconOnly={<CloseIcon strokeWidth='2'/>} accessibilityLabel='Close' tooltip='Close' tooltipShortcut='Esc' {...props}/>
    </D.Close>);
}

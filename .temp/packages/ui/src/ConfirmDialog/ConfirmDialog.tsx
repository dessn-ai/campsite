...
import { Button, ButtonProps } from "../Button/index.tsx";
import * as CampsiteDialog from "../Dialog/index.tsx";
...
function Dialog({ title, description, confirmLabel = 'Confirm', dialogProps = { size: 'base' } }: DialogProps) {
    ...
    return (<CampsiteDialog.Root open={open} onOpenChange={handleOpenChange} {...dialogProps}>
      ...
      <CampsiteDialog.Footer>
        <CampsiteDialog.TrailingActions>
          ...
          <Button variant='destructive' onClick={() => onConfirm(handleOpenChange)} disabled={isLoading} loading={isLoading} autoFocus>
            {confirmLabel}
          </Button>
        </CampsiteDialog.TrailingActions>
      </CampsiteDialog.Footer>

...
function Root(props: RootProps) {
    return (<ScopeProvider atoms={[isHoveringAtom]}>
      <InnerRoot {...props}/>
    </ScopeProvider>);
}
function InnerRoot({ children, open, onOpenChange, disabled: _disabled, targetHref }: RootProps) {
    ...
    const handleOpenChange = useCallback((newVal: boolean) => {
        if (disabled) {
    ...
    if (open && disabled)
        onOpenChange?.(false);
    return (<RadixHoverCard.Root open={open} onOpenChange={handleOpenChange} openDelay={isWarmedUp ? 100 : 500} closeDelay={50}>
      {children}
    </RadixHoverCard.Root>);
}
...
function Trigger({ children, asChild }: TriggerProps) {
    ...
    return (<RadixHoverCard.Trigger data-hello='world' asChild={asChild} onMouseEnter={(e) => {
            setIsHovering(true);
...
function PredictionCone({ sideOffset }: PredictionConeProps) {
    ...
    const [predictionMousePosition, setPredictionMousePosition] = useState<{
        x: number;
        y: number;
    } | undefined>(undefined);
    const normalizedMousePosition = predictionMousePosition ?? triggerMousePosition;
    const { width, clipPath } = useMemo(() => {
        if (!normalizedMousePosition || !achorRef.current)
            return {};
    ...
    return (<>
      ...
      <div className={cn('absolute -inset-y-5 right-full cursor-pointer', {
            'bg-red-500/50': DEBUG
        })} style={{
            clipPath,
            width: `${width ?? sideOffset}px`
        }} onMouseEnter={(e) => setPredictionMousePosition({ x: e.clientX, y: e.clientY })} onMouseMove={(e) => setPredictionMousePosition({ x: e.clientX, y: e.clientY })} onMouseLeave={() => setPredictionMousePosition(undefined)}/>
    </>);
}
interface ContentProps extends Omit<RadixHoverCard.HoverCardContentProps, 'hideWhenDetached'> {
    portalContainer?: string;
}
function Content({ children, side = 'right', align = 'start', sideOffset = 8, alignOffset = 0, collisionPadding = 8, portalContainer, className, ...props }: ContentProps) {
    const container = portalContainer ? document.getElementById(portalContainer) : document.body;
    return (<RadixHoverCard.Portal container={container}>
      <RadixHoverCard.Content {...props} side={side} align={align} sideOffset={sideOffset} alignOffset={alignOffset} collisionPadding={collisionPadding} hideWhenDetached className={cn('animate-scale-fade shadow-popover dark:border-primary-opaque bg-primary relative flex h-[420px] w-[420px] flex-1 origin-[--radix-hover-card-content-transform-origin] flex-col rounded-lg border border-transparent dark:shadow-[0px_2px_16px_rgba(0,0,0,1)]', className, 'overflow-visible' // prediction cone only works if we allow the triangle to "leak" beyond the bounds of the container
        )}>
        <DismissibleLayer>
          <>{children}</>
        </DismissibleLayer>

        <div className={cn('absolute -inset-5 -z-10', { 'bg-blue-500/50': DEBUG })} style={{
...
function ContentTitleBar({ children }: {

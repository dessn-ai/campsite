...
export function LimitIndicator({ maxLength, currentLength, charThreshold }: Props) {
    ...
    return (<div className='flex items-center gap-0.5'>
      <span className={cn('font-mono text-xs font-medium leading-none text-transparent', {
      ...
      <svg className='h-3.5 w-3.5'>
        <circle className='origin-center -rotate-90 text-gray-300 dark:text-gray-600' strokeWidth={strokeWidth} stroke='currentColor' fill='transparent' r={radius} cx={radius + strokeWidth} cy={radius + strokeWidth}/>
        <circle className={cn('origin-center -rotate-90 text-blue-500', {
            'text-red-600': tooManyCharacters
        })} strokeWidth={strokeWidth} strokeDasharray={circumference} strokeDashoffset={dashOffset} strokeLinecap='round' stroke='currentColor' fill='transparent' r={radius} cx={radius + strokeWidth} cy={radius + strokeWidth}/>
      </svg>
    </div>);
}

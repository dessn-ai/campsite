...
import { MenuHeadingType, MenuItem, MenuItemType, MenuSubType, MenuTextType } from "./types.ts";
function DrawerSeparator() {
    return (<hr className='relative -mx-1 my-1.5 h-px md:my-0.5 dark:bg-gray-900 dark:shadow-[0px_1px_0px_rgb(255_255_255_/_0.05)]'/>);
}
function DrawerHeading({ item }: {
    item: MenuHeadingType;
}) {
    return (<div className='pointer-events-none flex w-full items-center justify-start px-4 pt-2'>
      <UIText tertiary className='line-clamp-1 flex-1 text-left' size='text-xs'>
        {item.label}
      </UIText>
    </div>);
}
function DrawerText({ item }: {
    item: MenuTextType;
}) {
    return (<div className='pointer-events-none flex w-full items-center justify-start px-4 py-2'>
...
function DrawerItem({ item, onClose }: DrawerItemProps) {
    return (<button disabled={item.disabled} onClick={(event) => {
            onClose();
            // @ts-expect-error
            item.onSelect?.(event);
        }} className={cn('group relative flex w-full cursor-pointer items-center justify-start gap-2.5 rounded-[5px] border-none px-4 font-medium outline-none disabled:cursor-not-allowed disabled:opacity-50', 'md:h-8.5 h-10.5 text-base md:text-sm')}>
...
function DrawerSubItem({ item, onClose }: DrawerSubItemProps) {
    ...
    return (<DrawerPrimitive.Root open={open} onOpenChange={onOpenChange}>
      <DrawerPrimitive.Trigger disabled={item.disabled} className={cn('group relative flex w-full cursor-pointer items-center justify-start gap-2.5 rounded-[5px] border-none px-4 font-medium outline-none disabled:cursor-not-allowed disabled:opacity-50', 'md:h-8.5 h-10.5 text-base md:text-sm')}>
        {item.leftSlot && <span className='initial:text-neutral-400 scale-125 transition-colors'>{item.leftSlot}</span>}
        <span className='line-clamp-1 flex-1 text-left'>{item.label}</span>
...
function DrawerActions({ items, onClose, header }: DrawerActionsProps) {
    return (<DrawerPrimitive.Portal>
      <div className='relative isolate z-50'>
        ...
        <DrawerPrimitive.Content className={cn('bg-elevated text-secondary flex max-h-[95dvh] flex-col rounded-t-xl', 'focus:outline-none', 'fixed inset-x-0 bottom-0')}>
          {/* Handle */}

...
import { useBreakpoint } from "../hooks/index.tsx";
import { cn, ConditionalWrap, CONTAINER_STYLES } from "../utils/index.ts";
...
const PopoverTrigger = React.forwardRef<React.ElementRef<typeof PopoverPrimitive.Trigger>, React.ComponentPropsWithoutRef<typeof PopoverPrimitive.Trigger>>(function PopoverTrigger(props, ref) {
    const context = React.useContext(PopoverContext);
    const Trigger = context.sheet ? Drawer.Trigger : PopoverPrimitive.Trigger;
    return (<Trigger ref={ref} {...props} onPointerDown={(e) => {
...
const PopoverContent = React.forwardRef<React.ElementRef<typeof PopoverPrimitive.Content>, React.ComponentPropsWithoutRef<typeof PopoverPrimitive.Content> & {
...
}>(({ className, align = 'center', sideOffset = 4, children, collisionPadding = 8, addDismissibleLayer = false, ...props }, ref) => {
    const context = React.useContext(PopoverContext);
    ...
    return (<React.Fragment>
        ...
        <ConditionalWrap condition={addDismissibleLayer} wrap={(children) => <DismissibleLayer>{children}</DismissibleLayer>}>
          <Content 
    // must be an empty value to work
    disable-escape-layered-hotkeys='' ref={ref} align={align} sideOffset={sideOffset} className={cn(context.sheet && 'bg-elevated pb-safe-offset-1 fixed inset-x-0 bottom-0 -mb-10 rounded-t-xl', !context.sheet && 'max-h-[--radix-popper-available-height] max-w-[--radix-popper-available-width]', CONTAINER_STYLES.animation, '!outline-none focus:outline-none focus:ring-0 focus-visible:outline-none focus-visible:ring-0', className)} collisionPadding={collisionPadding} {...props}>
            <div>
              {/* Handle */}
              {context.sheet && (<div className='flex cursor-grab justify-center p-3 pt-1'>

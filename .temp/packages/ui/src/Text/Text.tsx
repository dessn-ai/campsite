...
export const UIText = forwardRef<HTMLElement, BaseTextProps>(function UIText({ id = undefined, element: Element = 'p', className = '', weight = 'font-normal', size = 'text-sm', primary = false, secondary = false, tertiary = false, quaternary = false, inherit = false, selectable = false, ...rest }, ref) {
    const classes = cn(weight, size, {
...
export function LargeTitle({ element = 'h1', weight = 'font-bold', size = 'text-2xl md:text-4xl', ...rest }: BaseTextProps) {
    return <UIText element={element} weight={weight} size={size} {...rest}/>;
}
export function Title1({ element = 'h2', weight = 'font-medium', size = 'text-xl md:text-2xl', ...rest }: BaseTextProps) {
    return <UIText element={element} weight={weight} size={size} {...rest}/>;
}
export function Title2({ element = 'h2', weight = 'font-medium', size = 'text-lg md:text-xl', ...rest }: BaseTextProps) {
    return <UIText element={element} weight={weight} size={size} {...rest}/>;
}
export function Title3({ element = 'h2', weight = 'font-medium', size = 'text-md md:text-lg', ...rest }: BaseTextProps) {
    return <UIText element={element} weight={weight} size={size} {...rest}/>;
}

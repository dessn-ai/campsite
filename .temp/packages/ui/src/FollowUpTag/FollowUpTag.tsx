...
function followUpShortString(show_at: string) {
    ...
    const viewerFollowUpIsMoreThanOneWeekAway = new Date(show_at) > new Date(new Date().getTime() + 7 * 24 * 60 * 60 * 1000);
...
export function FollowUpTag({ followUpAt }: Props) {
    if (!followUpAt)
        return null;
    return (<Badge tooltip={`Follow-up scheduled for ${format(followUpAt, 'PPp')}`} color='orange' className='flex items-center font-mono' icon={<ClockIcon size={14} strokeWidth='2'/>}>

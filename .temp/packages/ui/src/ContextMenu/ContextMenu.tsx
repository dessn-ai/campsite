...
import { MenuHeadingType, MenuItemType, MenuSubType, MenuTextType, MenuWidth } from "../Menu/types.ts";
...
function ContextMenuHeading({ item }: {
    item: MenuHeadingType;
}) {
    return (<ContextMenuPrimitive.Label className='text-tertiary dark:text-secondary pl-1.5 pt-1 text-[10px] font-medium uppercase'>
      {item.label}
    </ContextMenuPrimitive.Label>);
}
function ContextMenuText({ item }: {
    item: MenuTextType;
}) {
    return (<ContextMenuPrimitive.Label className='text-tertiary dark:text-secondary pl-1.5 pt-1 text-[10px] font-medium'>
      {item.label}
    </ContextMenuPrimitive.Label>);
}
...
function ContextMenuItem({ item }: ContextMenuItemProps) {
    return (<ContextMenuPrimitive.Item disabled={item.disabled} onSelect={async (event) => {
            if (item.onSelect) {
                item.onSelect(event);
            }
            else if (item.url) {
        ...
        }} className={cn('text-primary data-[highlighted]:bg-quaternary dark:data-[highlighted]:shadow-select-item group flex h-8 cursor-pointer items-center gap-1.5 rounded-md !border-0 text-sm !ring-0 focus-visible:!border-0 focus-visible:!outline-none focus-visible:!ring-0', {
      ...
      {item.leftSlot && (<span className={cn('text-tertiary flex-none', {
...
function ContextSubItem({ children, item, width }: ContextSubItemProps) {
    return (<ContextMenuPrimitive.Sub>
      ...
      <ContextMenuPrimitive.Portal>
        <ContextMenuPrimitive.SubContent sideOffset={4} alignOffset={-4} className={cn('bg-elevated dark:border-primary-opaque max-h-[--radix-context-menu-content-available-height] min-w-[--radix-context-menu-trigger-width] origin-[--radix-context-menu-content-transform-origin] overflow-y-auto rounded-lg border border-neutral-400/40 p-1 shadow-md dark:shadow-[0px_0px_0px_0.5px_rgba(0,0,0,1),_0px_4px_4px_rgba(0,0,0,0.24)]', width)}>
          {children}
...
function ContextMenuActions({ items, width }: ContextMenuActionsProps) {
    return items.map((item, i) => {
        ...
        if (item.type === 'heading')
            return <ContextMenuHeading key={i} item={item}/>;
        if (item.type === 'sub') {
            return (
            // eslint-disable-next-line react/no-array-index-key
        ...
        // eslint-disable-next-line react/no-array-index-key
        if (item.type === 'text')
            return <ContextMenuText key={i} item={item}/>;
        // eslint-disable-next-line react/no-array-index-key
...
export function ContextMenu({ children, items, onOpenChange, asChild }: ContextMenuProps) {
    return (<ContextMenuPrimitive.Root onOpenChange={onOpenChange}>
      <ContextMenuPrimitive.Trigger asChild={asChild}>{children}</ContextMenuPrimitive.Trigger>
      <ContextMenuPrimitive.Portal>
        <DismissibleLayer>
          <ContextMenuPrimitive.Content collisionPadding={8} alignOffset={4} className={cn('focus:outline-none', 'max-h-[--radix-context-menu-content-available-height] min-w-[--radix-context-menu-trigger-width]', 'bg-elevated dark:border-primary-opaque overflow-y-auto rounded-lg border border-neutral-400/40 p-1 shadow dark:shadow-[0px_0px_0px_0.5px_rgba(0,0,0,1),_0px_4px_4px_rgba(0,0,0,0.24)]', CONTAINER_STYLES.animation)}>

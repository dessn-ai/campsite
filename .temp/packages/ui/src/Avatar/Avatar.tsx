...
import { MoonFilledIcon } from "../Icons/index.tsx";
import { Link } from "../Link/index.tsx";
import { UIText } from "../Text/index.tsx";
import { Tooltip } from "../Tooltip/index.tsx";
import { cn, ConditionalWrap } from "../utils/index.ts";
import { AvatarFacepileClip } from "./AvatarFacepileClip.tsx";
import { AvatarNotificationReasonClip } from "./AvatarNotificationReasonClip.tsx";
import { AvatarNotificationReasonSquareClip } from "./AvatarNotificationReasonSquareClip.tsx";
import { AvatarOnlineClip } from "./AvatarOnlineClip.tsx";
interface Urls {
    xs: string;
    sm: string;
    base: string;
...
function _Avatar(props: Props) {
    const { online = false, notificationsPaused = false, clip, size = 'base', name, src, urls, alt = '', href, tooltip, tooltipDelayDuration, tooltipSide = 'top', fade, deactivated, rounded = 'rounded-full' } = props;
    ...
    const clipId = useMemo(() => {
        if (clip === 'facepile')
            return facepileClipId;
        if (clip === 'notificationReason')
            return NotificationReasonClipId;
        if (clip === 'notificationReasonSquare')
            return NotificationReasonSquareClipId;
        if (online || notificationsPaused)
            return onlineClipId;
        return undefined;
    }, [
        clip,
    ...
    return (<ConditionalWrap condition={!!href} wrap={(children) => <Link href={href as string}>{children}</Link>}>
      <ConditionalWrap condition={!!tooltip} wrap={(children) => (<Tooltip delayDuration={tooltipDelayDuration} disableHoverableContent label={tooltip} side={tooltipSide}>
            {children}
          </Tooltip>)}>
        <span className='relative flex shrink-0 select-none' style={{
        ...
        }} className={cn('relative flex shrink-0 select-none items-center justify-center font-semibold', !showImage && accentBackgroundColor, textSize, fade && 'opacity-50 saturate-[10%]', deactivated && 'saturate-[10%]', rounded, showImage && 'border border-black/10 dark:border-white/5', 
        // prevents broken clip path in Safari
        clipId && 'will-change-transform')}>
            {showImage && (<Image className={cn('absolute inset-0 aspect-square object-cover', rounded)} alt={alt || `Avatar image of ${name}`} width={avatarSize} height={avatarSize} src={url} draggable={false} onError={() => setImageErrored(true)}/>)}

            {!showImage && (<span className='text-dark flex text-opacity-60'>
          ...
          {online && !notificationsPaused && !clip && (<div className={cn('absolute rounded-full bg-green-500', ONLINE_POSITION[size], ONLINE_SIZE[size])}/>)}

          {notificationsPaused && !clip && (<MoonFilledIcon size={DND_SIZE[size]} className={cn('absolute text-violet-500', DND_POSITION[size])}/>)}
        </span>
      </ConditionalWrap>
    </ConditionalWrap>);
}

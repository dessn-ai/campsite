...
import markdownItTasks from "./markdownItTasks.ts";
import { MarkdownParser, ParseSpec } from "./MarkdownParser.ts";
const ignoreAllMarkdownTokens: {
...
export function createMarkdownParser(schema: Schema, extensions: AnyExtension[], 
// WARNING: Provide JSDOM window and document objects to avoid errors in Node.js environment
domParser: DOMParser = new window.DOMParser(), document: Document = window.document) {
    const tokens = extensions
        .filter((extension) => 'markdownParseSpec' in extension.config && typeof extension.config.markdownParseSpec === 'function')
    ...
    return new MarkdownParser(schema, MarkdownIt('default', {
        // enables raw inline html with the html_inline token
        html: true,
        breaks: false,
        linkify: true
    }).use(markdownItTasks), tokens, domParser, document);
}

...
export function mergeTransforms(tr1, tr2, automerge = true, rebase = false, wordDiffs = false) {
    ...
    tr1Conflict = mapTransform(recreateTransform(tr1NoConflicts.doc, tr1.doc, false, wordDiffs), tr.doc, new Mapping(tr1NoConflicts.mapping.invert().maps.concat(tr.mapping.maps)));
    const tr2Conflict = mapTransform(recreateTransform(tr2NoConflicts.doc, tr2.doc, false, wordDiffs), tr.doc, new Mapping(tr2NoConflicts.mapping.invert().maps.concat(tr.mapping.maps)));
    if (rebase) {
        ...
        const conflicts = findConflicts(tr1Conflict, tr2Conflict), { inserted, deleted, conflictingSteps1, conflictingSteps2 } = createConflictingChanges(tr1Conflict, tr2Conflict);
        return {
            tr,
...
function rebaseMergedTransform(doc, nonConflictingDoc, conflictingDoc, wordDiffs) {
    const trNonConflict = recreateTransform(doc, nonConflictingDoc, true, wordDiffs), changes = ChangeSet.create(doc, { compare: (a, b) => false }).addSteps(nonConflictingDoc, trNonConflict.mapping.maps, { user: 2 }), trConflict = recreateTransform(nonConflictingDoc, conflictingDoc, false, wordDiffs), { inserted, deleted, conflictingSteps2 } = createConflictingChanges(new Transform(trNonConflict.doc), trConflict);
    return {
        tr: trNonConflict,
...
export class Merge {
    constructor(doc, changes, conflicts = [], conflictingSteps1 = [], conflictingSteps2 = [], conflictingChanges = { inserted: [], deleted: [] }) {
        this.doc = doc;
    ...
    map(mapping, doc) {
        let conflictingSteps1 = this.conflictingSteps1, conflictingSteps2 = this.conflictingSteps2, conflicts = this.conflicts, inserted = this.conflictingChanges.inserted, deleted = this.conflictingChanges.deleted;
    ...
    apply(user, index) {
        const step = user === 1
            ...
            : this.conflictingSteps2.find(([conflictIndex, conflictStep]) => conflictIndex === index)[1], map = step.getMap(), tr = new Transform(this.doc);
        let conflictingSteps1 = this.conflictingSteps1, conflictingSteps2 = this.conflictingSteps2, conflicts = this.conflicts;
        tr.step(step);
        const changes = this.changes.addSteps(tr.doc, [map], { user });
        if (user === 1) {
            conflictingSteps1 = conflictingSteps1
                .map(([conflictIndex, conflictStep]) => conflictIndex === index ? false : [conflictIndex, conflictStep.map(map)])
                .filter((step) => step);
            conflicts = conflicts.filter((conflict) => conflict[0] !== index);
        }
        else {
    ...
    applyAll(user) {
        const steps = this.conflictingSteps.map(([index, step]) => step), tr = new Transform(this.doc);
...
function automergeTransforms(tr1, tr2) {
    ...
    const doc = trDoc(tr1), conflicts = findConflicts(tr1, tr2), tr = new Transform(doc);
    let changes = ChangeSet.create(doc, { compare: (a, b) => false });
    const tr1NoConflicts = removeConflictingSteps(tr1, conflicts.map((conflict) => conflict[0])), tr2NoConflicts = removeConflictingSteps(tr2, conflicts.map((conflict) => conflict[1]));
...
function removeConflictingSteps(tr, conflicts) {
    const doc = trDoc(tr), newTr = new Transform(doc), removedStepsMap = new Mapping();
...
function findConflicts(tr1, tr2) {
    const changes1 = findContentChanges(tr1), changes2 = findContentChanges(tr2), conflicts = [];
    ...
    changes1.deleted.forEach((deleted1) => {
        changes2.deleted.forEach((deleted2) => {
            if ((deleted1.from >= deleted2.from && deleted1.from <= deleted2.to) ||
...
function createConflictingChanges(tr1Conflict, tr2Conflict) {
    ...
    iter.forEach(({ steps, user }) => steps.forEach(([index, step]) => {
        if (!step) {
            return;
        }

...
export const PostNoteAttachment = Node.create<PostNoteAttachmentOptions>({
    ...
    addCommands() {
        return {
            handleLinkAttachment: (url) => ({ dispatch, editor, chain, tr }) => {
                this.options.onCreateLinkAttachment?.({ url, editor, chain });
                tr.scrollIntoView();
                return dispatch?.(tr);
            },
            insertAttachments: (attachments, pos) => ({ tr, state, dispatch }) => {
                const { schema } = state;
            ...
            updateAttachment: (optimistic_id: string, value: Partial<InlineAttachmentAttributes>) => ({ tr, state, dispatch }) => {
                const match = findNodeAndPos(state, (n) => n.attrs.optimistic_id === optimistic_id && n.type.name === this.name);
                if (!match) {
                    return false;
                }
                const { pos } = match;
    ...
    addKeyboardShortcuts() {
        const maybeOpenAttachment = (options: PostNoteAttachmentOptions) => ({ editor }: {
            editor: Editor;
        }) => {

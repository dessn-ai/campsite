...
import { handleRestoreMentionBackspace } from "../utils/handleRestoreMentionBackspace.ts";
...
export const ResourceMention = Node.create<ResourceMentionOptions>({
    ...
    addCommands() {
        return {
            insertResourceMention: (href, range) => ({ chain, state }) => insertContent({
                chain,
                range,
                state,
                content: {
    ...
    addKeyboardShortcuts() {
        return {
            Backspace: () => this.editor.commands.command(({ tr, state }) => handleRestoreMentionBackspace({ transaction: tr, state, nodeName: this.name, char: '+' }))
        };
    }
});

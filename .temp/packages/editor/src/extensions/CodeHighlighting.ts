...
import { isRemoteTransaction } from "../utils/isRemoteTransaction.ts";
function parseNodes(nodes: any[], className: string[] = []): {
    text: string;
    classes: string[];
}[] {
    return nodes
...
export default function CodeHighlighting({ name }: {
    name: string;
}) {

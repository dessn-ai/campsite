...
import { findNodeAndPos } from "../utils/findNodeAndPos.ts";
import { insertNodes } from "../utils/insertNodes.ts";
import { MediaGalleryItem } from "./MediaGalleryItem.ts";
import { InlineAttachmentAttributes } from "./PostNoteAttachment.ts";
...
export const MediaGallery = Node.create<MediaGalleryOptions>({
    ...
    addCommands() {
        return {
            insertGallery: (galleryId, attachments, pos) => ({ tr, state, dispatch }) => {
                const { schema } = state;
            ...
            appendGalleryItem: (gallery_id: string, attachment: InlineAttachmentAttributes) => ({ tr, state, dispatch }) => {
                const { schema } = state;
            ...
            updateGalleryItem: (optimistic_id: string, value: Partial<InlineAttachmentAttributes>) => ({ tr, state, dispatch }) => {
                const match = findNodeAndPos(state, (n) => n.attrs.optimistic_id === optimistic_id && n.type.name === MediaGalleryItem.name);
                if (!match) {
                    return false;
                }
                const { pos } = match;
            ...
            removeGalleryItem: (optimistic_id: string) => ({ tr, state, dispatch }) => {
                let galleryPos = -1;
                ...
                state.doc.descendants((node, pos) => {
                    ...
                    node.descendants((child, childPos) => {
                        ...
                        if (child.type.name !== MediaGalleryItem.name)
                            return true;
                        // not the item we want to delete
                        if (child.attrs.optimistic_id !== optimistic_id)
                            return true;
                ...
                if (galleryPos === -1 || galleryItemPos === -1)
                    return false;
                const mediaGalleryNode = state.doc.nodeAt(galleryPos);
                if (!mediaGalleryNode)
                    return false;
                let newTr = tr.delete(galleryItemPos, galleryItemPos + mediaGalleryNode.child(galleryItemPos - galleryPos - 1).nodeSize);
                const updatedMediaGalleryNode = newTr.doc.nodeAt(galleryPos);
                if (!updatedMediaGalleryNode)
                    return false;
